...
import { diffSegmentBody, finalTail, hasDetails, parseTodos } from './helpers.js';
...
export class TurnController {
  ...
  pruneTransient() {
    ...
    patchTurnState(state => {
      ...
      return next.length === state.turnTrail.length ? state : {
        ...state,
        turnTrail: next
      };
    });
  ...
  flushStreamingSegment() {
    ...
    const split = raw ? hasReasoningTag(raw) ? splitReasoning(raw) : {
      reasoning: '',
      text: raw
    } : {
      reasoning: '',
  ...
  removeTrailGroup(name, ctx) {
    patchTurnState(state => {
      const next = state.turnTrail.filter(item => !sameToolTrailGroup(name, ctx, item));
      this.turnTools = next;
      return {
        ...state,
        turnTrail: next
  ...
  recordMessageComplete(payload) {
    ...
    if (finishedSubagents.length > 0) {
      pushSnapshot(finishedSubagents, {
        sessionId,
        startedAt: null
      });
  ...
  completeTool(toolId, fallbackName, error, summary, duration) {
    ...
    const technical = done?.technical || '';
    const fallbackDuration = done?.startedAt ? (Date.now() - done.startedAt) / 1000 : undefined;
    const line = buildToolTrailLine(name, ctx, Boolean(error), error || summary || technical || '', duration ?? fallbackDuration);
    this.activeTools = this.activeTools.filter(tool => tool.id !== toolId);
    const next = this.turnTools.filter(item => !sameToolTrailGroup(name, ctx, item));
    if (!this.activeTools.length) {
      next.push('Analisando saída da ferramenta…');
    }
  ...
  recordToolStart(toolId, name, context, technical = '') {
    // #region agent log
    const preStreaming = this.bufRef.trim();
    const preSegments = this.segmentMessages.length;
    // #endregion
    this.flushStreamingSegment();
    this.closeReasoningSegment();
    this.pruneTransient();
    this.endReasoningPhase();
    const sample = `${name} ${context} ${technical}`.trim();
    ...
    // #region agent log
    debugSessionLog('TurnController.ts:recordToolStart', 'tool.start state transition', {
      contextLen: context.length,
      name,
      postSegments: this.segmentMessages.length,
      postTools: this.activeTools.length,
      preSegments,
      preStreamingLen: preStreaming.length,
      toolId
    }, 'E');
    // #endregion
  }
  ...
  upsertSubagent(p, patch, opts = {
    ...
    patchTurnState(state => {
      ...
      const outputTail = p.output_tail ? p.output_tail.map(e => ({
