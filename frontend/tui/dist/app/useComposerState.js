...
export function looksLikeDroppedPath(text) {
  ...
  if (trimmed.startsWith('file://') || trimmed.startsWith('~/') || trimmed.startsWith('./') || trimmed.startsWith('../') || trimmed.startsWith('"/') || trimmed.startsWith("'/") || trimmed.startsWith('"~') || trimmed.startsWith("'~") || /^[A-Za-z]:[/\\]/.test(trimmed) || /^["'][A-Za-z]:[/\\]/.test(trimmed)) {
    return true;
  }
...
export function useComposerState({
  ...
  const {
    ...
    setCompIdx,
    compReplace
  } = useCompletion(input, isBlocked, gw);
  ...
  const handleResolvedPaste = useCallback(async ({
    bracketed,
    cursor,
    text,
    value
    ...
    void gw.request('paste.collapse', {
      text: cleanedText
    }).then(r => {
      const path = r?.path;
      if (!path) {
  ...
  const openEditor = useCallback(async () => {
    const dir = mkdtempSync(join(tmpdir(), 'ector-'));
    const file = join(dir, 'prompt.md');
    const [cmd, ...args] = resolveEditor();
    writeFileSync(file, [...inputBuf, input].join('\n'));
    let exitCode = null;
    await withInkSuspended(async () => {
      exitCode = spawnSync(cmd, [...args, file], {
        stdio: 'inherit'
      }).status;
    });
    try {
  ...
  const actions = useMemo(() => ({
    ...
    setQueueEdit,
    syncQueue
  }), [clearIn, dequeue, enqueue, handleTextPaste, openEditor, pushHistory, replaceQ, setCompIdx, setHistoryIdx, setQueueEdit, syncQueue]);
  const refs = useMemo(() => ({
    historyDraftRef,
    historyRef,
    queueEditRef,
