...
export const debugCommands = [{
  help: 'write a V8 heap snapshot + memory diagnostics (see ECTOR_HEAPDUMP_DIR)',
  name: 'heapdump',
  run: (_arg, ctx) => {
    const {
      heapUsed,
  ...
  run: (_arg, ctx) => {
    ...
    ctx.transcript.panel('Memory', [{
      rows: [['heap used', formatBytes(heapUsed)], ['heap total', formatBytes(heapTotal)], ['external', formatBytes(external)], ['array buffers', formatBytes(arrayBuffers)], ['rss', formatBytes(rss)], ['uptime', `${process.uptime().toFixed(0)}s`]]
    }]);
  }
}];