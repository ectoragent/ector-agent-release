...
export async function configureTerminalKeybindings(terminal, options) {
  ...
  try {
    ...
    const conflicts = TARGET_BINDINGS.filter(target => keybindings.some(existing => isKeybinding(existing) && existing.key === target.key && !sameBinding(existing, target)));
    if (conflicts.length) {
      return {
        success: false,
