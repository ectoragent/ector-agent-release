...
export async function performHeapDump(trigger = 'manual') {
  try {
    ...
    const dir = process.env.ECTOR_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.ector', 'heapdumps');
    await mkdir(dir, {
      recursive: true
    });
    const base = `ector-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`;
    ...
    await pipeline(getHeapSnapshot(), createWriteStream(heapPath, {
      mode: 0o600
    }));
    return {
      diagPath,
