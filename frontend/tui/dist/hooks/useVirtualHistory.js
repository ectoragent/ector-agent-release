...
export function useVirtualHistory(scrollRef, items, columns, {
  estimate = ESTIMATE,
  initialHeights,
  liveTailActive = false,
  onHeightsChange,
  ...
  useEffect(() => {
    ...
    let dirty = false;
    for (const k_0 of heights.current.keys()) {
      if (!keep.has(k_0)) {
        heights.current.delete(k_0);
        nodes.current.delete(k_0);
        refs.current.delete(k_0);
        dirty = true;
      }
    }
    if (dirty) {
  ...
  if (offsetsCache.current.version !== offsetVersion.current || offsetsCache.current.n !== n) {
    ...
    arr[0] = 0;
    for (let i_0 = 0; i_0 < n; i_0++) {
      arr[i_0 + 1] = arr[i_0] + Math.max(1, Math.floor(heights.current.get(items[i_0].key) ?? estimate));
    }
    offsetsCache.current = {
      arr,
      n,
      version: offsetVersion.current
    };
  }
  const offsets = offsetsCache.current.arr;
  const total = offsets[n] ?? 0;
  const headKey = items[0]?.key;
  if (n > 0 && prevItemCountRef.current === 0) {
    forceTailFollowRef.current = true;
  } else if (n > 0 && headKey !== undefined && prevItemsHeadRef.current !== undefined && headKey !== prevItemsHeadRef.current) {
    forceTailFollowRef.current = true;
  }
  prevItemsHeadRef.current = n > 0 ? headKey : undefined;
  ...
  const target_0 = Math.max(0, top + pendingDelta);
  ...
  // isSticky() can stay true (attr / renderer follow) while scrollTop has
  // already moved up — tail-only mounting then ignores scrollTop and paints a
  // giant topSpacer + tail, i.e. empty band in the viewport. Gate tail-follow
  // on geometry (same idea as getViewportSnapshot.atBottom).
  ...
  if (frozenRange) {
    ...
    if (vp <= 0) {
      // During SIGWINCH layout passes viewport height can be 0 briefly; remounting
      // only the tail while scrollTop is mid-history paints an empty viewport.
      if (prevRange.current && prevRange.current[0] < n) {
        start = prevRange.current[0];
        end = Math.min(prevRange.current[1], n);
      } else {
        start = Math.max(0, n - coldStartCount);
      }
    } else if (stickyTailMount) {
      ...
      // during catch-up.
      const MAX_SPAN = vp * 3;
      const rawLo = Math.min(top, target_0);
      const rawHi = Math.max(top, target_0);
  ...
  if (end - start > maxMounted) {
    stickyTailMount ? start = Math.max(0, end - maxMounted) : end = Math.min(n, start + maxMounted);
  }
  ...
  if (n > 0 && vp > 0 && !frozenRange) {
    ...
    for (let i_1 = start; i_1 < end; i_1++) {
      coverage += heights.current.get(items[i_1].key) ?? PESSIMISTIC;
    }
    if (stickyTailMount) {
  ...
  //
  // effStart: when start *decreases* (scroll up / older messages), the
  // deferred dStart still holds the previous *larger* index. Using that as
  // effStart would mount a window that starts *below* the viewport top →
  // blank strip until the deferred commit catches up. Prefer the urgent
  // `start` in that case; when start catches up past dStart, keep deferring
  // toward dStart so we do not spike mounts on fast scroll down through
  // history (mirrors the effEnd + pendingDelta>0 tail bypass).
  const dStart = useDeferredValue(start);
  const dEnd = useDeferredValue(end);
  let effStart = start < dStart ? start : dStart;
  let effEnd = end > dEnd ? dEnd : end;
  // Inverted range (large jump with deferred value lagging) or sticky tail
  // follow (scrollToBottom needs the tail mounted NOW so maxScroll lands on
  // content, not bottomSpacer) — skip deferral.
  if (effStart > effEnd || stickyTailMount) {
    effStart = start;
    effEnd = end;
  }
  ...
  // the user feels "stuck before bottom".
  if (pendingDelta > 0) {
    effEnd = end;
  }
  ...
  const measureRef = useCallback(key => {
    ...
    if (!fn) {
      fn = el => {
        ...
        const h_0 = measuredHeight(existing);
        if (h_0 > 0 && heights.current.get(key) !== h_0) {
          heights.current.set(key, h_0);
          offsetVersion.current++;
  ...
  useLayoutEffect(() => {
    ...
    if (skipMeasurement.current) {
      ...
      for (let i_2 = effStart; i_2 < effEnd; i_2++) {
        const k_1 = items[i_2]?.key;
        if (!k_1) {
          continue;
        }
        const h_1 = measuredHeight(nodes.current.get(k_1));
        if (h_1 > 0 && heights.current.get(k_1) !== h_1) {
          heights.current.set(k_1, h_1);
          dirty_0 = true;
        }
      }
    }
    if (s_1) {
      const next = {
        sticky: s_1.isSticky(),
        top: Math.max(0, s_1.getScrollTop() + s_1.getPendingDelta()),
        vp: Math.max(0, s_1.getViewportHeight())
      };
...
const measuredHeight = node => {
  const yoga = Math.ceil(node?.yogaNode?.getComputedHeight?.() ?? 0);
  if (yoga > 0) {
    return yoga;
  }
  return Math.ceil(node?.height ?? 0);
};