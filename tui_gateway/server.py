...
load_ector_dotenv(
    ector_home=_ector_home, project_env=Path(__file__).parent.parent / ".env"
)

try:
    from agent.session_cwd import bootstrap_interactive_terminal_cwd

    bootstrap_interactive_terminal_cwd()
except Exception:
    pass
...
# appends every unhandled exception to ~/.ector/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_ector_home, "logs", "tui_gateway_crash.log")
...
_pending: dict[str, tuple[str, threading.Event, str]] = {}
...
_SLASH_WORKER_TIMEOUT_S = max(
    5.0, float(os.environ.get("ECTOR_TUI_SLASH_TIMEOUT_S", "45") or 45)
)
_TITLE_THREAD_JOIN_TIMEOUT_S = max(
    0.0, float(os.environ.get("ECTOR_TUI_TITLE_JOIN_TIMEOUT_S", "4") or 4)
)
...
_pool = concurrent.futures.ThreadPoolExecutor(
    max_workers=max(2, int(os.environ.get("ECTOR_TUI_RPC_POOL_WORKERS", "4") or 4)),
...
from tui_gateway.slash_handlers import InProcessSlashRunner

_slash_exec_counts: dict[str, int] = defaultdict(int)


def _session_slash_runner(session: dict):
    """Return in-process slash runner (reads legacy ``slash_worker`` key)."""
    return session.get("slash_runner") or session.get("slash_worker")


def _set_session_slash_runner(session: dict, runner) -> None:
    session["slash_runner"] = runner
    session.pop("slash_worker", None)
...
def _wait_title_thread(session: dict | None, timeout: float = _TITLE_THREAD_JOIN_TIMEOUT_S) -> None:
    if not session:
        return
    thread = session.get("title_thread")
    if thread is None:
        return
    try:
        if thread.is_alive():
...
def shutdown_sessions_for_exit() -> None:
    """Give short-lived background work a chance to persist before EOF exit."""
    for session in list(_sessions.values()):
        _wait_title_thread(session)
        agent = session.get("agent")
        key = session.get("session_key")
        if agent and key:
            _persist_session_token_totals(agent, key)
...
_IDENTITY_REQUIRED_MSG = (
    "Sessão encerrada. Execute `ector login` para continuar."
)


def _require_identity(rid) -> dict | None:
    """Return an RPC error when no persisted identity session is available."""
    try:
        from ector_cli.identity_auth import is_logged_in

        if is_logged_in():
            return None
    except Exception:
        pass
    return _err(rid, 4010, _IDENTITY_REQUIRED_MSG)
...
def _block(event: str, sid: str, payload: dict, timeout: int | None = None) -> str:
    if timeout is None:
        ...
        timeout = get_wiser_timeout_seconds(_load_cfg())
...
def _clear_pending(sid: str | None = None) -> None:
    ...
    collaterally cancel Wiser (wiser) / sudo / secret prompts on unrelated
    ...
    for rid, entry in list(_pending.items()):
        owner_sid, ev, kind = entry[0], entry[1], entry[2] if len(entry) > 2 else ""
        if sid is None or owner_sid == sid:
            _answers[rid] = WISER_USER_CANCELLED if kind == "wiser.request" else ""
            ev.set()
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("ECTOR_TUI_PROVIDER", "").strip()
    ...
    try:
        from ector_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ECTOR_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _restart_slash_runner(session: dict):
    worker = _session_slash_runner(session)
    if worker:
        try:
            worker.close()
        except Exception:
            pass
    try:
        _set_session_slash_runner(
            session,
            InProcessSlashRunner(
                session["session_key"],
                getattr(session.get("agent"), "model", _resolve_model()),
            ),
        )
    except Exception:
        _set_session_slash_runner(session, None)
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from ector_cli.model_switch import parse_model_flags, switch_model
    ...
    if agent:
        ...
        _restart_slash_runner(session)
        _emit("session.info", sid, _session_info(agent))

    os.environ["ECTOR_MODEL"] = result.new_model
    os.environ["ECTOR_INFERENCE_MODEL"] = result.new_model
    ...
    if result.target_provider:
        os.environ["ECTOR_INFERENCE_PROVIDER"] = result.target_provider
        if os.environ.get("ECTOR_TUI_PROVIDER"):
            os.environ["ECTOR_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _get_usage(agent) -> dict:
    ...
    usage = {
        ...
        "auxiliary_input": g("session_auxiliary_input_tokens"),
        "auxiliary_output": g("session_auxiliary_output_tokens"),
        "auxiliary_total": g("session_auxiliary_total_tokens"),
        "auxiliary_calls": g("session_auxiliary_api_calls"),
        "auxiliary_cost_usd": float(g("session_auxiliary_estimated_cost_usd") or 0),
        "actual_cost_usd": float(g("session_actual_cost_usd") or 0),
    }
    ...
    try:
        ...
        usage["cost_status"] = (
            "actual" if getattr(agent, "session_cost_status", "") == "actual" else cost.status
        )
        if getattr(agent, "session_actual_cost_usd", 0) > 0:
            usage["cost_usd"] = float(agent.session_actual_cost_usd)
        elif cost.amount_usd is not None:
            usage["cost_usd"] = float(cost.amount_usd)
        elif getattr(agent, "session_estimated_cost_usd", 0) > 0:
            usage["cost_usd"] = float(agent.session_estimated_cost_usd)
    except Exception:
        pass
    return usage


def _persist_session_token_totals(agent, session_key: str) -> None:
    """Write cumulative agent token/cost totals into SessionDB (absolute set)."""
    db = _get_db()
    if db is None or not session_key or agent is None:
        return
    ...
    try:
        ...
        db.update_token_counts(
            ...
            estimated_cost_usd=estimated if estimated > 0 else None,
            cost_status=getattr(agent, "session_cost_status", None) or None,
            cost_source=getattr(agent, "session_cost_source", None) or None,
            billing_provider=getattr(agent, "provider", None) or None,
            billing_base_url=getattr(agent, "base_url", None) or None,
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        from ector_cli.config import is_legacy_display_personality

        personality = str(display_cfg.get("personality", "") or "").strip().lower()
        if (
            personality
            and not is_legacy_display_personality(personality)
            and isinstance(agent_cfg, dict)
            and not agent_cfg.get("personalities")
        ):
            warnings.append(
...
def _agent_cwd(session_key: str | None = None) -> str:
    """Return the directory the agent is logically working in.

    The Python process's ``os.getcwd()`` never changes after startup, but the
    terminal-tool environment tracks ``cd`` commands across turns via
    ``self.cwd``. Surfacing that value lets the TUI footer reflect the
    directory the agent actually operates in (e.g. after ``cd ../axis-web``).
    Falls back to configured ``terminal.cwd`` when no environment is active yet.
    """
    from agent.session_cwd import resolve_chat_cwd

    return resolve_chat_cwd(
        session_key, cwd_placeholder="launch", allow_default_env=False
    )


def _refresh_session_cwd_info(sid: str, session: dict) -> None:
    """Persist and push cwd updates to the TUI footer."""
    ...
    try:
        ...
        sync_session_cwd_from_env(
            session_key, allow_default_env=False, source="tui"
        )
        _emit("session.info", sid, _session_info(agent, session_key=session_key))
    except Exception:
        pass


def _session_info(agent, session_key: str | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from ector_cli.presentation import get_available_skills

        info["skills"] = get_available_skills()
    except Exception:
        pass
    try:
        from ector_cli.presentation import get_update_result
...
def _tool_technical(name: str, args: dict) -> str:
    try:
        from agent.display import build_tool_technical_summary

        return build_tool_technical_summary(name, args) or ""
    except Exception:
        return ""
...
def _agent_cbs(sid: str) -> dict:
    return dict(
        ...
        wiser_callback=lambda q, c: _block(
            "wiser.request", sid, {"question": q, "choices": c}
        ),
    )
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from ector_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from ector_cli.config import load_config as _load_full_cfg
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    try:
        ...
        _identity = get_persisted_session()
        if _identity is not None:
            identity_user_id = _identity.user_id or None
    except Exception:
        pass
    return AIAgent(
        model=model,
...
def _init_session(sid: str, key: str, agent, history: list, cols: int = 80):
    ...
    try:
        _set_session_slash_runner(
            _sessions[sid],
            InProcessSlashRunner(key, getattr(agent, "model", _resolve_model())),
        )
    except Exception:
        # Defer hard-failure to slash.exec; chat still works without slash runner.
        _set_session_slash_runner(_sessions[sid], None)
    try:
...
def _enrich_with_attached_images(user_text: str, image_paths: list[str]) -> str:
    from ector_cli.attachments import enrich_with_attached_images
...
def _(rid, params: dict) -> dict:
    identity_err = _require_identity(rid)
    if identity_err:
        return identity_err
    ...
    if params.get("worktree"):
        try:
            ...
            wt = _setup_worktree(repo)
            if wt and wt.get("path"):
                os.chdir(wt["path"])
                os.environ["TERMINAL_CWD"] = wt["path"]
    ...
    def _build() -> None:
        ...
        # unconditionally and tries to close the slash_runner it finds;
        # if _build is still mid-construction when close runs, close
        # finds slash_runner=None / notify unregistered and returns
        # cleanly — leaving us, the build thread, to later install the
        # runner + notify on an orphaned session dict.  The finally
        # block below detects the orphan and cleans up instead of
        # leaking resources and a global notify registration.
        worker = None
        notify_registered = False
        try:
            _status_update(sid, "status", "iniciando assistente…")
            from agent.session_cwd import prime_session_cwd

            prime_session_cwd(key)
            ...
            try:
                worker = InProcessSlashRunner(
                    key, getattr(agent, "model", _resolve_model())
                )
                _set_session_slash_runner(session, worker)
            except Exception:
                pass
            ...
            _emit("error", sid, {"message": f"Não foi possível iniciar o agente: {e}"})
        finally:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `ECTOR_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(
        rid,
        {
            "session_id": sid,
            "resumed": target,
            "title": db.get_session_title(target) or "",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        db.set_session_title(key, title, user_set=True)
        _emit("session.title", params.get("session_id", ""), {"title": title})
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"ector_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
def _(rid, params: dict) -> dict:
    ...
    if key:
        try:
            ...
            sync_session_cwd_from_env(key, allow_default_env=False, source="tui")
        except Exception:
            pass
    agent = session.get("agent")
    if agent and key:
    ...
    try:
        db = _get_db()
        key = session.get("session_key")
        if db is not None and key and not db.has_user_messages(key):
...
def _(rid, params: dict) -> dict:
    ...
    # _clear_pending() would collaterally cancel wiser/sudo/secret
...
# Powers the TUI's /agents overlay (see frontend/tui/src/components/agentsOverlay).
...
#
# Layout:  $ECTOR_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_ector_home() / "spawn-trees"
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from ector_cli.tui_run_session import record_tui_run_session

        record_tui_run_session(session.get("session_key", ""))
    except Exception:
        pass
    with session["history_lock"]:
        if session.get("running"):
    ...
    def run():
        ...
        try:
            ...
            prime_session_cwd(session_key)
            chat_cwd = resolve_chat_cwd(session_key)
            ...
            if (
                ...
                try:
                    ...
                    session["title_thread"] = maybe_auto_title(
                        _get_db(),
                        session.get("session_key") or sid,
                        text,
                        raw,
                        session.get("history", []),
                        title_callback=lambda title: (
                            _emit("session.title", sid, {"title": title})
            ...
            if (
                ...
                try:
                    from ector_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: ector_cli.voice unavailable")
                except Exception as e:
            ...
            _refresh_session_cwd_info(sid, session)
            try:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from ector_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _ector_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from ector_cli.attachments import detect_file_drop as _detect_file_drop
...
def _background_user_message(session: dict, prompt: str) -> str:
    """Inclui o transcripto recente do chat principal — prompts vagos («o que é isso?»)."""
    with session["history_lock"]:
        history = list(session.get("history", []))
    ...
    for msg in history[-8:]:
        ...
        if role not in ("user", "assistant"):
            continue
        body = _history_message_text(msg)
        if not body:
            continue
        if len(body) > 500:
            body = body[:497] + "..."
    ...
    return (
        "Contexto recente do chat principal (a pergunta em segundo plano refere-se a isto):\n"
        + "\n".join(lines[-4:])
        + f"\n\nPergunta em segundo plano: {prompt}"
    )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        pass
    user_message = _background_user_message(session, text.strip())
    ...
    if db is not None:
        try:
            ...
            db.create_session(
                task_id,
                source="tui",
                model=getattr(session.get("agent"), "model", None) or _resolve_model(),
                parent_session_id=parent_key,
            )
        except Exception:
            logger.debug("background create_session failed", exc_info=True)
    ...
    return _ok(
        rid,
        {"task_id": task_id, "parent_session_key": session["session_key"]},
    )
...
def _respond(rid, params, key):
    ...
    _, ev, _kind = entry if len(entry) > 2 else (*entry, "")
    _answers[r] = params.get(key, "")
    ev.set()
    return _ok(rid, {"status": "ok"})


@method("wiser.respond")
def _rpc_wiser_respond(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                # with the gateway's running-agent /provider guard.
                if session.get("running"):
                    return _err(
                        rid,
                        4009,
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from ector_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_ector_home), "display": display_ector_home()})
    if key == "full":
        return _ok(rid, {"config": _load_cfg()})
    if key == "prompt":
        return _ok(rid, {"prompt": _load_cfg().get("custom_prompt", "")})
    if key == "personality":
        from ector_cli.config import resolve_display_personality_overlay

        display = (_load_cfg().get("display") or {})
        return _ok(
            rid,
            {"value": resolve_display_personality_overlay(display) or "default"},
        )
    if key == "reasoning":
        cfg = _load_cfg()
        effort = str(
...
_TUI_HIDDEN: frozenset[str] = frozenset(
    {
        ...
        "approve",
        "deny",
        # CLI maintenance — still runnable via slash.exec, omitted from TUI menus.
        "redraw",
        "reset",
        "snapshot",
    }
...
def _tui_slash_completion_allowed(slash_cmd: str) -> bool:
    """Hide registry entries (and aliases like /snap) from TUI tab completion."""
    from ector_cli.commands import resolve_command

    name = slash_cmd.lstrip("/").lower()
    resolved = resolve_command(name)
    if resolved is not None:
        return resolved.name not in _TUI_HIDDEN
    return name not in _TUI_HIDDEN
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `ector` is interactive — use `/ector chat -q …` or run `ector` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`ector setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`ector gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`ector sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`ector config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m ector_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "ector_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _resolve_name(name: str) -> str:
    ...
    return parse_slash(f"/{name.lstrip('/')}").canonical or name
...
def _(rid, params: dict) -> dict:
    ...
    from ector_cli.slash_resolve import resolve_quick_command

    cfg = _load_cfg()
    qc = resolve_quick_command(name, cfg) if name else None
    if qc:
        if qc.get("type") == "exec":
            r = subprocess.run(
                qc.get("command", ""),
    ...
    from ector_cli.slash_resolve import resolve_plugin_handler, resolve_skill_message

    handler = resolve_plugin_handler(name)
    if handler:
        try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from ector_cli.commands import SlashCommandCompleter
        ...
        completer = SlashCommandCompleter(
            ...
            command_filter=_tui_slash_completion_allowed,
        )
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `ector provider` and
        # classic CLI's /provider picker). Do NOT overwrite with live
        # provider_model_ids() — that bypasses curation and pulls in
        # non-agentic models (e.g. Ector /models returns ~400 IDs including
...
def _mirror_slash_side_effects(sid: str, session: dict, command: str) -> str:
    ...
    # runner's running-agent /provider guard.
...
def _(rid, params: dict) -> dict:
    ...
    _base = cmd.lstrip("/").split(None, 1)[0] if cmd else ""
    if _base:
        _slash_exec_counts[_base] += 1
    if os.environ.get("ECTOR_LOG_SLASH") == "1":
        logger.info(
            "slash.exec",
            extra={
                "cmd": _base,
                "session_id": params.get("session_id", ""),
                "count": _slash_exec_counts[_base],
            },
        )
    ...
    if not worker:
        try:
            ...
            return _err(rid, 5030, f"slash runner start failed: {e}")
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ECTOR_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ECTOR_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in ("on", "off"):
        ...
        os.environ["ECTOR_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from ector_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["ECTOR_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from ector_cli.voice import start_continuous
...
@method("stats.get")
@method("insights.get")  # backward compat
def _(rid, params: dict) -> dict:
...
def _(rid, params: dict) -> dict:
    try:
        from ector_cli.slash_handlers.dispatch import _run_profile

        text = _run_profile("", "")
        lines = [ln.split(": ", 1) for ln in text.splitlines() if ": " in ln]
        return _ok(
            rid,
            {
                "sections": [
...
def _(rid, params: dict) -> dict:
    try:
        from ector_cli.slash_handlers.dispatch import _run_platforms

        text = _run_platforms("", "")
        rows = [[ln, ""] for ln in text.splitlines() if ln.strip()]
        return _ok(rid, {"rows": rows, "text": text})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    try:
        from ector_cli.slash_handlers.dispatch import _run_gquota

        return _ok(rid, {"text": _run_gquota("", "")})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ECTOR_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ECTOR_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_ector_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from ector_cli.skills_hub import do_install
        ...
        if action == "browse":
            from ector_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from ector_cli.skills_hub import inspect_skill
