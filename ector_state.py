#!/usr/bin/env python3
"""
SQLite State Store for Ector Agent.
...
class SessionDB:
    ...
    # With multiple ector processes (gateway + CLI sessions + worktree agents)
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 6:
                ...
                # providers that replay reasoning (OpenRouter, OpenAI, Ector).
            ...
            if current_version < 10:
                # v10: pin chats to top of dashboard sidebar
                for col_name, col_type in [
                    ("pinned", "INTEGER NOT NULL DEFAULT 0"),
                    ("pinned_at", "REAL"),
                ]:
            ...
            if current_version < 11:
                # v11: lock session title after explicit user rename (dashboard/CLI)
                try:
                    cursor.execute(
                        'ALTER TABLE sessions ADD COLUMN "title_user_set" '
        ...
        # Fresh DBs insert SCHEMA_VERSION directly (skipping incremental migrations).
        try:
            cursor.execute(
                'ALTER TABLE sessions ADD COLUMN "title_user_set" '
                "INTEGER NOT NULL DEFAULT 0"
    ...
    def merge_model_config(
        ...
        """Shallow-merge *patch* into the session's ``model_config`` JSON."""
        if not patch:
            return
        row = self.get_session(session_id)
        meta: Dict[str, Any] = {}
        raw = (row or {}).get("model_config")
        if raw:
            try:
    ...
    def set_session_title(
        ...
        *,
        ...
        ``user_set=True`` marks the title as user-owned so auto-title generation
        will not overwrite it. ``user_set=False`` clears that lock. When
        ``user_set`` is omitted, the update is skipped if the user already owns
        the title.
        """
        ...
        def _do(conn):
            ...
            if user_set is True:
                title_user_set = 0 if not title else 1
                cursor = conn.execute(
                    "UPDATE sessions SET title = ?, title_user_set = ? WHERE id = ?",
                    (title, title_user_set, session_id),
                )
            elif user_set is False:
                cursor = conn.execute(
                    "UPDATE sessions SET title = ?, title_user_set = 0 WHERE id = ?",
                    (title, session_id),
                )
            else:
                cursor = conn.execute(
                    "UPDATE sessions SET title = ? WHERE id = ? AND title_user_set = 0",
                    (title, session_id),
                )
    ...
    def is_session_title_user_set(self, session_id: str) -> bool:
        """Return True when the user explicitly set the session title."""
    ...
    def get_session_pinned(self, session_id: str) -> tuple[bool, Optional[float]]:
        """Return (pinned, pinned_at) for a session, or (False, None) if missing."""
        sid = self.resolve_session_id(session_id)
        if not sid:
            return False, None
        with self._lock:
            cursor = self._conn.execute(
                "SELECT pinned, pinned_at FROM sessions WHERE id = ?",
                (sid,),
            )
            row = cursor.fetchone()
        if not row:
            return False, None
        return bool(row["pinned"]), row["pinned_at"]
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Ector) receive
    ...
    def has_user_messages(self, session_id: str) -> bool:
        """Return True if the session has at least one persisted user message."""
        with self._lock:
            row = self._conn.execute(
    ...
    def _list_session_ids_matching(
    ...
    def delete_sessions_for_user(
        ...
        *,
        ...
        """Delete all sessions owned by *user_id*. Returns count deleted."""
        uid = (user_id or "").strip()
        if not uid:
            return 0
        ids = self._list_session_ids_matching("user_id = ?", (uid,))
        count = 0
        for sid in ids:
    ...
    def delete_untagged_sessions(
        self,
        *,
        sessions_dir: Optional[Path] = None,
    ) -> int:
        """Delete sessions with no ``user_id`` (legacy rows before identity tagging)."""
        ids = self._list_session_ids_matching(
    ...
    def delete_all_sessions(
        ...
        """Delete every session and all messages. Returns count of deleted sessions."""
        def _do(conn):
        ...
        if count and sessions_dir:
            for f in sessions_dir.iterdir():
                if f.suffix in (".json", ".jsonl") or f.name.startswith("request_dump_"):
                    try:
                        f.unlink()
                    except OSError:
                        pass
