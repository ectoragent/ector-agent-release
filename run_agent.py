...
from ector_cli.timeouts import (
...
_loaded_env_paths = load_ector_dotenv(ector_home=_ector_home, project_env=_project_env)
...
from agent.rag_context import build_rag_context_block, build_session_rag_context
...
from agent.prompt_builder import (
    ...
    build_iterative_workflow_guidance,
    LONG_RUNNING_TERMINAL_GUIDANCE,
    USER_FACING_RESPONSE_GUIDANCE,
    build_visible_reasoning_guidance,
    build_git_commit_coauthor_guidance,
)
...
from agent.prompt_builder import (
    ...
    build_session_working_directory_guidance,
    build_memory_hierarchy_guidance,
    has_skills_index_tools,
    load_soul_md,
    should_inject_git_coauthor_guidance,
    should_inject_skills_guidance,
    TOOL_USE_ENFORCEMENT_GUIDANCE,
    TOOL_USE_ENFORCEMENT_MODELS,
    GOOGLE_MODEL_OPERATIONAL_GUIDANCE,
    OPENAI_MODEL_EXECUTION_GUIDANCE,
    web_stack_disabled_guidance,
)
...
from agent.usage_recorder import (
    clear_active_usage_recorder,
    set_active_usage_agent,
)
...
from agent.user_status import (
    api_error_fallback_status,
    connection_exhausted_status,
    conversation_compressing_status,
    fallback_model_status,
    history_compressed_retry_status,
    history_compressing_status,
    incomplete_response_fallback_status,
    internet_unavailable_status,
    interrupt_during_reasoning_message,
    invalid_response_retry_status,
    rate_limit_fallback_status,
    rate_limit_pause_status,
    reasoning_status,
    reconnected_resuming_status,
    retries_exhausted_fallback_status,
    retry_pause_status,
    stale_response_status,
    tool_stream_reconnect_status,
    unstable_connection_status,
)
from agent.network_errors import (
    format_api_failure_final_response,
    is_likely_internet_unavailable,
    is_transient_connection_error,
    offline_error_summary,
    user_facing_api_error_summary,
)
...
class _SafeWriter:
    ...
    When ector-agent runs as a systemd service, Docker container, or headless
...
_NEVER_PARALLEL_TOOLS = frozenset({"wiser", "ask_user"})
...
def _should_parallelize_tool_batch(tool_calls) -> bool:
    ...
    tool_names = [canonical_wiser_tool_name(tc.function.name) for tc in tool_calls]
    if any(name in _NEVER_PARALLEL_TOOLS for name in tool_names):
        return False

    reserved_paths: list[Path] = []
    for tool_call in tool_calls:
        tool_name = canonical_wiser_tool_name(tool_call.function.name)
        try:
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"EctorAgent/{_ECTOR_VERSION}",
    }
...
def _read_git_global_author() -> tuple[str, str]:
    """Return global git author identity as (name, email) or empty strings."""
    ...
    name = _read_one("user.name")
    email = _read_one("user.email")
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[ector-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            wiser_callback (callable): Callback ``(question, choices) -> str`` for the ``wiser`` (Wiser) tool.
                Provided by the platform layer (CLI, gateway, or TUI). If None, ``wiser`` returns an error JSON.
        ...
        self.wiser_callback = wiser_callback
        ...
        self._cache_ttl = "5m"
        try:
            from ector_cli.config import load_config as _load_pc_cfg
        ...
        self._tool_closure_grace_call = False
        ...
        # both live under ~/.ector/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from ector_logging import (
            setup_logging,
            setup_verbose_logging,
            suppress_console_noise,
        )
        setup_logging(ector_home=_ector_home)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from ector_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://ector.cc",
                        "X-OpenRouter-Title": "Ector Agent",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif base_url_host_matches(effective_base, "api.routermint.com"):
                    client_kwargs["default_headers"] = _routermint_headers()
                elif base_url_host_matches(effective_base, "api.githubcopilot.com"):
                    from ector_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        if _explicit == "gemini":
                            _oauth_hint = (
                                " Ou conecte via OAuth com "
                                "`ector auth add google-gemini-cli` e escolha "
                                "o provedor «Google Gemini (OAuth)» em Configurações."
                            )
                        raise RuntimeError(
                            f"O provedor '{_explicit}' está configurado em config.yaml, "
                            f"mas nenhuma API key foi encontrada. Defina a variável de ambiente "
                            f"{_env_hint}{_oauth_hint} "
                            f"Ou mude de provedor com `ector provider`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "Nenhum provedor de LLM configurado. Execute `ector provider` para "
                        "selecionar um provedor, ou execute `ector setup` para a primeira"
                        "configuração."
                    )
        ...
        self.tools = self._filter_tools_for_plan_mode(self.tools)
        ...
        if self.tools:
            ...
            if "wiser" in self.valid_tool_names and self.wiser_callback is None:
                self.tools = [
                    t for t in self.tools
                    if t.get("function", {}).get("name") != "wiser"
                ]
                self.valid_tool_names.discard("wiser")
        ...
        # Session logs go into ~/.ector/sessions/ alongside gateway sessions
        ector_home = get_ector_home()
        self.logs_dir = ector_home / "sessions"
        ...
        # Session rows are created lazily in _flush_messages_to_session_db via
        # ensure_session() so opening a chat without sending a message does not
        # leave empty sessions in the index.
        ...
        try:
            from ector_cli.config import load_config as _load_agent_config
        ...
        self._rag_max_results = 4
        self._rag_max_chars = 2500
        self._rag_min_query_chars = 8
        _ctx_cfg = _agent_cfg.get("context", {}) or {}
        try:
            self._ephemeral_injection_cap_ratio = float(
                _ctx_cfg.get("ephemeral_injection_cap_percent", 0.15)
            )
        except (TypeError, ValueError):
            self._ephemeral_injection_cap_ratio = 0.15
        ...
        self._memory_session_delta_injection = False
        ...
        if not skip_memory:
            try:
                ...
                self._memory_session_delta_injection = bool(
                    mem_config.get("session_delta_injection", False)
                )
        ...
        # Lightweight RAG over session history (FTS-backed, per-turn injection).
        # Kept ephemeral and out of the system prompt to preserve prompt caching.
        try:
            _rag_cfg = _agent_cfg.get("rag", {})
            if not isinstance(_rag_cfg, dict):
                _rag_cfg = {}
            self._rag_enabled = bool(_rag_cfg.get("enabled", False))
            self._rag_max_results = max(1, min(int(_rag_cfg.get("max_results", 4)), 10))
            self._rag_max_chars = max(600, min(int(_rag_cfg.get("max_chars", 2500)), 12000))
            self._rag_min_query_chars = max(1, min(int(_rag_cfg.get("min_query_chars", 8)), 80))
        except Exception:
            self._rag_enabled = False
            self._rag_max_results = 4
            self._rag_max_chars = 2500
            ...
            try:
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        # (e.g. memory provider derives chat-scoped session keys)
                        if self._session_db:
                            try:
                        ...
                        # Gateway session key for stable per-chat memory session isolation
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "ector"
                        except Exception:
                            pass
        ...
        self._inject_memory_provider_tools()
        ...
        try:
            compression_prune_threshold = float(_compression_cfg.get("prune_threshold", 0))
        except (TypeError, ValueError):
            compression_prune_threshold = 0.0
        if compression_prune_threshold < 0:
            compression_prune_threshold = 0.0
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from ector_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from ector_cli.plugins import get_plugin_context_engine
        ...
        if _selected_engine is not None:
            ...
            self.context_compressor = ContextCompressor(
                ...
                prune_threshold_percent=compression_prune_threshold,
            )
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Ector Agent.  Choose a model with at least "
        ...
        self._inject_context_engine_tools()
        ...
        self._init_tool_routing(_agent_cfg)

        try:
            from agent.session_paths import resolve_session_cwd

            _hint_cwd = resolve_session_cwd(self.session_id) or None
        except Exception:
            _hint_cwd = os.getenv("TERMINAL_CWD") or None
        self._subdirectory_hints = SubdirectoryHintTracker(
            working_dir=_hint_cwd,
        )
        ...
        self.session_auxiliary_input_tokens = 0
        self.session_auxiliary_output_tokens = 0
        self.session_auxiliary_total_tokens = 0
        self.session_auxiliary_api_calls = 0
        self.session_auxiliary_estimated_cost_usd = 0.0
        self._system_prompt_dirty_layers: set = set()
    ...
    def reset_session_state(self):
        ...
        self.session_auxiliary_api_calls = 0
        self.session_auxiliary_estimated_cost_usd = 0.0
        self._system_prompt_dirty_layers = set()
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        Called by the /provider command handlers (CLI and gateway) after
        ...
        turn-scoped).
        """
        from ector_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from ector_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        ``suppress_status_output`` is a stricter automation mode used when no
        interactive UI is available. In that mode,
    ...
    def _emit_auxiliary_failure(self, task: str, exc: BaseException) -> None:
        ...
        self._emit_warning(f"▲ Tarefa auxiliar {task} falhou: {detail}")
    ...
    def _record_auxiliary_usage_callback(
        self,
        response_usage: Any,
        *,
        ...
        """Attribute auxiliary LLM usage (compression, vision, etc.) to this session."""
        canonical = normalize_usage(
            response_usage,
            provider=provider,
            api_mode=api_mode,
        )
        billing_model = model or getattr(self, "model", "") or ""
        billing_provider = provider or getattr(self, "provider", None)
        billing_base_url = base_url or getattr(self, "base_url", None)
        billing_key = api_key or getattr(self, "api_key", "")
        ...
        actual = extract_actual_cost_usd(
            response_usage,
            provider=billing_provider,
            base_url=billing_base_url,
        )

        self.session_auxiliary_input_tokens += canonical.input_tokens
        self.session_auxiliary_output_tokens += canonical.output_tokens
        self.session_auxiliary_total_tokens += canonical.total_tokens
        self.session_auxiliary_api_calls += 1
        if cost_result.amount_usd is not None:
            self.session_auxiliary_estimated_cost_usd += float(cost_result.amount_usd)

        # Roll auxiliary usage into session totals for /usage and analytics.
        self.session_prompt_tokens += canonical.prompt_tokens
        self.session_completion_tokens += canonical.output_tokens
        self.session_total_tokens += canonical.total_tokens
        self.session_input_tokens += canonical.input_tokens
        self.session_output_tokens += canonical.output_tokens
        self.session_cache_read_tokens += canonical.cache_read_tokens
        self.session_cache_write_tokens += canonical.cache_write_tokens
        self.session_reasoning_tokens += canonical.reasoning_tokens
        if cost_result.amount_usd is not None:
            self.session_estimated_cost_usd += float(cost_result.amount_usd)
        if actual is not None:
            self.session_actual_cost_usd += float(actual)
        ...
        if self._session_db and self.session_id:
            try:
                self._session_db.update_token_counts(
                    ...
                    cost_status="actual" if actual is not None else cost_result.status,
                    cost_source="provider_cost_api" if actual is not None else cost_result.source,
        ...
        logger.info(
            "Auxiliary usage (%s): model=%s in=%d out=%d total=%d",
            label,
            billing_model,
    ...
    def _record_main_api_usage(
        self,
        response_usage: Any,
        *,
        update_compressor: bool = True,
    ) -> None:
        """Normalize and persist usage from a main agent API completion."""
        canonical_usage = normalize_usage(
            response_usage,
            provider=self.provider,
            api_mode=self.api_mode,
        )
        if update_compressor:
            usage_dict = {
        ...
        self.session_prompt_tokens += canonical_usage.prompt_tokens
        self.session_completion_tokens += canonical_usage.output_tokens
        self.session_total_tokens += canonical_usage.total_tokens
        ...
        actual_cost = extract_actual_cost_usd(
            response_usage,
            provider=self.provider,
            base_url=self.base_url,
        )
        ...
        if actual_cost is not None:
            self.session_actual_cost_usd += float(actual_cost)
            self.session_cost_status = "actual"
            self.session_cost_source = "provider_cost_api"
        else:
        ...
        if self._session_db and self.session_id:
            try:
                self._session_db.update_token_counts(
                    ...
                    cost_status="actual" if actual_cost is not None else cost_result.status,
                    cost_source="provider_cost_api" if actual_cost is not None else cost_result.source,
    ...
    def _mark_system_prompt_layer_dirty(self, layer: str) -> None:
        if layer:
            self._system_prompt_dirty_layers.add(layer)

    def _rebuild_system_prompt_after_compression(self, system_message: str) -> str:
        """Rebuild the cached system prompt only when volatile layers changed."""
        dirty = getattr(self, "_system_prompt_dirty_layers", None) or set()
        if dirty:
            self._invalidate_system_prompt()
            prompt = self._build_system_prompt(system_message)
            self._system_prompt_dirty_layers.clear()
            return prompt
        if self._cached_system_prompt:
            return self._cached_system_prompt
        return self._build_system_prompt(system_message)

    def _cap_ephemeral_injections(self, parts: List[str]) -> List[str]:
        """Limit memory + RAG + plugin context injected per API call."""
        if not parts:
            return parts
        cap_ratio = getattr(self, "_ephemeral_injection_cap_ratio", 0.0) or 0.0
        if cap_ratio <= 0:
            return parts
        ctx_len = getattr(self.context_compressor, "context_length", 0) or 0
        if ctx_len <= 0:
            return parts
        from agent.token_estimation import count_text_tokens

        cap_tokens = max(1, int(ctx_len * cap_ratio))
        kept: List[str] = []
        used = 0
        for part in parts:
            part_tokens = count_text_tokens(part)
            if used + part_tokens <= cap_tokens:
                kept.append(part)
                used += part_tokens
                continue
            ...
            if remaining <= 0:
                break
            # Truncate the overflowing block (~4 chars/token fallback inside helper).
            approx_chars = max(0, remaining * 4 - 4)
            if approx_chars > 0:
                kept.append(part[:approx_chars] + "\n\n[… context truncated to fit token budget …]")
            break
        if len(kept) < len(parts) and not self.quiet_mode:
            logger.info(
                "Ephemeral injection capped: kept %d/%d blocks (~%d/%d tokens)",
                len(kept),
                len(parts),
                used,
                cap_tokens,
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    "▲ Nenhum provedor LLM auxiliar configurado — a compressão de "
                    "contexto vai descartar mensagens do meio sem resumo. "
                    "Execute `ector setup` ou defina OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Ector "
            ...
            if aux_context < threshold:
                ...
                msg = (
                    f"▲ O modelo de compressão ({aux_model}) tem "
                    f"{aux_context:,} tokens de contexto, mas o limiar de "
                    f"compressão do modelo principal era {old_threshold:,} tokens. "
                    f"Nesta sessão o limiar foi reduzido para "
                    f"{new_threshold:,} tokens para a compressão funcionar.\n"
                    f"  Para fixar de forma permanente, edite o config.yaml — opções:\n"
                    f"  1. Usar um modelo de compressão maior:\n"
                    f"       auxiliary:\n"
                    f"         compression:\n"
                    f"           model: <modelo-com-contexto-{old_threshold:,}+>\n"
                    f"  2. Baixar o limiar de compressão:\n"
                    f"       compression:\n"
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        helper, the hardcoded ``ECTOR_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("ECTOR_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ECTOR_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("ECTOR_API_CALL_STALE_TIMEOUT")
    ...
    def _anthropic_prompt_cache_policy(
        ...
        *,
        ...
        Qwen / Alibaba-family models on Alibaba (DashScope) also honour
        Anthropic-style ``cache_control`` markers on OpenAI-wire chat
        completions. Without markers these providers can serve zero cache
        ...
        # Qwen/Alibaba on native DashScope: OpenAI-wire transport that accepts
        # Anthropic-style cache_control markers and rewards them with real cache hits.
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from ector_cli.models import _should_use_copilot_responses_api
    ...
    def _strip_think_blocks(self, content: str) -> str:
        ...
        # 1b. Tool-call XML blocks. Handle the
        ...
        #     intentional asymmetry: do not strip bare <function> tails.)
        content = re.sub(
    ...
    _MEMORY_REVIEW_PROFILE_ROUTING = (
        "\n\nProfile routing (do not duplicate the ector.cc cloud profile): "
        "If the main system prompt already includes '# Perfil do usuário', do NOT save "
        "nickname, personality, initiative level, custom instructions, or timezone to "
        "memory(target='user'). Save only preferences not covered by that block. "
        "Use target='user' for personal work preferences; target='memory' for "
    ...
    _SKILL_REVIEW_PROMPT = (
        ...
        "Never hardcode absolute repo paths — reference the current session workspace only. "
        "Git/commit skills should be class-scoped (e.g. git-commit-push), not app-named.\n\n"
    ...
    _COMBINED_REVIEW_PROMPT = (
        ...
        "the class level, not this one session. Never hardcode absolute repo paths; "
        "use the current session cwd. Git/commit skills: class names, not app names.\n"
    ...
    def _summarize_background_review_actions(
        ...
        for msg in review_messages or []:
            ...
            if "skill" in msg_lower or "habilidade" in msg_lower:
                if "created" in msg_lower or "criada" in msg_lower or "desenvolvi" in msg_lower:
                    import re
                    name_match = re.search(r"['\"]([^'\"]+)['\"]", message)
                    skill_name = name_match.group(1) if name_match else ""
                    if skill_name:
                        actions.append(f"Eu desenvolvi uma habilidade ({skill_name})")
                    else:
                        actions.append("Eu desenvolvi uma habilidade nova")
                    continue
                if "updated" in msg_lower or "atualizada" in msg_lower or "melhorei" in msg_lower:
                    import re
                    name_match = re.search(r"['\"]([^'\"]+)['\"]", message)
                    skill_name = name_match.group(1) if name_match else ""
                    if skill_name:
                        actions.append(f"Eu melhorei a minha habilidade de '{skill_name}'")
                    else:
                        actions.append("Eu melhorei uma das minhas habilidades")
                    continue

            # Handle Memory/User profile actions (humanized)
            if target == "memory" or "memory" in msg_lower or "memória" in msg_lower:
                actions.append("Atualizei a minha memória.")
                continue
            if target == "user" or "profile" in msg_lower or "perfil" in msg_lower:
                actions.append("Atualizei o seu perfil.")
                continue

            # Fallback for other generic tool messages
            if "created" in msg_lower or "criada" in msg_lower:
                actions.append(message.rstrip("."))
            elif "updated" in msg_lower or "atualizada" in msg_lower or "added" in msg_lower:
                actions.append(message.rstrip("."))
            elif "removed" in msg_lower or "replaced" in msg_lower:
                actions.append(message.rstrip("."))
        return actions
    ...
    def _spawn_background_review(
        ...
        def _run_review():
            ...
            try:
                ...
                # Background review agents can initialize the active memory
                # provider plugin, which may own network clients.
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("ECTOR_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        status_code = getattr(error, "status_code", None)
        prefix = f"HTTP {status_code}: " if status_code else ""
        if is_likely_internet_unavailable(error):
            return offline_error_summary()
        if is_transient_connection_error(error):
            summary = user_facing_api_error_summary(error)
            if summary:
                return summary
    ...
    def _clean_error_message(self, error_msg: str) -> str:
        ...
        if error_msg.strip().startswith('<!DOCTYPE html') or '<html' in error_msg:
            return "Serviço temporariamente indisponível (a API retornou uma página de erro HTML)"
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("ECTOR_DUMP_REQUEST_STDOUT"):
    ...
    def commit_memory_session(self, messages: list = None) -> None:
        ...
        if not self._memory_manager:
            return
        self._mark_system_prompt_layer_dirty("memory")
        try:
            self._memory_manager.on_session_end(messages or [])
        except Exception:
            pass
    ...
    def _inject_memory_provider_tools(self) -> None:
        if not self._memory_manager or self.tools is None:
            return
        existing = self._tool_name_set()
        for _schema in self._memory_manager.get_all_tool_schemas():
            _tname = _schema.get("name", "")
            if not _tname or _tname in existing:
                continue
            self.tools.append({"type": "function", "function": _schema})
            self.valid_tool_names.add(_tname)
            existing.add(_tname)

    def _inject_context_engine_tools(self) -> None:
        if not getattr(self, "context_compressor", None) or self.tools is None:
            return
        existing = self._tool_name_set()
    ...
    def _init_tool_routing(self, agent_cfg: dict) -> None:
        """Initialize semantic/keyword tool routing (on by default)."""
        from toolsets import resolve_toolset

        tr_cfg = agent_cfg.get("tool_routing", {}) if isinstance(agent_cfg, dict) else {}
        self._tool_routing_cfg = tr_cfg if isinstance(tr_cfg, dict) else {}
        self._tool_router: Optional[ToolRouter] = None
        self._tool_routing_enabled = False
        self._routing_context: Optional[RoutingContext] = None
        self._expanded_tools_this_turn: set = set()
        self._tools_used_this_turn: set = set()
        self._routing_turn_query: str = ""
        self._routing_turn_embedding = None
        self._last_visible_tool_names: set = set()
        self._routing_logged_this_turn = False

        if not self.tools or not bool(self._tool_routing_cfg.get("enabled", True)):
            return

        def _expand_toolset(ts_name: str) -> set:
            try:
                return set(resolve_toolset(ts_name) or [])
            except Exception:
                return set()

        self._tool_router = ToolRouter.from_tools(
            self.tools,
            config=self._tool_routing_cfg,
            toolset_resolver=get_toolset_for_tool,
            toolset_expander=_expand_toolset,
        )
        self._tool_routing_enabled = self._tool_router.should_route()
        if self._tool_routing_enabled:
            register_routing_agent(self)
            if not self.quiet_mode:
                logger.info(
                    "Tool routing active (mode=%s, %d tools indexed)",
                    self._tool_router.config.mode,
                    len(self._tool_router.all_tool_names),
                )

    def _filter_tools_for_plan_mode(self, tools):
        try:
            from tools.agent_mode import filter_tools_for_plan_mode

            return filter_tools_for_plan_mode(tools or [], self.session_id or "")
        except Exception:
            return tools

    def _reload_tools_for_routing(self) -> None:
        """Refresh tool schemas and routing index after registry changes (MCP)."""
        try:
            self.tools = get_tool_definitions(
            ...
            self._inject_context_engine_tools()
            if self.tools:
                self.valid_tool_names = self._tool_name_set()
            if self._tool_router is not None:
                self._tool_router.rebuild(self.tools or [])
                self._tool_router.purge_stale_cache_entries()
                self._tool_routing_enabled = self._tool_router.should_route()
        except Exception as exc:
            logger.debug("Tool registry refresh failed: %s", exc)

    def _reset_tool_routing_for_turn(self, turn_query: str) -> None:
        """Reset per-turn routing state and precompute semantic embedding."""
        self._expanded_tools_this_turn = set()
        self._tools_used_this_turn = set()
        self._routing_turn_query = (turn_query or "").strip()
        self._routing_turn_embedding = None
        self._routing_context = RoutingContext(turn_query=self._routing_turn_query)
        self._last_visible_tool_names = set()
        self._routing_logged_this_turn = False
        if self._tool_router and self._tool_routing_enabled:
            self._routing_turn_embedding = self._tool_router.prepare_turn_embedding(
                self._routing_turn_query
            )
            if self._routing_turn_embedding is not None and self._routing_context is not None:
                self._routing_context.turn_embedding = self._routing_turn_embedding

    def _update_tool_routing_context(self, messages: list) -> None:
        """Refresh iteration-level routing query before an API call."""
        if not self._tool_router or not self._tool_routing_enabled:
            self._routing_context = None
            return
        self._routing_context = RoutingContext(
            turn_query=self._routing_turn_query,
            iteration_query=build_iteration_query(self._routing_turn_query, messages),
            turn_embedding=self._routing_turn_embedding,
            expanded=set(self._expanded_tools_this_turn),
            used_this_turn=set(self._tools_used_this_turn),
        )

    def _note_tool_execution(self, tool_name: str) -> None:
        """Track tool usage for routing (sticky tools within a turn)."""
        if not tool_name:
            return
        self._tools_used_this_turn.add(tool_name)
        if (
            self._tool_router
            and self._tool_routing_enabled
            and self._last_visible_tool_names
            and tool_name not in self._last_visible_tool_names
        ):
            self._expanded_tools_this_turn.add(tool_name)
            self._tool_router.expand_tool(tool_name)

    def _get_effective_tools(self) -> List[Dict[str, Any]]:
        """Return tool definitions sent to the LLM (routed when active)."""
        full = self.tools or []
        if not self._tool_router or not self._tool_routing_enabled:
            return full
        visible = self._tool_router.select_for_context(full, self._routing_context)
        self._last_visible_tool_names = {
            (t.get("function") or {}).get("name")
            for t in visible
            if isinstance(t, dict) and (t.get("function") or {}).get("name")
        }
        selection = self._tool_router.last_selection
        if selection and not self._routing_logged_this_turn:
            self._routing_logged_this_turn = True
            logger.info(
                "Tool routing: %d/%d tools visible (mode=%s)",
                selection.visible,
                selection.total,
                selection.mode,
            )
    ...
    def _prefetch_rag_context(self, query: str) -> str:
        """Retrieve per-turn RAG context from prior sessions."""
        if not self._rag_enabled or not self._session_db:
            return ""
        clean_query = (query or "").strip()
        if len(clean_query) < self._rag_min_query_chars:
            return ""
        try:
            payload = build_session_rag_context(
                db=self._session_db,
                query=clean_query,
                current_session_id=self.session_id,
                limit=self._rag_max_results,
                max_chars=self._rag_max_chars,
                role_filter=["user", "assistant"],
                exclude_sources=["background_process"],
            )
            return build_rag_context_block(payload)
        except Exception:
            return ""
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # User persona block: nickname / personality / behavior come from
        # GET /agent/auth/me, mirrored to config.user.* by identity_auth on
        # login and refreshed on each /me cache miss. Empty profile (e.g.
        # env-token override without a /me probe) renders to "" and is
        # filtered out by the join below.
        try:
            from agent.prompt_builder import build_user_profile_guidance
            from ector_cli.identity_auth import get_user_profile

            _user_block = build_user_profile_guidance(get_user_profile())
        except Exception:
            _user_block = ""
        if _user_block:
            prompt_parts.append(_user_block)
        ...
        try:
            ...
            if display.get("show_reasoning", True):
                prompt_parts.append(build_visible_reasoning_guidance())
        except Exception:
            pass
        ...
        try:
            from tools.agent_mode import is_plan_mode

            _skip_iterative = is_plan_mode(self.session_id or "")
            _skip_tool_enforcement = _skip_iterative
        except Exception:
            _skip_iterative = False
            _skip_tool_enforcement = False
        if not _skip_iterative:
            prompt_parts.append(build_iterative_workflow_guidance(self.valid_tool_names))
        if should_inject_git_coauthor_guidance(self.platform):
            _git_author_name, _git_author_email = _read_git_global_author()
            prompt_parts.append(
                build_git_commit_coauthor_guidance(
                    global_git_name=_git_author_name,
                    global_git_email=_git_author_email,
                )
            )
        ...
        if "memory" in self.valid_tool_names:
            ...
            tool_guidance.append(build_memory_hierarchy_guidance())
        if "session_search" in self.valid_tool_names:
            tool_guidance.append(SESSION_SEARCH_GUIDANCE)
        if should_inject_skills_guidance(self.valid_tool_names):
            tool_guidance.append(SKILLS_GUIDANCE)
        if "terminal" in self.valid_tool_names:
            tool_guidance.append(LONG_RUNNING_TERMINAL_GUIDANCE)
        if tool_guidance:
        ...
        if self.valid_tool_names:
            ...
            if _inject and not _skip_tool_enforcement:
            ...
            _web_gone = web_stack_disabled_guidance(self.valid_tool_names)
            if _web_gone:
                prompt_parts.append(_web_gone)
        ...
        if has_skills_index_tools(self.valid_tool_names):
            ...
            skills_prompt = build_skills_system_prompt(
                ...
                session_cwd=resolve_session_cwd(self.session_id),
            )
        else:
            skills_prompt = ""
        if skills_prompt:
        ...
        if not self.skip_context_files:
            # Prefer per-session cwd — TERMINAL_CWD is process-global and may
            # belong to another concurrent web chat.
            from agent.session_paths import resolve_session_cwd

            _context_cwd = resolve_session_cwd(self.session_id) or None
        ...
        _cwd_task = self.session_id if (self.platform or "").lower() in ("web", "tui") else None
        _cwd_block = build_session_working_directory_guidance(task_id=_cwd_task)
        if _cwd_block:
            prompt_parts.append(_cwd_block)
        ...
        from agent.prompt_builder import format_platform_hint

        _platform_hint_text = format_platform_hint(platform_key)
        if _platform_hint_text:
            prompt_parts.append(_platform_hint_text)

        if platform_key in ("cli", "web"):
            try:
                ...
                _local_name = "Web dashboard" if platform_key == "web" else "CLI"
                _cli_src = SessionSource(
                    platform=Platform.LOCAL,
                    chat_id=platform_key,
                    chat_name=_local_name,
                )
                _sess_ctx = build_session_context(_cli_src, _gw_cfg, None)
                if any(p != Platform.LOCAL for p in (_sess_ctx.connected_platforms or [])):
                    _ctx_block = build_session_context_prompt(_sess_ctx, redact_pii=False)
                    if _ctx_block.strip():
                        prompt_parts.append(_ctx_block)
            except Exception:
                pass
    ...
    def _repair_tool_call(self, tool_name: str) -> str | None:
        ...
        # Legacy tool ids (pre-``wiser`` registration) — map only when ``wiser`` is enabled.
        _legacy = canonical_wiser_tool_name(tool_name)
        if _legacy != tool_name and "wiser" in self.valid_tool_names:
            return _legacy
    ...
    def _invalid_tool_reply_content(self, tool_name: str, *, available: str) -> str:
        """Tool-role text when the model emits a name that is not in ``valid_tool_names``."""
        if tool_name in ("web_search", "web_extract", "web_crawl"):
            bits = [
                f"Tool '{tool_name}' is not enabled in this session (web toolset disabled or no provider API key). "
                "Configure a web backend with `ector tools` (EXA, Tavily, Firecrawl, Parallel, or Ector subscription gateway) "
                "and ensure the `web` toolset is enabled.",
            ]
            fallbacks = []
            if "browser_navigate" in self.valid_tool_names:
                fallbacks.append(
                    "browser_navigate + browser_snapshot on a public page (e.g. a news site) for current headlines"
                )
            if "terminal" in self.valid_tool_names:
                fallbacks.append("terminal/curl against reachable URLs when appropriate")
            if fallbacks:
                bits.append("Without web_search, use: " + "; ".join(fallbacks) + ".")
            if "session_search" in self.valid_tool_names:
                bits.append(
                    "session_search only searches past conversation logs — it does not fetch live web news."
                )
            bits.append(f"Enabled tools in this session: {available}")
            return " ".join(bits)
        return f"Tool '{tool_name}' does not exist in this session. Available tools: {available}"
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from ector_cli.copilot_auth import resolve_copilot_token
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        self._emit_status(reasoning_status())
        ...
        while t.is_alive():
            ...
            if _poll_count % 100 == 0:  # 100 × 0.3s = 30s
                ...
                self._emit_status(reasoning_status(int(_elapsed)))
            ...
            if _elapsed > _stale_timeout:
                ...
                self._emit_status(stale_response_status(int(_elapsed), streaming=False))
                try:
                    if self.api_mode == "anthropic_messages":
                        self._anthropic_client.close()
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # wins over the ECTOR_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("ECTOR_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # ECTOR_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("ECTOR_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode ECTOR_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("ECTOR_STREAM_RETRIES", 2))
                ...
                for _stream_attempt in range(_max_stream_retries + 1):
                    ...
                    try:
                        ...
                        if deltas_were_sent["yes"]:
                            ...
                            self._emit_status(
                                tool_stream_reconnect_status(
                                    _stream_attempt + 2, _max_stream_retries + 1
                                )
                            )
                            self._touch_activity(
                            ...
                            try:
                                ...
                                pass
                            self._emit_status(reconnected_resuming_status())
                            continue
                        ...
                        if _is_timeout or _is_conn_err or _is_sse_conn_err:
                            ...
                            if _stream_attempt < _max_stream_retries:
                                ...
                                continue
                            self._emit_status(
                                connection_exhausted_status(_max_stream_retries + 1)
                            )
                            logger.warning(
                            ...
                            if _is_stream_unsupported:
                                ...
                                self._safe_print(
                                    "\n▲  Streaming não é suportado para este "
                                    "modelo/provedor. Mudando para modo sem streaming.\n"
                                    "   Para evitar este atraso, defina display.streaming: false "
                                    "no config.yaml\n"
                                )
                            logger.info(
        ...
        _stream_stale_timeout_base = float(os.getenv("ECTOR_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set ECTOR_STREAM_STALE_TIMEOUT.
        ...
        while t.is_alive():
            ...
            if _hb_now - _last_heartbeat >= _HEARTBEAT_INTERVAL:
                ...
                self._emit_status(reasoning_status(_waiting_secs))
            ...
            if _stale_elapsed > _stream_stale_timeout:
                ...
                self._emit_status(stale_response_status(int(_stale_elapsed), streaming=True))
                try:
                    rc = request_client_holder.get("client")
                    if rc is not None:
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            try:
                from ector_cli.model_normalize import normalize_model_for_provider
            ...
            self._emit_status(fallback_model_status(fb_model))
            logging.info(
    ...
    def _try_recover_primary_transport(
        ...
        Skipped for proxy/aggregator providers (OpenRouter, Ector) which
        ...
        if provider_lower in ("ector", "ector-research"):
            return False
    ...
    def _describe_image_for_anthropic_fallback(self, image_url: str, role: str) -> str:
        ...
        from agent.vision_prompts import build_preanalysis_prompt

        analysis_prompt = build_preanalysis_prompt("")
        ...
        try:
            ...
            if isinstance(result, dict) and result.get("success"):
                ...
                # Do not treat error payloads as image descriptions.
                description = ""
        except Exception as e:
        ...
        if not vision_ok:
            local_source = vision_source
            local_path = Path(os.path.expanduser(str(local_source or "")))
            if local_path.is_file():
                try:
                    from tools.vision_tools import _local_image_analysis

                    local = _local_image_analysis(local_path)
                    if local and local.get("analysis"):
                        description = str(local["analysis"]).strip()
                        vision_ok = True
                except Exception:
                    pass
        ...
        from agent.vision_prompts import format_image_context_block, format_image_context_failed

        if vision_ok and description and description != "Image analysis failed.":
            note = format_image_context_block(description, vision_source)
        else:
            note = format_image_context_failed(vision_source)
    ...
    def _anthropic_preserve_dots(self) -> bool:
        ...
        Regression for #11976; mirrors the Bedrock allowlist fix for #5211."""
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        _is_ector_host = "ector" in self._base_url_lower
        ...
        # Anthropic max output for Claude on OpenRouter/Ector
        _ant_max = None
        if (_is_or or _is_ector_host) and "claude" in (self.model or "").lower():
            try:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "ector",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from ector_cli.models import github_model_reasoning_efforts
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        if summary_error:
            if getattr(self, "_last_compression_summary_warning", None) != summary_error:
                ...
                self._emit_warning(
                    f"▲ Resumo da compressão falhou: {summary_error}. "
                    "Foi inserido um marcador de contexto alternativo."
                )
        ...
        new_system_prompt = self._rebuild_system_prompt_after_compression(system_message)
        self._cached_system_prompt = new_system_prompt

        if self._session_db:
            try:
                platform_key = (self.platform or "").lower()
                # Web dashboard resumes by stable URL session id. Splitting the
                # SQLite row (CLI/gateway behaviour) writes post-compress traffic
                # to a tip child; the next /api/chat/send reloads the root and
                # drops context. Keep the same id and rewrite messages instead.
                if platform_key == "web":
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=self.platform or os.environ.get("ECTOR_SESSION_SOURCE", "cli"),
                        model=self.model,
        ...
        # compression (not a fresh /new). Plugin engines (e.g. ector-lcm) use
        # boundary_reason="compression" to preserve DAG lineage across the
        # rollover instead of re-initializing fresh per-session state.
        # See ector-lcm#68. Built-in ContextCompressor ignores kwargs.
        try:
        ...
        if _cc >= 2:
            self._vprint(
                f"{self.log_prefix}▲  Session compressed {_cc} times — "
    ...
    def _deliver_assistant_preamble_before_tools(self, assistant_message) -> None:
        """Push assistant preamble to stream/interim consumers before tool cards."""
        raw = getattr(assistant_message, "content", None)
        if raw is None and isinstance(assistant_message, dict):
            raw = assistant_message.get("content")
        visible = self._strip_think_blocks(raw or "").strip()
        if not visible or visible == "(empty)":
            return
        if self._interim_content_was_streamed(visible):
            return
        cb = getattr(self, "interim_assistant_callback", None)
        if cb is not None:
            try:
                ...
                logger.debug("interim preamble delivery failed", exc_info=True)
        elif self._has_stream_consumers():
            self._fire_stream_delta(visible if visible.endswith("\n") else f"{visible}\n\n")
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        try:
            self._deliver_assistant_preamble_before_tools(assistant_message)
    ...
    def _plan_mode_block_message(
        ...
        try:
            ...
            return get_plan_mode_block_message(
                self.session_id or "",
                function_name,
                function_args,
    ...
    def _plan_mode_block_result(
        self,
        function_name: str,
        function_args: dict,
    ) -> Optional[str]:
        block = self._plan_mode_block_message(function_name, function_args)
        if block is None:
            return None
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        try:
            from ector_cli.plugins import get_pre_tool_call_block_message
        ...
        function_name = canonical_wiser_tool_name(function_name)
        plan_block = self._plan_mode_block_result(function_name, function_args)
        if plan_block is not None:
            return plan_block
        self._note_tool_execution(function_name)
        ...
        if function_name == "todo":
        ...
        elif function_name == "mark_chapter":
            from tools.mark_chapter_tool import mark_chapter as _mark_chapter
            return _mark_chapter(
            ...
            result = _run_async(_memory_tool(
        ...
        elif function_name in ("wiser", "ask_user"):
            from tools.wiser_tool import wiser_tool as _wiser_tool

            return _wiser_tool(
                question=function_args.get("question", ""),
                choices=function_args.get("choices"),
                context=function_args.get("context"),
                callback=self.wiser_callback,
            )
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for tool_call in tool_calls:
            function_name = canonical_wiser_tool_name(tool_call.function.name)
            ...
            if function_name == "terminal" and self._checkpoint_mgr.enabled:
                try:
                    ...
                    if _is_destructive_command(cmd):
                        cwd = function_args.get("workdir") or os.getenv("TERMINAL_CWD", safe_getcwd())
        ...
        if self._should_emit_quiet_tool_messages() and self._should_start_quiet_spinner():
            spinner = KawaiiSpinner(
                f"⚡ running {num_tools} tools concurrently",
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if _block_msg is None:
                _block_msg = self._plan_mode_block_message(function_name, function_args)
            ...
            if _block_msg is not None:
                ...
                from model_tools import _run_async
                from tools.memory_tool import memory_tool as _memory_tool
                function_result = _run_async(_memory_tool(
                    action=function_args.get("action"),
                    target=target,
                    content=function_args.get("content"),
                    old_text=function_args.get("old_text"),
                ...
                if self._should_emit_quiet_tool_messages():
                    self._vprint(
                        f"  {_get_cute_tool_message_impl('wiser', function_args, tool_duration, result=function_result)}"
                    )
            elif function_name == "delegate_task":
                ...
                # Memory provider tools — not in the tool registry; routed via MemoryManager.
                spinner = None
                if self._should_emit_quiet_tool_messages() and self._should_start_quiet_spinner():
                    emoji = _get_tool_emoji(function_name)
                    preview = _build_tool_preview(function_name, function_args) or function_name
    ...
    def _handle_tool_closure_grace(
        self, messages: list, effective_task_id: str
    ) -> str | None:
        """One grace API call when the loop ends with pending tool results."""
        nudge = (
            "As ferramentas acima terminaram mas você não respondeu. "
            "Analise os resultados (incluindo falhas ou interrupções) e continue: "
            "explique o estado atual, proponha contramedidas concretas e "
            "execute o próximo passo se apropriado."
        )
        messages.append({"role": "user", "content": nudge})
        self._emit_status(
            "↻ Ferramentas concluídas sem resposta — pedindo continuação ao modelo"
        )
        ...
        try:
            if self.api_mode == "codex_responses":
                ...
                _transport = self._get_transport()
                assistant_message = _transport.normalize_response(response)
            elif self.api_mode == "anthropic_messages":
                _transport = self._get_transport()
                _ant_kw = _transport.build_kwargs(
                    model=self.model,
                    messages=self._prepare_anthropic_messages_for_api(api_messages),
                    tools=self._get_effective_tools(),
                    max_tokens=self.max_tokens,
                    reasoning_config=self.reasoning_config,
                    is_oauth=self._is_anthropic_oauth,
                ...
                response = self._anthropic_messages_create(_ant_kw)
                assistant_message = _transport.normalize_response(
                    response, strip_tool_prefix=self._is_anthropic_oauth
                )
            else:
                kwargs = self._build_api_kwargs(api_messages)
                response = self._ensure_primary_openai_client(
                    reason="tool_closure_grace"
                ).chat.completions.create(**kwargs)
                assistant_message = self._get_transport().normalize_response(response)

            tool_calls = getattr(assistant_message, "tool_calls", None) or []
            content = (getattr(assistant_message, "content", None) or "").strip()
            if tool_calls:
                interim = self._build_assistant_message(assistant_message, "tool_calls")
            ...
            logging.warning("tool closure grace failed: %s", exc)
        return None
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_ector_host:
                summary_extra_body["tags"] = ["product=ector-agent"]
            ...
            if final_response:
                ...
                if final_response:
                    ...
                    final_response = "Atingi o limite de iterações e não consegui gerar um resumo."
            else:
            ...
            final_response = f"Atingi o máximo de iterações ({self.max_iterations}), mas não consegui resumir. Erro: {str(e)}"
    ...
    def run_conversation(
        ...
        # ``ector logs --session <id>`` can filter a single conversation.
        from ector_logging import set_session_context
        set_session_context(self.session_id)

        clear_active_usage_recorder()
        set_active_usage_agent(self)
        ...
        # Strip leaked <memory-context> blocks from user input.  When a memory
        # provider persists a turn that included injected context, the block
        ...
        self._tool_closure_grace_attempted = False
        ...
        if self.api_mode != "anthropic_messages":
            try:
                if self._cleanup_dead_connections():
                    self._emit_status(
                        "🔌 Limpando conexões antigas após falha anterior "
                        "detectadas — limpeza automática. Seguindo com conexão nova."
                    )
            except Exception:
                pass
        ...
        self._reset_tool_routing_for_turn(original_user_message)
        ...
        # Add user message. Stamped with the real turn-start time — flushing to
        # the session DB happens once at turn exit, so without this both the
        # user and assistant rows would get ~the same time.time(), collapsing
        # the dashboard's "turn duration" display to its 1s floor.
        ...
        if self._cached_system_prompt is None:
            ...
            # Stale snapshot after openProject / landing bind: the SQLite prompt
            # may still say home while resolve_session_cwd already points at the
            # real project — rebuilding avoids the model "delirating" on /Users/…
            if stored_prompt and self.session_id:
                try:
                    from agent.session_paths import resolve_session_cwd

                    current_cwd = resolve_session_cwd(self.session_id)
                    if current_cwd:
                        current_resolved = str(
                            Path(current_cwd).expanduser().resolve()
                        )
                        marker = (
                            f"Terminal e ferramentas de arquivo rodam em: "
                            f"`{current_resolved}`"
                        )
                        if marker not in stored_prompt:
                            # Also accept the path with a different resolve form
                            # (symlink) by checking basename + that home isn't
                            # the only cwd claim.
                            if current_resolved not in stored_prompt:
                                stored_prompt = None
                except Exception:
                    pass
        ...
        if (
            ...
            from agent.token_estimation import estimate_messages_tokens

            _preflight_tokens = estimate_messages_tokens(
                messages,
                system_prompt=active_system_prompt or "",
                tools=self._get_effective_tools() or None,
            )
        ...
        # are reused.  The system prompt is Ector's territory; plugins
        ...
        try:
            ...
            _rag_prefetch_cache = self._prefetch_rag_context(_rag_query)
        except Exception:
            _rag_prefetch_cache = ""

        _session_delta_cache = ""
        if self._memory_session_delta_injection and self._memory_store:
            try:
                _delta_raw = self._memory_store.format_session_delta_block()
                if _delta_raw:
                    _session_delta_cache = build_memory_context_block(_delta_raw)
            except Exception:
                _session_delta_cache = ""
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            if self._budget_grace_call:
                ...
                if not self.quiet_mode:
                    self._safe_print(f"\n▲  Iteration budget exhausted ({self.iteration_budget.used}/{self.iteration_budget.max_total} iterations used)")
                break
            ...
            for idx, msg in enumerate(messages):
                ...
                if idx == current_turn_user_idx and msg.get("role") == "user":
                    ...
                    if _session_delta_cache:
                        _injections.append(_session_delta_cache)
                    if _rag_prefetch_cache:
                        _injections.append(_rag_prefetch_cache)
                    if _plugin_user_context:
                        _injections.append(_plugin_user_context)
                    if _injections:
                        _injections = self._cap_ephemeral_injections(_injections)
                    if _injections:
            ...
            # cache prefix.  The system prompt is reserved for Ector internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            self._update_tool_routing_context(messages)
            ...
            if not self.quiet_mode:
                ...
                self._vprint(f"{self.log_prefix}   🔧 Available tools: {len(self._get_effective_tools())}")
            else:
                # Animated thinking spinner in quiet mode (Braille dots only)
                verb = random.choice(KawaiiSpinner.get_thinking_verbs())
                if self.thinking_callback:
                    # CLI TUI mode: use prompt_toolkit widget instead of raw spinner
                    # (works in both streaming and non-streaming modes)
                    self.thinking_callback(f"{verb}...")
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("ECTOR_DUMP_REQUESTS"):
                    ...
                    if response_invalid:
                        ...
                        if thinking_spinner:
                            thinking_spinner.stop("(´;ω;`) ops, tentando de novo...")
                            thinking_spinner = None
                        if self.thinking_callback:
                            self.thinking_callback("")
                        ...
                        if self._fallback_index < len(self._fallback_chain):
                            self._emit_status(incomplete_response_fallback_status())
                        ...
                        self._vprint(f"{self.log_prefix}▲  Invalid API response (attempt {retry_count}/{max_retries}): {', '.join(error_details)}", force=True)
                        ...
                        if retry_count >= max_retries:
                            ...
                            self._emit_status(invalid_response_retry_status(max_retries))
                            if self._try_activate_fallback():
                                retry_count = 0
                                compression_attempts = 0
                                primary_recovery_attempted = False
                                continue
                            self._emit_status(f"❌ Limite de tentativas ({max_retries}) para respostas inválidas atingido. Desistindo.")
                        ...
                        while time.time() < sleep_end:
                            if self._interrupt_requested:
                                self._vprint(f"{self.log_prefix}⚡ Interrupção detectada durante a espera entre tentativas, encerrando.", force=True)
                                self._persist_session(messages, conversation_history)
                                self.clear_interrupt()
                                return {
                                    "final_response": f"Operação interrompida durante a retentativa ({_failure_hint}, tentativa {retry_count}/{max_retries}).",
                                    "messages": messages,
                                    "api_calls": api_call_count,
                                    "completed": False,
                                    "interrupted": True,
                    ...
                    if self.api_mode == "codex_responses":
                        ...
                        if self._should_treat_stop_as_truncated(
                            ...
                            self._vprint(
                                f"{self.log_prefix}▲  Treating suspicious Ollama/GLM stop response as truncated",
                                force=True,
                            )
                            finish_reason = "length"

                    if finish_reason == "length":
                        self._vprint(f"{self.log_prefix}▲  Response truncated (finish_reason='length') - model hit max output tokens", force=True)
                        ...
                        if _thinking_exhausted:
                            ...
                            _exhaust_response = (
                                ...
                                "→ Or switch to a larger/non-reasoning model with `/provider`"
                            )
                        ...
                        if self.api_mode in ("chat_completions", "bedrock_converse", "anthropic_messages"):
                            ...
                            if assistant_message is not None and _trunc_has_tool_calls:
                                if truncated_tool_call_retries < 1:
                                    ...
                                    self._vprint(
                                        f"{self.log_prefix}▲  Chamada de ferramenta truncada — nova tentativa da API...",
                                        force=True,
                                    )
                                    ...
                                    continue
                                self._vprint(
                                    f"{self.log_prefix}▲  Resposta com chamada de ferramenta truncada de novo — não executando argumentos incompletos.",
                                    force=True,
                                )
                                self._cleanup_task_resources(effective_task_id)
                                self._persist_session(messages, conversation_history)
                    ...
                    if hasattr(response, 'usage') and response.usage:
                        self._record_main_api_usage(response.usage)
                        canonical_usage = normalize_usage(
                            response.usage,
                            provider=self.provider,
                            api_mode=self.api_mode,
                    ...
                    self._vprint(f"{self.log_prefix}⚡ Interrompido durante a chamada à API.", force=True)
                    self._persist_session(messages, conversation_history)
                    interrupted = True
                    final_response = interrupt_during_reasoning_message(api_elapsed)
                    break

                except Exception as api_error:
                    # Stop spinner before printing error messages
                    if thinking_spinner:
                        thinking_spinner.stop("(╥_╥) erro, tentando de novo...")
                        thinking_spinner = None
                    if self.thinking_callback:
                        self.thinking_callback("")
                    ...
                    if isinstance(api_error, UnicodeEncodeError) and getattr(self, '_unicode_sanitization_passes', 0) < 2:
                        ...
                        if _surrogates_found or _is_surrogate_error:
                            ...
                            if _surrogates_found:
                                self._vprint(
                                    f"{self.log_prefix}▲  Removidos caracteres substitutos inválidos das mensagens. Tentando de novo...",
                                    force=True,
                                )
                            else:
                                self._vprint(
                                    f"{self.log_prefix}▲  Erro de codificação (surrogate) — nova tentativa após sanitizar o payload...",
                                    force=True,
                                )
                            continue
                        if _is_ascii_codec:
                            ...
                            if _raw_key:
                                ...
                                if _clean_key != _raw_key:
                                    ...
                                    self._vprint(
                                        f"{self.log_prefix}▲  API key contained non-ASCII characters "
                            ...
                            if _any_sanitized:
                                self._vprint(
                                    f"{self.log_prefix}▲  System encoding is ASCII — stripped non-ASCII characters from request payload. Retrying...",
                                    force=True,
                                )
                            else:
                                self._vprint(
                                    f"{self.log_prefix}▲  System encoding is ASCII — enabling full-payload sanitization for retry...",
                                    force=True,
                                )
                            continue
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Ector-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Remova chaves obsoletas editando {_dhh}/.env")
                    ...
                    if (
                        ...
                        self._vprint(
                            f"{self.log_prefix}▲  Assinatura do bloco de raciocínio inválida — "
                            f"blocos de thinking removidos, tentando de novo...",
                            force=True,
                        )
                        logging.warning(
                    ...
                    self._touch_activity(
                        f"Recuperação de erro da API (tentativa {retry_count}/{max_retries})"
                    )
                    
                    error_type = type(api_error).__name__
                    error_msg = str(api_error).lower()
                    _error_summary = self._summarize_api_error(api_error)
                    if is_likely_internet_unavailable(api_error):
                        consecutive_offline_errors += 1
                        self._emit_status(internet_unavailable_status())
                        if consecutive_offline_errors >= 2:
                            retry_count = max_retries
                    elif is_transient_connection_error(api_error):
                        consecutive_offline_errors = 0
                    logger.warning(
                    ...
                    self._vprint(f"{self.log_prefix}▲  API call failed (attempt {retry_count}/{max_retries}): {error_type}{_status_code_str}", force=True)
                    ...
                    if self._interrupt_requested:
                        ...
                        self.clear_interrupt()
                        return {
                            "final_response": f"Operação interrompida: tratando erro de API ({error_type}: {self._clean_error_message(str(api_error))}).",
                            "messages": messages,
                            "api_calls": api_call_count,
                            "completed": False,
                            "interrupted": True,
                    ...
                    if classified.reason == FailoverReason.long_context_tier:
                        ...
                        if old_ctx > _reduced_ctx:
                            ...
                            self._vprint(
                                f"{self.log_prefix}▲  Anthropic long-context tier "
                        ...
                        if compression_attempts <= max_compression_attempts:
                            ...
                            if len(messages) < original_len or old_ctx > _reduced_ctx:
                                self._emit_status(
                                    f"🗜️ Contexto reduzido para {_reduced_ctx:,} tokens "
                                    f"(antes {old_ctx:,}), tentando de novo..."
                                )
                                time.sleep(2)
                    ...
                    if is_rate_limited and self._fallback_index < len(self._fallback_chain):
                        ...
                        if not pool_may_recover:
                            self._emit_status(rate_limit_fallback_status())
                    ...
                    if is_payload_too_large:
                        ...
                        if len(messages) < original_len:
                            self._emit_status(history_compressed_retry_status())
                    ...
                    if is_context_length_error:
                        ...
                        if available_out is not None:
                            ...
                            self._vprint(
                                f"{self.log_prefix}▲  Output cap too large for current prompt — "
                        ...
                        if new_ctx and new_ctx < old_ctx:
                            ...
                            self._vprint(f"{self.log_prefix}▲  Context length exceeded — stepping down: {old_ctx:,} → {new_ctx:,} tokens", force=True)
                        else:
                    ...
                    if is_client_error:
                        ...
                        self._emit_status(api_error_fallback_status())
                        if self._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            primary_recovery_attempted = False
                        ...
                        self._emit_status(
                            f"❌ Erro sem nova tentativa (HTTP {status_code}): "
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `ector auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: ector setup", force=True)
                        ...
                        if status_code == 400 and (approx_tokens > 50000 or len(api_messages) > 80):
                            self._vprint(
                                f"{self.log_prefix}▲  Skipping session persistence "
                    ...
                    if retry_count >= max_retries:
                        ...
                        self._emit_status(retries_exhausted_fallback_status(max_retries))
                        if self._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            primary_recovery_attempted = False
                        ...
                        if is_rate_limited:
                            self._emit_status(f"❌ Limite de taxa após {max_retries} tentativas — {_final_summary}")
                        else:
                            self._emit_status(f"❌ Falha da API após {max_retries} tentativas — {_final_summary}")
                        ...
                        self._persist_session(messages, conversation_history)
                        _final_response = format_api_failure_final_response(
                            api_error,
                            max_retries=max_retries,
                            stream_drop=_is_stream_drop,
                        )
                        return {
                    ...
                    if is_rate_limited:
                        ...
                        self._emit_status(
                            retry_pause_status(wait_time, retry_count, max_retries)
                        )
                    logger.warning(
                    ...
                    while time.time() < sleep_end:
                        if self._interrupt_requested:
                            ...
                            self.clear_interrupt()
                            return {
                                "final_response": f"Operação interrompida: repetindo chamada de API após erro (tentativa {retry_count}/{max_retries}).",
                                "messages": messages,
                                "api_calls": api_call_count,
                                "completed": False,
                                "interrupted": True,
                        ...
                        if _backoff_touch_counter % 150 == 0:  # 150 × 0.2s = 30s
                            self._touch_activity(
                                f"Espera entre tentativas após erro ({retry_count}/{max_retries}), "
                                f"faltam {int(sleep_end - time.time())}s"
                            )
            ...
            try:
                ...
                # delegation to relay the child's first-line commentary to the parent).
                # Never publish assistant *content* as reasoning.available — that
                # text is user-facing narration (TEXT / interim). Emitting it as
                # REASONING makes the web UI show the same line under «Pensou».
                ...
                if has_incomplete_scratchpad(assistant_message.content or ""):
                    ...
                    self._vprint(f"{self.log_prefix}▲  Incomplete <REASONING_SCRATCHPAD> detected (opened but never closed)")
                ...
                if assistant_message.tool_calls:
                    ...
                    if invalid_tool_calls:
                        ...
                        self._vprint(
                            f"{self.log_prefix}▲  Tool '{invalid_preview}' unavailable in this session "
                            f"— sending error to model for self-correction ({self._invalid_tool_retries}/3)"
                        )
                        ...
                        for tc in assistant_message.tool_calls:
                            if tc.function.name not in self.valid_tool_names:
                                content = self._invalid_tool_reply_content(
                                    tc.function.name, available=available
                                )
                            else:
                    ...
                    if invalid_json_args:
                        ...
                        if _truncated:
                            self._vprint(
                                f"{self.log_prefix}▲  Truncated tool call arguments detected "
                        ...
                        self._vprint(f"{self.log_prefix}▲  Invalid JSON in tool call arguments for '{tool_name}': {error_msg}")
                        ...
                        if self._invalid_json_retries < 3:
                            ...
                            self._vprint(f"{self.log_prefix}▲  Injecting recovery tool results for invalid JSON...")
                    ...
                    if _compressor.last_prompt_tokens > 0:
                        ...
                        _real_tokens = estimate_messages_tokens(
                            messages,
                            system_prompt=active_system_prompt or "",
                            tools=self._get_effective_tools() or None,
                        )
                    ...
                    if (
                        self.compression_enabled
                        and hasattr(_compressor, "should_prune")
                        and _compressor.should_prune(_real_tokens)
                    ):
                        pruned_messages, pruned_count = _compressor.prune_tool_results(
                            messages,
                        )
                        if pruned_count:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if self._has_content_after_think_block(_partial_streamed):
                            ...
                            self._emit_status(
                                "↻ Stream interrompido — usando o conteúdo já "
                                "entregue como resposta final"
                            )
                        ...
                        if fallback and getattr(self, '_last_content_tools_all_housekeeping', False):
                            ...
                            self._emit_status("↻ Resposta vazia após ferramentas — usando o conteúdo anterior como resposta final")
                            self._last_content_with_tools = None
                            self._last_content_tools_all_housekeeping = False
                            self._empty_content_retries = 0
                        ...
                        if (
                            ...
                            self._emit_status(
                                "▲ Modelo respondeu vazio após ferramentas — "
                                "reforçando para continuar"
                            )
                        ...
                        if _has_structured and self._thinking_prefill_retries < 2:
                            ...
                            self._emit_status(
                                f"↻ Só raciocínio estruturado, sem texto visível — "
                                f"continuando com preenchimento ({self._thinking_prefill_retries}/2)"
                            )
                        ...
                        if _truly_empty and (not _has_structured or _prefill_exhausted) and self._empty_content_retries < 3:
                            ...
                            self._emit_status(
                                f"▲ Resposta vazia do modelo — tentando de novo "
                        ...
                        if _truly_empty and self._fallback_chain:
                            ...
                            self._emit_status(
                                "▲ Modelo só devolve respostas vazias — "
                                "alternando para provedor de fallback..."
                            )
                            if self._try_activate_fallback():
                                self._empty_content_retries = 0
                                self._emit_status(
                                    f"↻ Fallback ativado: {self.model} "
                                    f"({self.provider})"
                                )
                                logger.info(
                        ...
                        if reasoning_text:
                            ...
                            self._emit_status(
                                "▲ O modelo gerou raciocínio mas nenhuma resposta "
                                "visível após todas as tentativas. Retornando vazio."
                            )
                        else:
                            logger.warning(
                                "Empty response (no content or reasoning) "
                            ...
                            self._emit_status(
                                "❌ Modelo não devolveu conteúdo após todas as tentativas"
                                + (" e tentativas de fallback." if self._fallback_chain else
                                   ". Nenhum provedor de fallback configurado.")
                            )
                    ...
                    if not self.quiet_mode:
                        self._safe_print(f"🎉 Conversa concluída após {api_call_count} chamada(s) à API compatível com OpenAI")
                    break
                
            except Exception as e:
                error_msg = f"Erro na chamada compatível com OpenAI #{api_call_count}: {str(e)}"
                try:
                    print(f"❌ {error_msg}")
                except (OSError, ValueError):
                        ...
                        for tc in msg["tool_calls"]:
                            ...
                            if tc["id"] not in answered_ids:
                                err_msg = {
                                    "role": "tool",
                                    "tool_call_id": tc["id"],
                                    "content": f"Erro ao executar a ferramenta: {error_msg}",
                                }
                ...
                if api_call_count >= self.max_iterations - 1:
                    ...
                    final_response = f"Desculpe, mas encontrei erros repetidos: {error_msg}"
        ...
        if (
            ...
            and not self._tool_closure_grace_attempted
        ):
            self._tool_closure_grace_attempted = True
            grace_text = self._handle_tool_closure_grace(messages, effective_task_id)
            if grace_text and grace_text.strip():
                final_response = grace_text.strip()
                _turn_exit_reason = "tool_closure_grace"
        ...
        if final_response is None and (
            ...
            self._emit_status(
                f"▲ Orçamento de iterações esgotado ({api_call_count}/{self.max_iterations}) "
                "— pedindo resumo ao modelo"
            )
            if not self.quiet_mode:
                self._safe_print(
                    f"\n▲  Orçamento de iterações esgotado ({api_call_count}/{self.max_iterations}) "
                    "— solicitando resumo..."
                )
