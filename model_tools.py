...
def get_tool_definitions(
    ...
    # When web search is enabled, nudge models away from using ``wiser`` as a
    # permission gate or substitute for lookups (news, prices, versions, etc.).
    if "wiser" in available_tool_names and "web_search" in available_tool_names:
        _wiser_extra = (
            "\n\nRuntime note: ``web_search`` is enabled — use it for factual "
            "or time-sensitive questions; do not call ``wiser`` to ask whether to search."
        )
        for i, td in enumerate(filtered_tools):
            fn = td.get("function", {})
            if fn.get("name") != "wiser":
                continue
            desc = (fn.get("description") or "").rstrip()
            if _wiser_extra.strip() not in desc:
                filtered_tools[i] = {
                    "type": "function",
                    "function": {**fn, "description": desc + _wiser_extra},
                }
            break
