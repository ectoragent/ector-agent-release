...
def _split_gateway_env_list(name: str) -> List[str]:
    """Parse a comma- or semicolon-separated gateway env list (best-effort JSON array)."""
...
def _whatsapp_send_target_token(raw: str) -> str:
    v = (raw or "").strip()
    if not v:
        return ""
...
def _whatsapp_send_routing_lines(*, redact_pii: bool, home_channel_id: str = "") -> List[str]:
    """Inject WhatsApp allowlists + cached contacts so send_message can target real IDs."""
    home = (home_channel_id or "").strip() or (os.environ.get("WHATSAPP_HOME_CHANNEL") or "").strip()

    def _fmt_token(value: str) -> str:
        v = (value or "").strip()
        if not v:
            return ""
        token = _whatsapp_send_target_token(v)
        return _hash_chat_id(token) if redact_pii else token

    allowed = _split_gateway_env_list("WHATSAPP_ALLOWED_USERS")
    group_allowed = _split_gateway_env_list("WHATSAPP_GROUP_ALLOWED_USERS")
    free_chats = _split_gateway_env_list("WHATSAPP_FREE_RESPONSE_CHATS")

    chans: List[Any] = []
    try:
        from gateway.channel_directory import load_directory

        directory = load_directory() or {}
        raw_chans = directory.get("platforms", {}).get("whatsapp") or []
        if isinstance(raw_chans, list):
            chans = raw_chans
    except Exception as exc:
        logger.debug("WhatsApp context: channel directory unavailable: %s", exc)

    if not home and not allowed and not group_allowed and not free_chats and not chans:
        return []

    lines: List[str] = [
        "",
        "**WhatsApp (send_message targets):**",
        "Use `send_message` with `target` like `whatsapp:+5511999999999` (E.164) or "
        "`whatsapp:<chat JID>` (e.g. `...@lid` / `...@s.whatsapp.net`). "
        "Prefer the tokens below instead of inventing placeholders.",
        "**LID vs número:** identidades `...@lid` são IDs internos do WhatsApp. "
        "Os dígitos antes de `@lid` **não** são o telefone E.164 do contato (podem coincidir "
        "por acaso). Não diga que \"salvou o número\" só por ver esse trecho; use o E.164 real "
        "(ex. +55…) ou o JID exato do diretório abaixo. Para `WHATSAPP_ALLOWED_USERS` / pairing, "
        "use o telefone em dígitos ou o JID que o gateway mostra, não confunda LID com número.",
    ]

    if home:
        lines.append(f"- Home / default delivery: `{_fmt_token(home)}`")

    if allowed:
        lines.append("- Allowlisted inbound identities (valid DM/chat targets for this gateway):")
        for entry in allowed[:40]:
            fv = _fmt_token(entry)
            if fv:
                lines.append(f"  - `{fv}`")
        if len(allowed) > 40:
            lines.append(f"  - … ({len(allowed) - 40} more omitted)")

    if group_allowed:
        lines.append("- Group allowlist (extra group senders):")
        for entry in group_allowed[:20]:
            fv = _fmt_token(entry)
            if fv:
                lines.append(f"  - `{fv}`")

    if free_chats:
        lines.append("- Free-response chats (IDs):")
        for entry in free_chats[:20]:
            fv = _fmt_token(entry)
            if fv:
                lines.append(f"  - `{fv}`")

    if chans:
        lines.append("- Known chats from recent sessions / directory (name → target):")
        for ch in chans[:35]:
            if not isinstance(ch, dict):
                continue
            cid = str(ch.get("id") or "").strip()
            cname = str(ch.get("name") or "").strip() or cid
            if not cid:
                continue
            show_tok = _fmt_token(cid)
            lines.append(f"  - {cname} → `{show_tok}`")
        if len(chans) > 35:
            lines.append(f"  - … ({len(chans) - 35} more omitted)")

    if redact_pii:
        lines.append(
            "(Tokens above are redacted because `privacy.redact_pii` is on — "
            "use `send_message(action='list')` if you need fresh targets.)"
        )
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `ector tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    display_user_name = (
        ...
        else effective_gateway_user_display_name(context.source)
    )
    ...
    if context.source.platform == Platform.LOCAL:
        ...
        if redact_pii:
            ...
            if src.chat_type == "dm" and display_user_name:
                desc = f"DM with {display_user_name}"
            else:
    ...
    elif display_user_name:
        lines.append(f"**User:** {display_user_name}")
    ...
    if context.source.platform == Platform.SLACK:
        ...
        lines.append("")
        lines.append(
            "**Platform notes:** You are responding via WhatsApp. "
            "WhatsApp does not render Markdown reliably — prefer plain text."
        )
    elif context.source.platform == Platform.DISCORD:
        # Inject the Discord IDs block only when the agent actually has
        # Discord tools loaded this session — i.e. the user opted into
        # `discord` / `discord_admin` via `ector tools` AND the bot
    ...
    if context.source.platform != Platform.LOCAL:
        lines.append("")
        lines.append(
            "**Gateway voice / STT:** Inbound voice and audio messages are often "
            "transcribed automatically by the gateway (STT) before they reach you "
            "when STT is enabled; you then see the transcript as normal user text. "
            "You do not have a Speech-to-Text tool for inbound platform audio — do not "
            "claim you will transcribe such messages via a tool."
        )
    ...
    if Platform.WHATSAPP in context.connected_platforms:
        wa_home = ""
        hc_map = context.home_channels or {}
        if Platform.WHATSAPP in hc_map:
            wa_home = (hc_map[Platform.WHATSAPP].chat_id or "").strip()
        wa_lines = _whatsapp_send_routing_lines(
            redact_pii=redact_pii,
            home_channel_id=wa_home,
        )
        if wa_lines:
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_ector_home()}/cron/output/)"
    )
    ...
    if context.source.platform != Platform.LOCAL:
        lines.append("")
        lines.append(
            "**Tom e extensão:** siga o que o usuário pede nas mensagens desta conversa e o perfil "
            "USER na memória (onboarding). Não presuma um tom vindo de config estática da plataforma."
        )
...
class SessionEntry:
    ...
    # Gateway idle "presence nudge" (optional; see display.presence_nudge)
    last_user_message_at: Optional[datetime] = None
    last_presence_nudge_at: Optional[datetime] = None
    presence_nudge_disabled: bool = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "presence_nudge_disabled": self.presence_nudge_disabled,
        }
        if self.last_user_message_at:
            result["last_user_message_at"] = self.last_user_message_at.isoformat()
        if self.last_presence_nudge_at:
            result["last_presence_nudge_at"] = self.last_presence_nudge_at.isoformat()
        if self.origin:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionEntry":
        ...
        _luma = data.get("last_user_message_at")
        if _luma:
            try:
                last_user_message_at = datetime.fromisoformat(_luma)
            except (TypeError, ValueError):
                last_user_message_at = None

        last_presence_nudge_at = None
        _lpna = data.get("last_presence_nudge_at")
        if _lpna:
            try:
                last_presence_nudge_at = datetime.fromisoformat(_lpna)
            except (TypeError, ValueError):
                last_presence_nudge_at = None
        ...
        return cls(
            ...
            last_presence_nudge_at=last_presence_nudge_at,
            presence_nudge_disabled=bool(data.get("presence_nudge_disabled", False)),
        )
...
def effective_gateway_user_display_name(
    source: SessionSource,
    *,
    group_sessions_per_user: bool = True,
    thread_sessions_per_user: bool = False,
) -> Optional[str]:
    """User-facing name for session context and agent scoping.

    Uses ``config.user.nickname`` when set (ector.cc profile), except in
    shared multi-user sessions where per-message sender names must stay
    platform-accurate.
    """
    ...
    try:
        from ector_cli.identity_auth import resolve_preferred_user_display_name

        resolved = resolve_preferred_user_display_name(
            source.user_name,
            use_profile=not shared,
        )
    except Exception:
        resolved = (source.user_name or "").strip() or None
    if resolved:
        return resolved
    trimmed = (source.user_name or "").strip()
...
class SessionStore:
    ...
    def touch_user_presence_activity(self, session_key: str) -> None:
        """Record a user-originated inbound message time (for idle presence nudge)."""
        with self._lock:
            self._ensure_loaded_locked()
            entry = self._entries.get(session_key)
            if entry is None:
                return
            entry.last_user_message_at = _now()
            self._save()

    def set_presence_nudge_disabled(self, session_key: str, disabled: bool) -> None:
        with self._lock:
            self._ensure_loaded_locked()
            entry = self._entries.get(session_key)
            if entry is None:
                return
            entry.presence_nudge_disabled = bool(disabled)
            self._save()

    def note_presence_nudge_sent(self, session_key: str) -> None:
        with self._lock:
            self._ensure_loaded_locked()
            entry = self._entries.get(session_key)
            if entry is None:
                return
            entry.last_presence_nudge_at = _now()
            self._save()
    ...
    def mark_recently_active_resume_pending(
        ...
        reason: str = "restart_crash",
    ) -> int:
        """Mark recently-active sessions as resume_pending (crash recovery).

        This is a *non-destructive* alternative to ``suspend_recently_active``:
        it preserves the existing ``session_id`` + transcript so the next
        inbound message auto-resumes the prior conversation lane.

        We intentionally skip entries already flagged as ``suspended`` (hard
        reset requested) and entries already marked ``resume_pending``.
        """
        cutoff = _now() - timedelta(seconds=max_age_seconds)
        count = 0
        with self._lock:
    ...
    def reset_session(self, session_key: str) -> Optional[SessionEntry]:
        ...
        with self._lock:
            ...
            new_entry = SessionEntry(
                ...
                presence_nudge_disabled=old_entry.presence_nudge_disabled,
            )
