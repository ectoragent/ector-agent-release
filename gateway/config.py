...
SUPPORTED_CHANNEL_PLATFORMS = frozenset(
    {
        Platform.TELEGRAM,
        Platform.DISCORD,
        Platform.WHATSAPP,
...
class StreamingConfig:
    ...
    # When >0, the final edit for a long-running streamed response is delivered
    # as a fresh message if the original preview has been visible for at least
    # this many seconds, so the platform's visible timestamp reflects completion
    # time instead of the preview creation time.  Currently applied to Telegram
    # only (other platforms ignore the setting).  Default 60s.  Set to 0 to disable.
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_ector_home() / "sessions")
    ...
    def get_connected_platforms(self) -> List[Platform]:
        ...
        for platform, config in self.platforms.items():
            if platform not in SUPPORTED_CHANNEL_PLATFORMS:
                continue
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.ector/config.yaml (primary user-facing config)
    3. ~/.ector/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_ector_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # /sethome (gateway) persists ``<PLATFORM>_HOME_CHANNEL`` at the root of
            # config.yaml. Bridge into os.environ when not already set so
            # ``_apply_env_overrides`` and tools see the same values as from .env.
            for _root_key, _root_val in list(yaml_cfg.items()):
                if not isinstance(_root_key, str) or not _root_key.endswith("_HOME_CHANNEL"):
                    continue
                if _root_key in os.environ and str(os.environ.get(_root_key, "")).strip():
                    continue
                if _root_val is None or not str(_root_val).strip():
                    continue
                os.environ[_root_key] = str(_root_val).strip()
            for _root_key, _root_val in list(yaml_cfg.items()):
                if not isinstance(_root_key, str) or not _root_key.endswith("_HOME_CHANNEL_NAME"):
                    continue
                if _root_key in os.environ and str(os.environ.get(_root_key, "")).strip():
                    continue
                if _root_val is None or not str(_root_val).strip():
