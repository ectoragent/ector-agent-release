...
Cache location: ~/.ector/sticker_cache.json
"""
...
CACHE_PATH = get_ector_home() / "sticker_cache.json"
...
def build_sticker_injection(
    ...
    return f"[O usuário enviou um sticker{context}~ Ele mostra: \"{description}\" (=^.w.^=)]"
...
def build_animated_sticker_injection(emoji: str = "") -> str:
    ...
    if emoji:
        return (
            f"[O usuário enviou um sticker animado {emoji}~ "
            f"Eu ainda não consigo ver stickers animados, mas o emoji sugere: {emoji}]"
        )
    return "[O usuário enviou um sticker animado~ Eu ainda não consigo ver stickers animados]"
