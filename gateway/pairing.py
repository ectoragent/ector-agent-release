...
PAIRING_DIR = get_ector_dir("platforms/pairing", "pairing")
...
class PairingStore:
    ...
    def is_approved_whatsapp_alias(self, platform: str, user_id: str) -> bool:
        """WhatsApp: pairing may store ``...@s.whatsapp.net`` while the bridge
        later reports ``...@lid`` for the same person.  Match via
        ``lid-mapping-*.json`` the same way as the gateway allowlist.
        """
        if self.is_approved(platform, user_id):
            return True
        try:
            from gateway.whatsapp_identity import expand_whatsapp_aliases as _expand
        except Exception:
            return False
        incoming = _expand(user_id)
        if not incoming:
            return False
        approved = self._load_json(self._approved_path(platform))
        for key in approved:
