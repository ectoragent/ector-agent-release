...
from ector_constants import get_ector_dir


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "A entrada segura de segredos não é suportada por mensagens. "
    "Execute esta habilidade no CLI local para ser solicitado, ou adicione a chave manualmente em ~/.ector/.env."
)
...
# Default location: {ECTOR_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_ector_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; EctorAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_ector_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_ector_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_ector_dir("cache/documents", "document_cache")
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Recent outbound snippets keyed by "{chat_id}:{message_id}" so quoted
        # replies can be resolved when the platform omits quoted body text.
        self._outbound_snippet_cache: "OrderedDict[str, str]" = OrderedDict()
        self._OUTBOUND_SNIPPET_CACHE_MAX = 250
    ...
    def remember_outbound_snippet(
        self,
        chat_id: str,
        message_id: Optional[str],
        content: str,
    ) -> None:
        """Cache a recently sent message so quoted replies can be resolved later."""
        ...
        self._outbound_snippet_cache[key] = text[:500]
        self._outbound_snippet_cache.move_to_end(key)
        while len(self._outbound_snippet_cache) > self._OUTBOUND_SNIPPET_CACHE_MAX:
            self._outbound_snippet_cache.popitem(last=False)

    def lookup_outbound_snippet(
        self,
        chat_id: str,
        message_id: Optional[str],
    ) -> Optional[str]:
        """Return cached outbound text for a message id in this chat, if known."""
        if not chat_id or not message_id:
            return None
        return self._outbound_snippet_cache.get(f"{chat_id}:{message_id}")

    def remember_outbound_chunks(
        self,
        chat_id: str,
        chunks: List[Tuple[Optional[str], str]],
    ) -> None:
        """Cache each sent chunk so quoted replies can target any fragment."""
        for message_id, content in chunks:
            self.remember_outbound_snippet(chat_id, message_id, content)
    ...
    async def delete_message(
        ...
        fresh_final_after_seconds) to remove long-lived preview messages
    ...
    async def _send_with_retry(
        ...
        if result.success:
            self.remember_outbound_snippet(chat_id, result.message_id, content)
            return result
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from ector_cli.commands import should_bypass_active_session

            # Aprovação pendente (comando perigoso): respostas "Sim/Não" e afins
            # precisam bypassar o guard também. Caso contrário, seriam apenas
            # enfileiradas e disparariam interrupt, enquanto o agente está
            # bloqueado em tools/approval.py esperando o gateway sinalizar.
            if not cmd:
                try:
                    from tools.approval import has_blocking_approval as _has_blocking_approval

                    raw = (event.text or "").strip()
                    if raw and len(raw) <= 80 and _has_blocking_approval(session_key):
                        logger.debug(
                            "[%s] Resposta de aprovação pendente bypassando guard para %s",
                        ...
                        try:
                            ...
                            logger.error(
                                "[%s] Dispatch de resposta de aprovação falhou: %s",
                                self.name, e, exc_info=True,
                            )
                        return
                except Exception:
                    # Fail-closed: fall back to normal busy behavior
                    pass

            if not cmd:
                try:
                    from tools.wiser_gateway import has_blocking_wiser as _has_blocking_wiser

                    raw = (event.text or "").strip()
                    if raw and len(raw) <= 500 and _has_blocking_wiser(session_key):
                        logger.debug(
                            "[%s] Wiser reply bypassing guard for %s",
                            self.name, session_key,
                        )
                        try:
                            _thread_meta = {"thread_id": event.source.thread_id} if event.source.thread_id else None
                            ...
                            logger.error(
                                "[%s] Wiser reply dispatch failed: %s",
                                self.name, e, exc_info=True,
                            )
                        return
                except Exception:
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars (ECTOR_*):
          ECTOR_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ECTOR_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ECTOR_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = (os.getenv("ECTOR_HUMAN_DELAY_MODE") or "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("ECTOR_HUMAN_DELAY_MIN_MS") or "800")
        max_ms = int(os.getenv("ECTOR_HUMAN_DELAY_MAX_MS") or "2500")
        if mode == "natural":
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        callback_generation = getattr(interrupt_event, "_ector_run_generation", None)
