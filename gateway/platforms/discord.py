...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("ECTOR_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("ECTOR_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Ector manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Ector session")
        ...
        async def slash_model(interaction: discord.Interaction, name: str = ""):
            await self._run_simple_slash(interaction, f"/provider {name}".strip())
        ...
        @tree.command(name="status", description="Show Ector session status")
        ...
        @tree.command(name="stop", description="Stop the running Ector agent")
        ...
        @tree.command(name="stats", description="Show usage statistics and analytics")
        @discord.app_commands.describe(days="Number of days to analyze (default: 7)")
        async def slash_stats(interaction: discord.Interaction, days: int = 7):
            await self._run_simple_slash(interaction, f"/stats {days}")
        ...
        @tree.command(name="restart", description="Gracefully restart the Ector gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Ector session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Ector in the thread",
        ...
        # ector_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from ector_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from ector_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        """
        try:
            from ector_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Ector session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Ector: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Ector"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Ector: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
        ...
        if message.reference:
            ...
            if message.reference.resolved:
                ...
                try:
                    ...
                    reply_to_text = (ref_msg.content or "").strip() or None
                except Exception as exc:
                    logger.debug(
                        "[%s] Could not fetch referenced message %s: %s",
                        self.name,
                        reply_to_id,
                        exc,
                    )
...
if DISCORD_AVAILABLE:
    ...
    class ModelPickerView(discord.ui.View):
        ...
        async def _on_provider_selected(self, interaction: discord.Interaction):
            ...
            extra = f"\n*{total - shown} more available — type `/provider <name>` directly*" if total > shown else ""
