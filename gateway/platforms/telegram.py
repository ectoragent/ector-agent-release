...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("ECTOR_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = float(os.getenv("ECTOR_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("ECTOR_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
        ...
        # Wiser button state: wiser_id → {session_key, choices, request_id}
        self._wiser_state: Dict[int, dict] = {}
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "(possibly another Ector instance or a conflicting bot). "
            "Ector stopped Telegram polling after %d retries. "
            "Only one poller can run per token — stop the other process "
            "and restart with 'ector start'."
    ...
    def _persist_dm_topic_thread_id(self, chat_id: int, topic_name: str, thread_id: int) -> None:
        ...
        try:
            ...
            config_path = get_ector_home() / "config.yaml"
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("ECTOR_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("ECTOR_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("ECTOR_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("ECTOR_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("ECTOR_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("ECTOR_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in ("1", "true", "yes", "on"))
            ...
            try:
                ...
                from ector_cli.commands import telegram_menu_commands
    ...
    async def send(
        ...
        try:
            ...
            self.remember_outbound_chunks(
                chat_id,
                [
                    (message_ids[i], chunks[i])
                    for i in range(min(len(message_ids), len(chunks)))
                ],
            )
    ...
    async def delete_message(self, chat_id: str, message_id: str) -> bool:
        ...
        via fresh_final_after_seconds) to remove long-lived preview
    ...
    async def send_wiser_prompt(
        ...
        """Send a Wiser question with inline choice buttons (when choices are set)."""
        if not self._bot:
            return SendResult(success=False, error="Not connected")
        ...
        try:
            ...
            if not hasattr(self, "_wiser_counter"):
                self._wiser_counter = itertools.count(1)
            wiser_id = next(self._wiser_counter)
            self._wiser_state[wiser_id] = {
                "session_key": session_key,
                "choices": list(choices or []),
                "request_id": request_id,
            }

            text = f"✦ <b>Wiser</b>\n\n{_html.escape((question or '')[:3500])}"
            keyboard = None
            if choices:
                rows: List[List[InlineKeyboardButton]] = []
                row: List[InlineKeyboardButton] = []
                for i, label in enumerate(choices):
                    short = _html.escape(str(label)[:36])
                    row.append(
                        InlineKeyboardButton(
                            f"{i + 1}. {short}",
                            callback_data=f"ws:{i}:{wiser_id}",
                        )
                    )
                    if len(row) >= 2:
                        rows.append(row)
                ...
                if row:
                    rows.append(row)
                rows.append([
                    InlineKeyboardButton("Outro (texto livre)", callback_data=f"ws:other:{wiser_id}")
                ])
            ...
            logger.warning("[%s] send_wiser_prompt failed: %s", self.name, e)
            return SendResult(success=False, error=str(e))
    ...
    async def _handle_model_picker_callback(
        ...
        if not state:
            await query.answer(text="Picker expired — use /provider again.")
            return
        ...
        if data.startswith("mp:"):
            ...
            shown = len(models)
            extra = f"\n_{total - shown} more available — type `/provider <name>` directly_" if total > shown else ""
    ...
    async def _handle_callback_query(
        ...
        # --- Wiser callbacks (ws:choice_index:wiser_id) ---
        if data.startswith("ws:"):
            parts = data.split(":", 2)
            if len(parts) == 3:
                choice_key = parts[1]
                try:
                    wiser_id = int(parts[2])
                except (ValueError, IndexError):
                    await query.answer(text="Invalid Wiser data.")
                    return
                ...
                state = self._wiser_state.pop(wiser_id, None)
                if not state:
                    await query.answer(text="This question was already answered.")
                    return
                ...
                if choice_key == "other":
                    await query.answer(text="Digite sua resposta neste chat.")
                    try:
                ...
                await query.answer(text=f"✓ {answer[:60]}")
                try:
                    await query.edit_message_text(
                        text=f"✓ {user_display}: {_html.escape(answer[:200])}",
                        parse_mode=ParseMode.HTML,
                        reply_markup=None,
                    )
                except Exception:
                ...
                try:
                    ...
                    resolve_gateway_wiser(session_key, answer)
                except Exception as exc:
                    logger.error("Failed to resolve Wiser from Telegram button: %s", exc)
            return
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@ector_bot.example" are not mentions
