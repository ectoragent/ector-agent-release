...
class SlackAdapter(BasePlatformAdapter):
    ...
    def _describe_slack_api_error(self, response: Any, *, file_obj: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        reinstall_hint = " Atualize os escopos/configurações do app Slack e reinstale o app no workspace."
        provided_hint = f" Escopos atuais do bot: {provided}." if provided else ""

        if error == "missing_scope":
            needed_hint = f"Escopo ausente: {needed}." if needed else "Escopo Slack obrigatório ausente."
            return f"Falha ao acessar anexo do Slack para {file_label}. {needed_hint}{provided_hint}{reinstall_hint}"
        if error in {"not_authed", "invalid_auth", "account_inactive", "token_revoked"}:
            return f"Falha ao acessar anexo do Slack para {file_label} porque o token do bot não está autorizado ({error}). Atualize o token/reinstale o app."
        if error in {"file_not_found", "file_deleted"}:
            return f"O anexo {file_label} do Slack não está mais disponível ({error})."
        if error in {"access_denied", "file_access_denied", "no_permission", "not_allowed_token_type", "restricted_action"}:
            return f"Falha ao acessar anexo do Slack para {file_label} porque o bot não tem permissão ({error}). Verifique as permissões/escopos do workspace e reinstale se necessário."
        return None
    ...
    def _describe_slack_download_failure(self, exc: Exception, *, file_obj: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        if httpx is not None and isinstance(exc, httpx.HTTPStatusError):
            ...
            if status == 401:
                return f"Falha ao acessar anexo do Slack para {file_label} com HTTP 401. O token do bot não está autorizado para este arquivo."
            if status == 403:
                return f"Falha ao acessar anexo do Slack para {file_label} com HTTP 403. O bot provavelmente não tem permissão ou escopo para ler este arquivo."
            if status == 404:
                return f"O anexo {file_label} do Slack retornou HTTP 404 e não está mais acessível."

        message = str(exc)
        if "Slack returned HTML instead of media" in message or "non-image data" in message:
            return (
                f"Falha ao acessar anexo do Slack para {file_label}: o Slack retornou uma resposta HTML/login ou não-mídia. "
                "Isso geralmente indica um problema de escopo, autenticação ou permissão de arquivo."
            )
        return None
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_ector_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /provider work directly without /ector prefix). A single regex
            ...
            # manifest (see `ector slack manifest`). In Socket Mode, Slack
            ...
            from ector_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/ector$")

            @self._app.command(_slash_pattern)
            async def handle_ector_command(ack, command):
                await ack()
            ...
            for _action_id in (
                "ector_approve_once",
                "ector_approve_session",
                "ector_approve_always",
                "ector_deny",
            ):
    ...
    async def send(
        ...
        try:
            ...
            sent_ts = chunk_pairs[-1][0] if chunk_pairs else None
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Ector session — matching the per-thread behavior channels already
    ...
    async def on_processing_start(self, event: MessageEvent) -> None:
        ...
        if channel_id:
            # Slack API name "eyes" (◎ in-progress; Slack renders the standard eyes emoji)
    ...
    async def on_processing_complete(self, event: MessageEvent, outcome: ProcessingOutcome) -> None:
        ...
        if not channel_id:
            return
        # Slack API name "eyes" (◎ in-progress)
    ...
    async def send_image_file(
        ...
        try:
            ...
            return SendResult(success=False, error=f"Arquivo de imagem não encontrado: {image_path}")
        except Exception as e:  # pragma: no cover - defensive logging
            logger.error(
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        try:
            ...
            return SendResult(success=False, error=f"Arquivo de áudio não encontrado: {audio_path}")
        except Exception as e:  # pragma: no cover - defensive logging
            logger.error(
    ...
    async def send_video(
        ...
        if not os.path.exists(video_path):
            return SendResult(success=False, error=f"Arquivo de vídeo não encontrado: {video_path}")
    ...
    async def send_document(
        ...
        if not os.path.exists(file_path):
            return SendResult(success=False, error=f"Arquivo não encontrado: {file_path}")
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if thread_ts and thread_ts != ts:
            try:
                reply_to_id, reply_to_text = await self._resolve_thread_reply_target(
                    channel_id=channel_id,
                    thread_ts=thread_ts,
                    current_ts=ts,
                    team_id=team_id,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            blocks = [
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": (
                            f":warning: *Aprovação de Comando Obrigatória*\n"
                            f"```{cmd_preview}```\n"
                ...
                },
                {
                    ...
                    "elements": [
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Permitir uma vez"},
                            "style": "primary",
                            "action_id": "ector_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Permitir na sessão"},
                            "action_id": "ector_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Sempre permitir"},
                            "action_id": "ector_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Negar"},
                            "style": "danger",
                            "action_id": "ector_deny",
                            "value": session_key,
                        },
                    ],
                },
            ...
            kwargs: Dict[str, Any] = {
                "channel": chat_id,
                "text": f"▲ Aprovação de comando obrigatória: {cmd_preview[:100]}",
                "blocks": blocks,
            }
            if thread_ts:
                kwargs["thread_ts"] = thread_ts
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "ector_approve_once": "once",
            "ector_approve_session": "session",
            "ector_approve_always": "always",
            "ector_deny": "deny",
        }
        ...
        label_map = {
            "once": f"✔ Aprovado uma vez por {user_name}",
            "session": f"✔ Aprovado para esta sessão por {user_name}",
            "always": f"✔ Aprovado permanentemente por {user_name}",
            "deny": f"✖ Negado por {user_name}",
        }
        decision_text = label_map.get(choice, f"Resolvido por {user_name}")
        ...
        updated_blocks = [
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": original_text or "Solicitação de aprovação de comando",
                },
            },
            {
                "type": "context",
    ...
    async def _fetch_thread_context(
        ...
        try:
            ...
            for msg in messages:
                ...
                prefix = "[pai da thread] " if is_parent else ""
            ...
            if context_parts:
                content = (
                    "[Contexto da thread — mensagens anteriores nesta thread (ainda não no histórico da conversa):]\n"
                    + "\n".join(context_parts)
                    + "\n[Fim do contexto da thread]\n\n"
                )
    ...
    async def _resolve_thread_reply_target(
        ...
        *,
        ...
        """Best-effort resolve which thread message the user is replying to."""
        bot_uid = self._team_bot_user_ids.get(team_id, self._bot_user_id)
        ...
        try:
            ...
            for idx, msg in enumerate(messages):
                if msg.get("ts") != current_ts or idx == 0:
                    continue
                prev = messages[idx - 1]
                predecessor_ts = prev.get("ts")
                if not predecessor_ts:
                    break
                cached = self.lookup_outbound_snippet(channel_id, predecessor_ts)
                if cached:
                    return predecessor_ts, cached
                predecessor_text = _clean_slack_text(prev.get("text") or "")
                if predecessor_text:
                    return predecessor_ts, predecessor_text
                break
        except Exception as exc:  # pragma: no cover - defensive
            logger.debug("[Slack] Failed to resolve thread reply target: %s", exc)
        ...
        cleaned_parent = _clean_slack_text(parent_text)
        if cleaned_parent:
            return thread_ts, cleaned_parent
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in ("ector", ""):
            # Legacy /ector <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from ector_cli.commands import slack_subcommand_map
