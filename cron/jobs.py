...
Jobs are stored in ~/.ector/cron/jobs.json
Output is saved to ~/.ector/cron/output/{job_id}/{timestamp}.md
"""
...
ECTOR_DIR = get_ector_home().resolve()
CRON_DIR = ECTOR_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"

# In-process lock protecting load_jobs→modify→save_jobs cycles.
# Legacy threading lock kept for back-compat; now primarily uses file-based lock.
...
@contextmanager
def lock_jobs():
    """Context manager for cross-process locking of the jobs database.
    
    Ensures that load_jobs -> modify -> save_jobs cycles are atomic even
    when multiple processes (CLI, Gateway, Cron agents) access the file
    simultaneously.
    """
    ensure_dirs()
    with open(_JOBS_LOCK_FILE, "w") as f:
        if fcntl:
            fcntl.flock(f, fcntl.LOCK_EX)
        elif msvcrt:
            # Simple Windows locking
            while True:
                try:
...
def parse_duration(s: str) -> int:
    ...
    if not match:
        raise ValueError(f"Invalid duration: '{s}'. Use format like '30s', '30m', '2h', or '1d'")
    
    value = int(match.group(1))
    unit_char = match.group(2)[0]  # First char: s, m, h, or d
    
    multipliers = {'s': 1, 'm': 60, 'h': 3600, 'd': 86400}
    return value * multipliers[unit_char]
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    # Duration like "30s", "30m", "2h", "1d" → one-shot from now
    try:
        seconds = parse_duration(schedule)
        run_at = _ector_now() + timedelta(seconds=seconds)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Ector configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Ector timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _ector_now().tzinfo
    if dt.tzinfo is None:
...
def _compute_grace_seconds(schedule: dict) -> int:
    ...
    if kind == "interval":
        period_seconds = schedule.get("seconds")
...
def compute_next_run(schedule: Dict[str, Any], last_run_at: Optional[str] = None) -> Optional[str]:
    ...
    if schedule["kind"] == "once":
        ...
        if seconds is None:
            seconds = schedule.get("minutes", 0) * 60
        
        if last_run_at:
            # Next run is last_run + interval
            last = _ensure_aware(datetime.fromisoformat(last_run_at))
            next_run = last + timedelta(seconds=seconds)
        else:
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "'ector-agent[cron]') to re-enable recurring cron jobs.",
                schedule.get("expr"),
            )
            return None
        cron = croniter(schedule["expr"], now)
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _ector_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _ector_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _ector_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _ector_now().strftime("%Y-%m-%d_%H-%M-%S")
