"""ACP session manager — maps ACP sessions to Ector AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.ector/state.db``) so they
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["ector-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Ector agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Ector AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Ector runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.ector/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ECTOR_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_ector_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=ector_home / "state.db")
            return self._db_instance
        except Exception:
