"""CLI entry point for the Ector Agent ACP adapter.

Loads environment variables from ``~/.ector/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ECTOR_HOME (default ``~/.ector``)."""
    from ector_cli.env_loader import load_ector_dotenv

    ector_home = get_ector_home()
    loaded = load_ector_dotenv(ector_home=ector_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", ector_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting Ector Agent ACP adapter")
    ...
    import acp
    from .server import EctorACPAgent
