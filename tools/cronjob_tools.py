"""
Cron job management tools for Ector Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("ECTOR_SESSION_PLATFORM")
    origin_chat_id = get_session_env("ECTOR_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("ECTOR_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("ECTOR_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via ector provider.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within ECTOR_HOME/scripts/.
    ...
    # Only relative paths within ~/.ector/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.ector/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.ector/scripts/ and use just the filename."
        )
...
def cronjob(
    ...
    try:
        ...
        if normalized == "create":
            ...
            # The literal "self" is reserved: it resolves at runtime to the
            # job's own ID so monitoring jobs can see their previous output
            # and avoid duplicate delivery (e.g. inbox watchers).
            if context_from:
        ...
        if normalized == "update":
            ...
            if context_from is not None:
                ...
                # The literal "self" is reserved and resolves at runtime to
                # the job's own ID — skip the existence check for it.
...
- For natural-language reminder requests (e.g. "remind me", "me lembre"), default to action='create'.
- Never claim that a reminder/job already exists unless you first call action='list' and verify a matching ACTIVE job in the returned data.
- If you did not call action='list' in this turn, do not assert existing reminders.
- If uncertain, create the reminder and confirm success from the tool response.

SCHEDULING DISCIPLINE (avoid surprise extra jobs):
- Match the user's timing exactly. Examples: "in 20 seconds" / "em 20 segundos" → schedule='20s' (one-shot). "in 5 minutes" → '5m'.
- Use one-shot duration forms ('20s', '5m', '2h') for a single delayed ping. Do NOT switch to 'every 30m' or recurring schedules unless the user explicitly asked for repetition (e.g. "every 30 minutes", "a cada 30 minutos", "lembrar sempre").
- Do not create a second "habit" or hydration job on your own when the user asked for one reminder only.
- Use at most one cronjob(create) for a simple reminder request unless the user clearly asked for multiple distinct schedules.

MONITORING JOBS (avoid duplicate-delivery flooding):
- For recurring "watch / monitor / check" requests (inbox, RSS, dashboards, prices,
  status pages, etc.), ALWAYS pass context_from=["self"] on create. This injects
  the previous run's output into the next run as a delta baseline so the agent
  only reports what changed and emits "[SILENT]" when nothing is new.
- Examples that REQUIRE context_from=["self"]: "monitorar meu e-mail", "me avise
  quando chegarem novos e-mails", "watch this RSS feed", "ping me on price drops".
- Pair it with an explicit instruction in the prompt, e.g. "Compare with the
  previous run output. If no genuinely new items, respond with exactly [SILENT]."
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "schedule": {
                "type": "string",
                "description": (
                    "For create/update: one-shot delay '20s','5m','2h','1d'; recurring 'every 30m','every 2h'; "
                    "cron '0 9 * * *'; or ISO timestamp. Prefer the smallest matching form the user asked for "
                    "(e.g. em 20s → '20s') — do not add 'every …' unless they asked for repeats."
                )
            },
            "name": {
                "type": "string",
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under {display_ector_home()}/scripts/. On update, pass empty string to clear."
            },
            "context_from": {
                "type": "array",
                "items": {"type": "string"},
                "description": (
                    "Optional job IDs whose most recent completed output is injected into "
                    "the prompt as context before each run. Two use cases: "
                    "(1) Chaining: job A collects data, job B processes it — each entry "
                    "must be a valid job ID from cronjob(action='list'). "
                    "(2) Self-context for monitoring jobs: pass the literal value [\"self\"] "
                    "and the previous run's output of THIS SAME job is injected as the "
                    "delta baseline. REQUIRED for any recurring 'monitor/watch/check' "
                    "request (inbox, RSS, dashboards, prices) so duplicates are not "
                    "re-delivered. Combine with [SILENT] guidance in the prompt. "
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("ECTOR_INTERACTIVE")
        or os.getenv("ECTOR_GATEWAY_SESSION")
        or os.getenv("ECTOR_EXEC_ASK")
    )
