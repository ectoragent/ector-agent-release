...
~/.ector/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `ector config edit` (skills.guard_agent_created: true).
    """
    try:
...
# All skills live in ~/.ector/skills/ (single source of truth)
...
# -----------------------------------------------------------------------------
# User-facing messages (best-effort via IA)
# -----------------------------------------------------------------------------

def _generate_skill_status_message(*, action: str, name: str) -> Optional[str]:
    """Best-effort: gerar uma mensagem curta e natural em pt-BR via LLM.

    Retorna None se não houver credenciais/modelo ou em qualquer erro.
    """
    action_norm = (action or "").strip().lower()
    if action_norm not in ("create", "edit"):
        return None
    ...
    try:
        ...
        action_pt = "criei" if action_norm == "create" else "atualizei"
        system = (
            "Você escreve mensagens curtas e amigáveis em pt-BR para o usuário. "
            "Gere 1 frase curta (máx ~140 caracteres), natural e profissional. "
            "Não use Markdown, não use aspas, não use emojis. "
            "Não invente detalhes técnicos. "
            "Inclua o nome da habilidade e que você está aprendendo mais."
        )
        user = (
            f"Ação: {action_pt}\n"
            f"Nome da habilidade: {name}\n"
            "Mensagem:"
        )
        ...
        logger.debug("Falha ao gerar mensagem de skill via IA (fallback): %s", e)
        return None
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.ector/skills/) first, then any
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    message = _generate_skill_status_message(action="create", name=name) or (
        f"✨ Eu desenvolvi uma habilidade nova de '{name}'! Agora eu já sei como realizar essa tarefa no futuro."
    )
    result = {
        "success": True,
        "message": message,
...
def _edit_skill(name: str, content: str) -> Dict[str, Any]:
    ...
    message = _generate_skill_status_message(action="edit", name=name) or (
        f"✨ Eu melhorei a minha habilidade de '{name}'! As atualizações foram aplicadas e estou pronto para usá-las."
    )
    return {
        "success": True,
        "message": message,
...
def _patch_skill(
    ...
    return {
        "success": True,
        "message": f"✨ Eu fiz alguns ajustes em '{'SKILL.md' if not file_path else file_path}' na minha habilidade de '{name}'.",
    }
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_ector_home()}/skills/; existing skills can be modified wherever they live.\n\n"
