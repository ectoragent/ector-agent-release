...
selectable via ``ector tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Ector Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Ector
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    """Submit a FAL request using direct FAL_KEY credentials."""
    if prefers_gateway("image_gen"):
        raise ValueError(
            "image_gen.use_gateway is set but the managed tool gateway was removed; "
            "set FAL_KEY to use FAL.ai directly."
        )
...
def check_fal_api_key() -> bool:
    """True if the FAL.ai API key is configured."""
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping ector with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `ector plugins list` to see "
