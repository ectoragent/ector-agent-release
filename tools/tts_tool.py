...
Configuration is loaded from ~/.ector/config.yaml under the 'tts:' key.
...
from tools.tool_backend_helpers import prefers_gateway, resolve_openai_audio_api_key
...
def _get_default_output_dir() -> str:
    ...
    return str(get_ector_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.ector/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.ector/config.yaml.
    ...
    try:
        ...
        logger.debug("ector_cli.config not available, using default TTS config")
        return {}
...
async def _generate_edge_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    logger.info("Edge TTS synthesizing with voice=%s", voice)
...
def _generate_openai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    try:
        ...
        # gpt-4o-mini-tts only; tts-1 / tts-1-hd reject ``instructions``.
        if instructions and isinstance(model, str) and "gpt-4o-mini-tts" in model:
            create_kwargs["instructions"] = instructions
        elif instructions:
            logger.warning(
                "OpenAI TTS: ignoring tts.openai.instructions for model %r (supported on gpt-4o-mini-tts)",
                model,
            )
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Ector actually needs a non-default format/override.
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.ector/config.yaml (tts: section).
    ...
    platform = get_session_env("ECTOR_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'ector-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run ector setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'ector setup tts' and choose KittenTTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    """Return direct OpenAI audio API key and base URL."""
    if prefers_gateway("tts"):
        raise ValueError(
            "tts.use_gateway is set but the managed tool gateway was removed; "
            "set VOICE_TOOLS_OPENAI_KEY or OPENAI_API_KEY instead."
        )
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_ector_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
