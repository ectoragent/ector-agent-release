...
Ector, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ECTOR_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Ector, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        exit(1)
    else:
...
VISION_ANALYZE_SCHEMA = {
    "name": "vision_analyze",
    "description": (
        "Analyze an image and answer a specific question. "
        "Default: concise, direct answer. Use a detailed question only when "
        "the user explicitly wants an exhaustive description."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "question": {
                "type": "string",
                "description": (
                    "What to determine about the image. Short questions get concise "
                    "answers; ask for detail explicitly when a full description is needed."
                )
            }
        },
...
def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> Awaitable[str]:
    from agent.vision_prompts import build_vision_analyze_prompt

    image_url = args.get("image_url", "")
    question = args.get("question", "")
    full_prompt = build_vision_analyze_prompt(question)
