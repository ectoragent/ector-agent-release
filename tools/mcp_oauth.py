...
This module provides the glue:
    - ``EctorTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Ector Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ECTOR_HOME so each profile gets its own OAuth tokens.
    Layout: ``ECTOR_HOME/mcp-tokens/``
    """
    try:
        from ector_constants import get_ector_home
        base = Path(get_ector_home())
    except ImportError:
        base = Path(os.environ.get("ECTOR_HOME", str(Path.home() / ".ector")))
...
# ---------------------------------------------------------------------------
# EctorTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class EctorTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ECTOR_HOME/mcp-tokens/<server_name>.json         -- tokens
        ECTOR_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Ector records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Ector.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Ector Agent")
    scope = cfg.get("scope")
