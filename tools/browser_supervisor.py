...
One ``CDPSupervisor`` runs per Ector ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "ector-dialog-bridge.invalid"
...
(() => {
  if (window.__vectorDialogBridgeInstalled) return;
  window.__vectorDialogBridgeInstalled = true;
  const ENDPOINT = "http://ector-dialog-bridge.invalid/";
