...
Architecture:
    ~/.ector/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        ECTOR_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_ector_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("ECTOR_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal Ector infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "ector@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Ector Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "ECTOR_WORKDIR").write_text(
...
#   * orphan:  the ``ECTOR_WORKDIR`` path no longer exists on disk
...
def _read_workdir_marker(shadow_repo: Path) -> Optional[str]:
    """Read ``ECTOR_WORKDIR`` from a shadow repo, or None if missing/unreadable."""
    try:
        return (shadow_repo / "ECTOR_WORKDIR").read_text(encoding="utf-8").strip()
...
def prune_checkpoints(
    ...
    * ``delete_orphans=True`` and its ``ECTOR_WORKDIR`` path no longer
...
def maybe_auto_prune_checkpoints(
    ...
    bounded regardless of how many times ector is invoked per day.
