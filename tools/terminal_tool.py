...
Supports local execution, containerized backends, and Modal cloud sandboxes (direct credentials only).

Environment Selection (via TERMINAL_ENV environment variable):
- "local": Execute directly on the host machine (default, fastest)
- "docker": Execute in Docker containers (isolated, requires Docker)
- "modal": Execute in Modal cloud sandboxes (``auto``/``direct``; ``managed`` mode is no longer supported)
...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Ector exit
...
# display_ector_home imported lazily at call site (stale-module safety during ector update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of ector directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "ector-*")):
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ECTOR_INTERACTIVE=1).
    ...
    try:
        ...
        print()
        print("┌" + "─" * 58 + "┐")
        print("│  🔐 SENHA SUDO REQUERIDA" + " " * 31 + "│")
        print("├" + "─" * 58 + "┤")
        print("│  Digite a senha abaixo (o campo está oculto), ou:       │")
        print("│    • Pressione Enter para pular (falha graciosa)        │")
        print(f"│    • Aguarde {timeout_seconds}s para pular automaticamente" + " " * 17 + "│")
        print("└" + "─" * 58 + "┘")
        print()
        print("  Senha (oculta): ", end="", flush=True)
        ...
        if result["done"]:
            ...
            if password:
                print("  ✓ Senha recebida (armazenada para esta sessão)")
            else:
                print("  ⏭ Pulado — continuando sem sudo")
            print()
            sys.stdout.flush()
            return password
        else:
            print("\n  ⏱ Tempo esgotado — continuando sem sudo")
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    if not has_configured_password and not sudo_password and os.getenv("ECTOR_INTERACTIVE"):
...
The session working directory is already set (see the system prompt). **Do not** prefix commands with ``cd /made-up/path &&`` — run ``git status``, ``git diff``, etc. directly unless you intentionally need another directory that exists. Use ``workdir`` only for a deliberate cwd change.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, EctorSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.ector/.env or environment variables."
        )
    ...
    if env_type == "local":
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    "Modal backend is configured for managed mode, which is no longer supported. "
                    "Set TERMINAL_MODAL_MODE=direct or auto and configure Modal credentials "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so Ector can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        approval_note = None
        session_cwd = getattr(env, "cwd", None) or cwd
        rewritten, cd_note = rewrite_phantom_cd_command(command, session_cwd)
        if rewritten != command:
            logger.info(
                "Rewrote terminal command for session cwd %s: %r -> %r (%s)",
                session_cwd,
                command[:120],
                rewritten[:120],
                cd_note,
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("ECTOR_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("ECTOR_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("ECTOR_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("ECTOR_SESSION_USER_ID", "")
                        _gw_user_name = _gse("ECTOR_SESSION_USER_NAME", "")
            ...
            if returncode == 0:
                try:
                    from tools.approval import (
                        get_current_session_key as _get_approval_session_key,
                        register_managed_temp_artifacts_from_command,
                    )
                    register_managed_temp_artifacts_from_command(
                        _get_approval_session_key(default=""),
                        command,
                    )
                except Exception:
                    pass
            ...
            try:
                from ector_cli.plugins import invoke_hook
    ...
    try:
        if env_type == "local":
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        "Modal TERMINAL_MODAL_MODE=managed is no longer supported. "
                        "Use direct/auto and configure Modal credentials."
                    )
                elif modal_state["mode"] == "direct":
                    logger.error(
                        "Modal TERMINAL_MODAL_MODE=direct but no Modal credentials/config found."
                    )
                else:
                    logger.error(
                        "Modal backend selected but no direct Modal credentials/config was found."
