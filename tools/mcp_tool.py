...
discovers their tools, and registers them into the ector-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.ector/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.ector/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            ector_home = os.path.expanduser(
                os.getenv(
                    "ECTOR_HOME", os.path.join(os.path.expanduser("~"), ".ector")
                )
            )
            candidates = [
                os.path.join(ector_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            try:
                from agent.tool_router import notify_registry_tools_changed
                notify_registry_tools_changed()
            except Exception:
                logger.debug("Tool routing registry notification failed", exc_info=True)
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `ector mcp login {server_name}` (or delete the tokens "
            f"file under ~/.ector/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Ector config file.
    ...
    ``os.environ`` (which includes ``~/.ector/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Ector's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Ector registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``ector tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Ector registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple ector
