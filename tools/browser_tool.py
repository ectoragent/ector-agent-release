...
supports multiple backends — **Browser Use** (cloud, default for Ector
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    ector_node_bin = str(ector_home / "node" / "bin")
    return [ector_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    if _cached_cloud_provider is None:
        # Prefer Browser Use (managed Ector gateway or direct API key),
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-ector_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed ector processes — this way every clean ector exit sweeps
    ...
    # Sweep orphans from other crashed ector processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live ector processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current ector PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live ector process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning ector process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced ector PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent ector instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-ector_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live ector process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Ector-managed
    ...
    # Build an extended search PATH including Ector-managed Node, macOS
...
def _run_browser_command(
    ...
    try:
        ...
        # Record this ector PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = ector_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_ector_dir("cache/screenshots", "browser_screenshots")
