...
it is automatically downloaded from GitHub releases to $ECTOR_HOME/bin/tirith
(falls back to legacy $ECTOR_HOME).
...
def _get_ector_home() -> str:
    """Return the Ector home directory (supports ECTOR_HOME and legacy ECTOR_HOME)."""
    return str(get_ector_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_ector_home(), ".tirith-install-failed")
...
def _ector_bin_dir() -> str:
    """Return $ECTOR_HOME/bin (or legacy $ECTOR_HOME/bin), creating it if needed."""
    d = os.path.join(_get_ector_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ECTOR_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_ector_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ECTOR_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ECTOR_HOME/bin/tirith
    ...
    ector_bin = os.path.join(_ector_bin_dir(), "tirith")
    if os.path.isfile(ector_bin) and os.access(ector_bin, os.X_OK):
        _resolved_path = ector_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return ector_bin
