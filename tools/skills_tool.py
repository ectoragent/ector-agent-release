...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ECTOR_HOME/.env."""
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ECTOR_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_ector_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.ector/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ECTOR_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("ECTOR_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ECTOR_PLATFORM`` environment variable
    3. ``ECTOR_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from ector_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ECTOR_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``ector skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_ector_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from ector_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: ector plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            ...
            from ector_cli.plugins import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.ector/skills/): {skill_md}")
        ...
        # Check metadata.ector.* first (agentskills.io convention), fall back to top-level
        ector_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            ector_meta = metadata.get("ector", {}) or {}

        tags = _parse_tags(ector_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            ector_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
