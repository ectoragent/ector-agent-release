#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Ector Skills Hub.
...
Used by ector_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            ector_meta = metadata.get("ector", {})
            if isinstance(ector_meta, dict):
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Ector) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.ector/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                ector_meta = meta_block.get("ector", {})
                if isinstance(ector_meta, dict):
                    tags = ector_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Ector centralized index source
# ---------------------------------------------------------------------------

ECTOR_HUB_BASE_URL = os.environ.get("ECTOR_HUB_BASE_URL", "https://ector.cc").rstrip("/")
ECTOR_INDEX_URL = os.environ.get(
    "ECTOR_INDEX_URL",
    f"{ECTOR_HUB_BASE_URL}/docs/api/skills-index.json",
)
ECTOR_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "ector-index.json"
ECTOR_INDEX_TTL = 6 * 3600  # 6 hours


def _hub_bundle_url(entry: dict) -> Optional[str]:
    """Resolve the public bundle URL for a hub index entry."""
    extra = entry.get("extra") or {}
    bundle_url = str(extra.get("bundle_url") or "").strip()
    if bundle_url:
        return bundle_url
    slug = str(extra.get("hub_slug") or "").strip()
    if slug:
        return f"{ECTOR_HUB_BASE_URL}/api/skills/{slug}/bundle"
    return None


def _fetch_hub_bundle(entry: dict, identifier: str) -> Optional[SkillBundle]:
    """Download skill files from the Ector Hub bundle API."""
    bundle_url = _hub_bundle_url(entry)
    if not bundle_url:
        return None

    try:
        resp = httpx.get(bundle_url, timeout=30, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug(
                "Hub bundle fetch returned %d for %s",
                resp.status_code,
                bundle_url,
            )
            return None
        ...
        logger.debug("Hub bundle fetch failed: %s", exc)
        return None

    files_raw = data.get("files")
    if not isinstance(files_raw, dict) or "SKILL.md" not in files_raw:
        return None

    files: Dict[str, Union[str, bytes]] = {}
    for rel_path, content in files_raw.items():
        if not isinstance(rel_path, str) or not rel_path:
            continue
    ...
    return SkillBundle(
        ...
        identifier=identifier,
        trust_level=str(entry.get("trust_level") or "builtin"),
        metadata={
            "hub_slug": extra.get("hub_slug") or data.get("slug"),
...
def _load_ector_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ECTOR_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ECTOR_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ECTOR_INDEX_CACHE_FILE.stat().st_mtime
            if age < ECTOR_INDEX_TTL:
                return json.loads(ECTOR_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ECTOR_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Ector index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Ector index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ECTOR_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ECTOR_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class EctorIndexSource(SkillSource):
    """Skill source backed by the centralized Ector Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        """Fetch a skill from the Ector Hub bundle API or GitHub fallback."""
        index = self._ensure_loaded()
        entry = self._find_entry(identifier, index)
        if not entry:
            return None

        bundle = _fetch_hub_bundle(entry, identifier)
        if bundle:
            return bundle

        # Use resolved path if available (legacy / mirrored GitHub skills)
        resolved = entry.get("resolved_github_id")
        if resolved:
            bundle = self._get_github().fetch(resolved)
            if bundle:
                bundle.source = entry.get("source", "ector-index")
    ...
    def _find_entry(self, identifier: str, index: dict) -> Optional[dict]:
        """Look up a skill in the index by identifier, slug, or name."""
        skills = index.get("skills", [])
        ...
        # Hub slug match (e.g. ector-hub-onboarding)
        for s in skills:
            extra = s.get("extra") or {}
            hub_slug = extra.get("hub_slug")
            if hub_slug and hub_slug == identifier:
                return s
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "ector-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        EctorIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def _source_matches_filter(source_id: str, source_filter: str) -> bool:
    """Map CLI --source values to SkillSource.source_id()."""
    if source_filter == "all":
        return True
    if source_filter == source_id:
        return True
    if source_filter == "builtin" and source_id == "official":
        return True
    if source_filter == "hub" and source_id == "ector-index":
        return True
    return False
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    for src in sources:
        sid = src.source_id()
        if not _source_matches_filter(sid, source_filter):
            continue
