...
DEFAULT_LOCAL_STT_LANGUAGE = "pt"
DEFAULT_STT_MODEL = os.getenv("STT_OPENAI_MODEL", "whisper-1")
DEFAULT_GROQ_STT_MODEL = os.getenv("STT_GROQ_MODEL", "whisper-large-v3-turbo")
DEFAULT_MISTRAL_STT_MODEL = os.getenv("STT_MISTRAL_MODEL", "voxtral-mini-latest")
LOCAL_STT_COMMAND_ENV = "ECTOR_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ECTOR_LOCAL_STT_LANGUAGE"
...
def _whisper_language_code(lang: str) -> str:
    """Normalize config/UI language tags to ISO 639-1 for Whisper backends."""
...
def _stt_language_for_api() -> Optional[str]:
    """Resolved STT language for API calls (None = provider auto-detect)."""
    configured = (
    ...
    if raw.lower() in ("auto", "detect", "automatic"):
        return None
    if not raw:
        return DEFAULT_LOCAL_STT_LANGUAGE
    return _whisper_language_code(raw)


def _resolve_stt_language() -> str:
    """ISO 639-1 language for STT; defaults to Portuguese (pt-BR)."""
    return _stt_language_for_api() or DEFAULT_LOCAL_STT_LANGUAGE
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set ECTOR_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local(file_path: str, model_name: str, *, fast: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        stt_lang = _stt_language_for_api()
        if stt_lang:
            transcribe_kwargs["language"] = stt_lang
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="ector-local-stt-") as output_dir:
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = _stt_language_for_api()
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = str(xai_config.get("language") or "").strip() or _resolve_stt_language()
    ...
    try:
        import requests
        from tools.xai_http import ector_xai_user_agent
        ...
        with open(file_path, "rb") as audio_file:
            response = requests.post(
                ...
                headers={
                    ...
                    "User-Agent": ector_xai_user_agent(),
                },
                files={
...
def transcribe_web_chat_audio(file_path: str) -> Dict[str, Any]:
    """Low-latency STT for the dashboard voice composer.

    Prefers cloud APIs (Groq/OpenAI/…) when keys are available — much faster than
    loading faster-whisper for short browser clips. Falls back to local with
    reduced beam search for interactive use.
    """
    ...
    if _HAS_OPENAI and os.getenv("GROQ_API_KEY"):
        result = _transcribe_groq(file_path, DEFAULT_GROQ_STT_MODEL)
        if result.get("success"):
            return result

    if _HAS_OPENAI and _has_openai_audio_backend():
        openai_cfg = stt_config.get("openai", {})
        model_name = openai_cfg.get("model", DEFAULT_STT_MODEL)
    ...
    if _HAS_MISTRAL and os.getenv("MISTRAL_API_KEY"):
        ...
        model_name = mistral_cfg.get("model", DEFAULT_MISTRAL_STT_MODEL)
    ...
    if os.getenv("XAI_API_KEY"):
        ...
        model_name = xai_cfg.get("model", "grok-stt")
    ...
    if _HAS_FASTER_WHISPER:
        ...
        return _transcribe_local(file_path, model_name, fast=True)
    ...
    return {
        ...
        "transcript": "",
        "error": (
            "No STT provider available for web transcription. Set GROQ_API_KEY, "
            "install faster-whisper, or configure another STT provider."
        ),
    }
