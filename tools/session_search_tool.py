...
_SESSION_SEARCH_CACHE_TTL_SECONDS = 180
_SESSION_SEARCH_CACHE: dict[str, tuple[float, str]] = {}
_SESSION_SEARCH_CACHE_LOCK = threading.Lock()
_CACHE_TOKEN_RE = re.compile(r"[a-zA-Z0-9_]+", re.UNICODE)
_CACHE_STOPWORDS = {
    "a", "as", "o", "os", "de", "da", "do", "das", "dos", "e", "ou", "que",
    "como", "faz", "fazer", "mesmo", "isso", "aquilo", "com", "sem", "por",
    "para", "um", "uma", "uns", "umas", "na", "no", "nas", "nos", "em",
    "the", "a", "an", "and", "or", "to", "of", "in", "on", "for", "with",
}


def _normalize_query_for_cache(query: str) -> str:
    """Build a stable cache key segment for semantically similar queries."""
    tokens = [
        token.lower()
        for token in _CACHE_TOKEN_RE.findall(query or "")
        if len(token) >= 3 and token.lower() not in _CACHE_STOPWORDS
    ]
    if not tokens:
        return (query or "").strip().lower()
    # Sort + dedupe to collapse word-order variants.
...
def _make_cache_key(
    ...
    *,
    ...
    role_key = (role_filter or "").strip().lower()
    session_key = (current_session_id or "").strip()
    sum_flag = "1" if summarize else "0"
    return (
        f"{_normalize_query_for_cache(query)}|roles={role_key}|limit={limit}|sid={session_key}"
        f"|sum={sum_flag}|smc={snippet_max_chars}"
    )
...
def _cache_get(key: str) -> Optional[str]:
    ...
    with _SESSION_SEARCH_CACHE_LOCK:
        hit = _SESSION_SEARCH_CACHE.get(key)
        if not hit:
            return None
        ts, payload = hit
        if now - ts > _SESSION_SEARCH_CACHE_TTL_SECONDS:
            _SESSION_SEARCH_CACHE.pop(key, None)
            return None
        return payload


def _cache_set(key: str, payload: str) -> None:
    now = time.time()
    with _SESSION_SEARCH_CACHE_LOCK:
        _SESSION_SEARCH_CACHE[key] = (now, payload)
        # Opportunistic tiny prune to keep memory bounded.
        if len(_SESSION_SEARCH_CACHE) > 256:
            cutoff = now - _SESSION_SEARCH_CACHE_TTL_SECONDS
            stale_keys = [k for k, (ts, _) in _SESSION_SEARCH_CACHE.items() if ts < cutoff]
            for k in stale_keys[:128]:
                _SESSION_SEARCH_CACHE.pop(k, None)
...
def _get_session_search_summarize(default: bool = False) -> bool:
    """Read auxiliary.session_search.summarize (LLM per session vs FTS snippets only)."""
    try:
        from ector_cli.config import load_config
        config = load_config()
    except ImportError:
...
def _get_session_search_snippet_max_chars(default: int = 2400) -> int:
    """Cap total snippet text per session in non-summarize mode."""
    try:
        from ector_cli.config import load_config
        config = load_config()
    except ImportError:
    ...
    if not isinstance(task_config, dict):
        return default
    raw = task_config.get("snippet_max_chars")
    if raw is None:
        return default
    try:
        value = int(raw)
        ...
        return default
    return max(400, min(value, 50_000))
...
# ECTOR_SESSION_SOURCE=tool so they don't clutter the user's session history.
...
def _build_local_session_snippets(
    raw_results: List[Dict[str, Any]],
    session_id: str,
    resolve_parent: Any,
    max_chars: int,
) -> str:
    """Join FTS snippets for one resolved parent session (no auxiliary LLM)."""
    chunks: List[str] = []
    seen_keys: set[str] = set()
    for row in raw_results:
        if resolve_parent(row["session_id"]) != session_id:
            continue
        frag = (row.get("snippet") or "").strip()
        if not frag:
            content = (row.get("content") or "").strip()
            if content:
                frag = content[:800] + ("…" if len(content) > 800 else "")
        if not frag:
            continue
        dedup = frag[:200]
...
def session_search(
    ...
    Uses FTS5 to find matches. With auxiliary.session_search.summarize true, each
    hit session is summarized via the auxiliary model; with summarize false (default),
    returns local FTS snippets only (fast, no extra API).
    ...
    query = query.strip()
    summarize = _get_session_search_summarize()
    snippet_max = _get_session_search_snippet_max_chars()
    ...
    try:
        ...
        if not summarize:
            ...
            def _resolve_cached(sid: str) -> str:
                if sid not in resolve_cache:
                    resolve_cache[sid] = _resolve_to_parent(sid)
                return resolve_cache[sid]

            summaries_fast: List[Dict[str, Any]] = []
            for sid, match_info in seen_sessions.items():
                body = _build_local_session_snippets(
                    raw_results, sid, _resolve_cached, snippet_max
                )
                if not body:
                    body = (
                        (match_info.get("snippet") or match_info.get("content") or "")
                        .strip()[:snippet_max]
                    )
                if not body:
                    body = "(sem trechos locais para esta sessão)"
                summaries_fast.append({
                    "session_id": sid,
            ...
            payload = json.dumps({
                "success": True,
                "query": query,
                "results": summaries_fast,
                "count": len(summaries_fast),
                "sessions_searched": len(seen_sessions),
                "mode": "fts_snippets",
                "message": (
                    "Apenas trechos locais (FTS), sem modelo auxiliar. "
                    "Para resumos com LLM: auxiliary.session_search.summarize: true em config.yaml."
                ),
            }, ensure_ascii=False)
            _cache_set(cache_key, payload)
            return payload
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "By default returns local FTS snippets only (fast). If auxiliary.session_search.summarize "
        "is true in config, returns LLM-generated summaries per session.\n\n"
        ...
        "Don't hesitate to search when it is actually cross-session — with default settings it's local-only. "
        "Better to search and confirm than to guess or ask the user to repeat themselves.\n\n"
        "Search syntax: keywords joined with OR for broad recall (elevenlabs OR baseten OR funding), "
        "phrases for exact match (\"docker networking\"), boolean (python NOT java), prefix (deploy*). "
        "IMPORTANT: Use OR between keywords for best results — FTS5 defaults to AND which misses "
        "sessions that only mention some terms. If a broad OR query returns nothing, try individual "
        "keyword searches in parallel. Returns the top matching sessions (snippets or summaries per config)."
    ),
    "parameters": {
        "type": "object",
        "properties": {
