...
def _is_fetch_promise_without_body(text: str, has_media: bool) -> bool:
    """True only when ``message`` is clearly a hollow fetch/send line about news, no payload.

    Conservative: if anything looks like delivered content (list, URL, long tail after
    ``:``, several sentences), we do **not** block — the model keeps full freedom.
    """
    if has_media:
        return False
    raw = (text or "").strip()
    if not raw or len(raw) > 360:
        return False
    tl = unicodedata.normalize("NFKC", raw).casefold()
    if "http://" in tl or "https://" in tl:
        return False
    # --- Positive signals: real body present → never block ---
    if re.search(r"\d+[\.)]\s+\S{10,}", raw):
        return False
    if re.search(r"(?m)^\s*[-*•]\s+\S{12,}", raw):
        return False
    if raw.count("\n") >= 1 and len(raw) >= 140:
        return False
    if raw.count(".") >= 2 and len(re.sub(r"\s+", " ", raw)) >= 100:
        return False
    if ":" in raw:
        _, after_colon = raw.split(":", 1)
        if len(after_colon.strip()) >= 50:
            return False
    lines = [ln.strip() for ln in raw.splitlines() if ln.strip()]
    if len(lines) >= 2 and max(len(ln) for ln in lines[1:]) >= 48:
        return False
    if len(raw.split()) > 28:
        return False
    fetch_start = bool(
        re.match(
            r"^\s*(?:agora\s+)?(?:vou\s+(?:buscar|pesquisar|procurar|enviar|mandar)|"
            r"(?:estou|t[oô])\s+(?:buscando|pesquisando|enviando)|"
            r"i(?:['’]ll| will)\s+(?:fetch|search|send|look\s+up))\b",
            raw,
            re.I,
        )
    )
    if not fetch_start:
        return False
    # News/headlines context only — avoid blocking unrelated "vou buscar X" short lines.
    newsish = bool(
        re.search(r"\bnot[ií]cias?\b", tl, re.I)
        or re.search(r"\bheadlines?\b", tl, re.I)
        or re.search(r"\bthe\s+news\b", tl, re.I)
        or re.search(r"\bnews\s+(?:from|on|about|for)\b", tl, re.I)
        or re.search(r"\b(?:latest|breaking)\s+news\b", tl, re.I)
    )
    return newsish
...
SEND_MESSAGE_SCHEMA = {
    ...
    "description": (
        ...
        "If a **home channel** is configured for a platform (often set via /sethome on that "
        "platform, stored as e.g. WHATSAPP_HOME_CHANNEL), the user phrases like "
        "'send to me', 'my number', 'my WhatsApp', or 'pra mim' mean that home — use "
        "`target` with **only** the platform name (e.g. `whatsapp`, `telegram`) and the "
        "message text. Do **not** ask for their phone or chat id in that case.\n"
        "If the user just says a platform name like 'send to telegram', send directly "
        "to the home channel without listing first.\n"
        "Anti-placeholder: Do **not** use ``send_message`` only for status lines like "
        "\"I will fetch/send the news\" or \"you will receive updates soon\" when the user "
        "asked for **substantive content** (news, summaries, lists, answers). Run the "
        "needed tools first (e.g. ``web_search`` for current news), then send **one** "
        "message (or split chunks) that actually contains that content. Hollow pings "
        "to WhatsApp/Telegram while claiming work is in progress are wrong — either deliver "
        "the payload in the same tool call batch after lookup, or reply only in this chat "
        "until you have text worth sending.\n"
        "If the user is in CLI (or another surface) and asked to mirror to WhatsApp, you may "
        "acknowledge briefly **here** in the assistant message; the off-channel ``message`` "
        "should still be the real body, not a duplicate promise."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target. Format: 'platform' (uses home channel), 'platform:#channel-name', 'platform:chat_id', or 'platform:chat_id:thread_id' for Telegram topics and Discord threads. Examples: 'telegram', 'telegram:-1001234567890:17585', 'discord:999888777:555444333', 'discord:#bot-home', 'slack:#engineering', 'whatsapp:+15551234567'"
            },
            "message": {
                "type": "string",
                "description": (
                    "The full text to deliver — not a promise to deliver later. For 'send the news to X', include the "
                    "news summary **after** ``web_search`` (or equivalent), same turn when possible. "
                    "Plain, readable wording; for WhatsApp, markdown emphasis is stripped to UTF-8. "
                    "To attach media, include MEDIA:<local_path> in the string."
                ),
            }
        },
        "required": []
...
def _handle_send(args):
    ...
    if not pconfig or not pconfig.enabled:
        return tool_error(
            ...
            "Set up credentials in ~/.ector/config.yaml or environment variables."
        )
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel in ~/.ector/.env ({platform_name.upper()}_HOME_CHANNEL=<channel_id>)"
            })
    ...
    if _is_fetch_promise_without_body(cleaned_message, bool(media_files)):
        return tool_error(
            "send_message rejected: this ``message`` reads like a status ping about news "
            "(no digest). Put the actual headlines/summary in ``message`` after "
            "web_search or browser tools — or reply only in this chat until you have that text. "
            "Short acknowledgements belong in the assistant reply, not as the sole outbound body."
        )
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("ECTOR_SESSION_PLATFORM", "cli")
                user_id = get_session_env("ECTOR_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("ECTOR_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("ECTOR_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("ECTOR_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_whatsapp(extra, chat_id, message):
    ...
    try:
        try:
            from gateway.platforms.whatsapp import normalize_whatsapp_outbound_text

            message = normalize_whatsapp_outbound_text(message)
        except Exception:
            pass
...
def _check_send_message():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("ECTOR_SESSION_PLATFORM", "")
