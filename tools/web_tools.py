...
Backend is selected during ``ector tools`` setup (``web.backend`` in config.yaml).
Bring your own API keys (Tavily, Firecrawl, Exa, Parallel, or ``FIRECRAWL_API_URL``).
The managed tool gateway for subscribers was removed; legacy ``web.use_gateway`` is
cleared on config migration.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract, crawl; direct API key or custom URL)
...
with warnings.catch_warnings():
    # firecrawl's Pydantic models shadow BaseModel.json — noisy on every import.
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.ector/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``ector tools``).
...
def _get_firecrawl_client():
    """Get or create Firecrawl client from direct env configuration."""
    global _firecrawl_client, _firecrawl_client_config

    direct_config = _get_direct_firecrawl_config()
    if direct_config is None:
        if prefers_gateway("web"):
            logger.error(
                "Firecrawl: web.use_gateway is set but managed tool gateway was removed; "
                "configure FIRECRAWL_API_KEY or FIRECRAWL_API_URL instead."
            )
        else:
...
def _is_ector_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is hosted on ector.cc."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "ector.cc" or host.endswith(".ector.cc")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_ector_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=ector-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "ector-agent"
    return _exa_client
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        print(
            "Set EXA_API_KEY, PARALLEL_API_KEY, TAVILY_API_KEY, FIRECRAWL_API_KEY, or FIRECRAWL_API_URL"
        )

    if not aux_available:
        print("❌ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY or set OPENAI_BASE_URL + OPENAI_API_KEY")
        print("▲  Without an auxiliary model, LLM content processing will be disabled")
    else:
