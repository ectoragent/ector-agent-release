...
class BrowserUseProvider(CloudBrowserProvider):
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Ector gateway)
    # ------------------------------------------------------------------
    ...
    def _get_config(self) -> Dict[str, Any]:
        ...
        if config is None:
            if prefers_gateway("browser"):
                raise ValueError(
                    "browser.use_gateway is set but the managed tool gateway was removed; "
                    "set BROWSER_USE_API_KEY instead."
                )
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Ector only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"ector_{task_id}_{uuid.uuid4().hex[:8]}"
