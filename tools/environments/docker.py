...
from tools.environments.local import _ECTOR_PROVIDER_ENV_BLOCKLIST
...
def _load_ector_env_vars() -> dict[str, str]:
    """Load ~/.ector/.env values without failing Docker command execution."""
    try:
        from ector_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ECTOR_DOCKER_BINARY`` env var (legado: ``ECTOR_DOCKER_BINARY``) — sobrescrita explícita (ex.: ``/usr/bin/podman``)
    ...
    override = os.getenv("ECTOR_DOCKER_BINARY") or os.getenv("ECTOR_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ECTOR_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'ector'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.ector/sandboxes/). Non-persistent
        ...
        container_name = f"ector-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Ector secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ECTOR_PROVIDER_ENV_BLOCKLIST)
        ector_env = _load_ector_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = ector_env.get(key)
