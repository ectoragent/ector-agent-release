...
# Ector-internal env vars that should NOT leak into terminal subprocesses.
_ECTOR_PROVIDER_ENV_FORCE_PREFIX = "_ECTOR_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from ector_cli.config import OPTIONAL_ENV_VARS
...
_ECTOR_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Ector-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ECTOR_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _ECTOR_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ECTOR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ECTOR_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _ECTOR_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ECTOR_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. Ector Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ECTOR_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ECTOR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ECTOR_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _ECTOR_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # npm …) into {ECTOR_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Ector trusts them.
    """
