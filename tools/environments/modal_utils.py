"""Shared Ector-side execution flow for Modal transports.

This module deliberately stops at the Ector boundary:
- command preparation
...
Direct Modal transport lives in ``tools.environments.modal``. The managed Modal
gateway was removed; ``BaseModalExecutionEnvironment`` remains for reference only.
"""
...
def wrap_modal_stdin_heredoc(command: str, stdin_data: str) -> str:
    ...
    marker = f"ECTOR_EOF_{uuid.uuid4().hex[:8]}"
...
class BaseModalExecutionEnvironment(BaseEnvironment):
    """Legacy managed-Modal flow (no longer wired — gateway transport removed).

    Subclasses are not registered after ``managed_modal`` removal. Direct Modal
    uses :class:`tools.environments.modal.ModalEnvironment` instead.
    """
