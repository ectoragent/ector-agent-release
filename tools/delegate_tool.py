...
from agent.tool_name_aliases import LEGACY_WISER_TOOLSET_KEYS
...
DELEGATE_BLOCKED_TOOLS = frozenset(
    [
        ...
        "wiser",  # no user interaction in subagents
...
# (ector-* prefixed), and scenario toolsets.
#
...
def _build_child_system_prompt(
    ...
    *,
    ...
    Subagent system prompt for spawn-capable delegates.
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on ector.cc).
    """
    ...
    # Extract parent's API key so subagents inherit auth (e.g. ector.cc).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.ector/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, ector, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'ector auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (ector_cli/config.py load_config()) so that
...
DELEGATE_TASK_SCHEMA = {
    ...
    "description": (
        ...
        "2. Batch (parallel): provide 'tasks' array with up to delegation.max_concurrent_children items (default 10, configurable via config.yaml, no hard ceiling). "
        ...
        "- Tasks needing user interaction -> subagents cannot use wiser\n\n"
        "IMPORTANT:\n"
        ...
        "delegate_task, wiser, memory, send_message, execute_code.\n"
        "- Orchestrator subagents (role='orchestrator') retain "
        "delegate_task so they can spawn their own workers, but still "
        "cannot use wiser, memory, send_message, or execute_code. "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "tasks": {
                ...
                "description": (
                    "Batch mode: tasks to run in parallel (limit configurable via delegation.max_concurrent_children, default 10). Each gets "
