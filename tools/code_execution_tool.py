...
Lets the LLM write a Python script that calls Ector tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `ector_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `ector_tools.py` with file-based RPC stubs
...
def generate_ector_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the ector_tools.py stub module.
...
"""Auto-generated Ector tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["ECTOR_RPC_SOCKET"])
...
"""Auto-generated Ector tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("ECTOR_RPC_DIR") or os.path.join(tempfile.gettempdir(), "ector_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated ector_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/ector_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_ector_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/ector_tools.py", tools_src)
        ...
        env_prefix = (
            f"ECTOR_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ECTOR_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with ector_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="ector_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"ector_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated ector_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_ector_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "ector_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["ECTOR_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        try:
            from ector_constants import get_agent_images_dir

            child_env["ECTOR_IMAGES_DIR"] = str(get_agent_images_dir())
        except Exception:
            pass
        # Ensure the ector-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from ector_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _ector_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _ector_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ECTOR_TIMEZONE is an internal Ector setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ECTOR_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ECTOR_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ECTOR_HOME}/home/ when that directory exists.
        from ector_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.ector/.env')).
...
def _load_config() -> dict:
    ...
    try:
        from ector_cli.cli_config import CLI_CONFIG
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (ector-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``ector tools`` (e.g. web is turned off),
    ...
    try:
        ...
        _display_home = display_ector_home()
    except Exception:
        _display_home = "~/.ector"

    _images_home = f"{_display_home}/images"
    ...
    # temp-dir staging and ector-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            f"({_display_home}/.env via os.path.expanduser) or terminal()/read_file() "
            "for user files."
        )
    else:
        cwd_note = (
    ...
    images_note = (
        f"Save charts/plots/images under os.environ['ECTOR_IMAGES_DIR'] "
        f"(set to {_images_home}) — never the home directory root or /tmp. "
        f"Example: plt.savefig(os.path.join(os.environ['ECTOR_IMAGES_DIR'], 'chart.png'))."
    )

    description = (
        "Run a Python script that can call Ector tools programmatically. "
        ...
        f"Available via `from ector_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into ector_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from ector_tools import {import_str}` "
