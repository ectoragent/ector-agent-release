...
if tinker_atropos_dir.exists():
    ...
    os.environ['ECTOR_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to repo root if submodule not found
...
from ector_constants import get_ector_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_ector_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.ector/config.yaml if not provided)
    ...
    """
    # Load config from ~/.ector/config.yaml
    config = load_ector_config()
