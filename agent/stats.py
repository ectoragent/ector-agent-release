"""
Session Stats Engine for Ector Agent.

Analyzes historical session data from the SQLite state database to produce
comprehensive usage statistics — token consumption, cost estimates, tool usage
...
class StatsEngine:
    """
    Analyzes session history and produces usage statistics.
    ...
    def __init__(self, db):
        ...
        Args:
            db: A SessionDB instance (from ector_state.py)
        """
        self.db = db
        self._conn = db._conn
    ...
    def generate(self, days: int = 30, source: str = None) -> Dict[str, Any]:
        """
        Generate a complete stats report.
    ...
    def format_terminal(self, report: Dict) -> str:
        """Format the stats report for terminal display (CLI)."""
        if report.get("empty"):
    ...
    def format_gateway(self, report: Dict) -> str:
        """Format the stats report for gateway/messaging (shorter)."""
        if report.get("empty"):
            days = report.get("days", 30)
        ...
        lines.append(f"📊 **Ector Stats** — Last {days} days\n")
