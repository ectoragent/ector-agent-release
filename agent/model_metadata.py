...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Ector also honours ECTOR_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ECTOR_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    ...
    "ollama",
    "stepfun", "vercel", "kilo", "dashscope", "aliyun", "qwen",
    "mimo", "xiaomi-mimo",
    "arcee-ai", "arceeai",
    "xai", "x-ai", "x.ai", "grok",
...
# Minimum context length required to run Ector Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "qwen": 131072,
    # MiniMax — M3: 1M context; M2.x: 204,800 (official Anthropic-compat docs)
    ...
    "grok-4.3": 1000000,
    "grok-code-fast": 256000,   # grok-code-fast-1 (legacy; prefer grok-4.3)
...
def _get_context_cache_path() -> Path:
    ...
    return get_ector_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Ector grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Ector uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_ector_catalog_context_length(model: str) -> Optional[int]:
    """Resolve ector.cc catalog model context length via OpenRouter metadata.

    Ector model IDs are bare (e.g. 'claude-opus-4-6') while OpenRouter uses
...
def get_model_context_length(
    ...
    6. Ector suffix-match via OpenRouter cache
    ...
    # This closes the gap where /provider switch and display paths used to fall
    ...
    if effective_provider in ("copilot", "copilot-acp", "github-copilot"):
        try:
            from ector_cli.models import get_copilot_model_context
    ...
    if effective_provider == "ector":
        ctx = _resolve_ector_catalog_context_length(model)
        if ctx:
            return ctx
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Ector sends to providers:
    system prompt, conversation messages, and tool schemas.  With 50+
    tools enabled, schemas alone can add 20-30K tokens — a significant
    blind spot when only counting messages.

    Uses tiktoken when available; falls back to chars/4.
    """
    try:
