...
Data resolution order:
  ...
  2. Disk cache (~/.ector/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Ector ↔ models.dev
# ---------------------------------------------------------------------------

# Ector provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Ector (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_ector_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Ector provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Ector agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Ector provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Ector ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Ector or models.dev provider ID.  Tries exact match then
