...
the ector.cc header format (also used by OpenRouter and OpenAI-compatible
...
def _bar(pct: float, width: int = 20) -> str:
    """Unicode progress bar: thin caps + full blocks + partial + dim track (no [])."""
    safe_width = max(1, int(width))
    safe_pct = float(pct)
    if not math.isfinite(safe_pct):
        safe_pct = 0.0
    safe_pct = max(0.0, min(100.0, safe_pct))

    # 8-step sub-cell resolution (▏..▉) gives smoother feedback than integer cells.
    filled_units = (safe_pct / 100.0) * safe_width
    full_cells = int(filled_units)
    frac = filled_units - full_cells
    partial_step = int(round(frac * 8))

    if partial_step == 8:
        full_cells += 1
        partial_step = 0

    full_cells = max(0, min(safe_width, full_cells))
    partial_chars = ["", "▏", "▎", "▍", "▌", "▋", "▊", "▉"]
    partial = partial_chars[partial_step] if full_cells < safe_width else ""
    empty_cells = safe_width - full_cells - (1 if partial else 0)
...
def _bucket_line(label: str, bucket: RateLimitBucket, label_width: int = 14) -> str:
    ...
    if bucket.limit <= 0:
        return f"  {label:<{label_width}}  (sem dados)"
    ...
    bar = _bar(pct)
    return f"  {label:<{label_width}} {bar} {pct:5.1f}%  {used}/{limit} usados  ({remaining} restantes, reinicia em {reset})"


def format_rate_limit_display(state: RateLimitState) -> str:
    """Format rate limit state for terminal/chat display."""
    if not state.has_data:
        return "Ainda não há dados de limite de taxa — faça uma chamada à API primeiro."

    age = state.age_seconds
    if age < 5:
        freshness = "agora"
    elif age < 60:
        freshness = f"há {int(age)}s"
    else:
        freshness = f"há {_fmt_seconds(age)}"

    provider_label = state.provider.title() if state.provider else "Provedor"

    lines = [
        f"Limites de taxa — {provider_label} (capturado {freshness}):",
        "",
        _bucket_line("Req./min", state.requests_min),
        _bucket_line("Req./hora", state.requests_hour),
        "",
        _bucket_line("Tok./min", state.tokens_min),
        _bucket_line("Tok./hora", state.tokens_hour),
    ]
    ...
    for label, bucket in [
        ...
        ("req./hora", state.requests_hour),
        ("tok./min", state.tokens_min),
        ("tok./hora", state.tokens_hour),
    ]:
        if bucket.limit > 0 and bucket.usage_pct >= 80:
            reset = _fmt_seconds(bucket.remaining_seconds_now)
            warnings.append(f"  ▲ {label} em {bucket.usage_pct:.0f}% — reinicia em {reset}")
...
def format_rate_limit_compact(state: RateLimitState) -> str:
    ...
    if not state.has_data:
        return "Sem dados de limite de taxa."
