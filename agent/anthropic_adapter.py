"""Anthropic Messages API adapter for Ector Agent.

Translates between Ector's internal OpenAI-style message format and
...
# Ector effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _resolve_anthropic_messages_max_tokens(
    ...
    Resolve max_tokens for Anthropic Messages API requests.
    """
...
# Matches what Claude Code (and similar clients) send.
_OAUTH_ONLY_BETAS = [
...
def read_claude_code_credentials() -> Optional[Dict[str, Any]]:
    ...
    This intentionally excludes ~/.claude.json primaryApiKey.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Ector historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
# ── Ector-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code and similar clients.
# Stores credentials in ~/.ector/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ECTOR_OAUTH_FILE = get_ector_home() / ".anthropic_oauth.json"
...
def run_ector_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Ector-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Autorize o Ector com sua assinatura Claude Pro/Max.")
    print()
    print("╭─ Autorização Claude Pro/Max ──────────────────────╮")
    print("│                                                   │")
    ...
    print()
    print("Após autorizar, você verá um código. Cole abaixo.")
    print()
    try:
    ...
    try:
        ...
        print(f"Falha na troca do token: {e}")
        return None
    ...
    if not access_token:
        print("Nenhum token de acesso encontrado na resposta.")
        return None
...
def read_ector_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Ector-managed OAuth credentials from ~/.ector/.anthropic_oauth.json."""
    if _ECTOR_OAUTH_FILE.exists():
        try:
            data = json.loads(_ECTOR_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Ector OAuth credentials: %s", e)
    return None
...
def convert_messages_to_anthropic(
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See ector-agent#13848.
            #
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Ector Agent", "Claude Code")
                text = text.replace("Ector agent", "Claude Code")
                text = text.replace("ector-agent", "claude-code")
                text = text.replace("Ector", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Ector surfaces in its CLI. We
