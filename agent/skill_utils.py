...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ECTOR_PLATFORM`` or
            ``ECTOR_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.ector/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    ector = metadata.get("ector") or {}
    if not isinstance(ector, dict):
        ector = {}
    return {
        "fallback_for_toolsets": ector.get("fallback_for_toolsets", []),
        "requires_toolsets": ector.get("requires_toolsets", []),
        "fallback_for_tools": ector.get("fallback_for_tools", []),
        "requires_tools": ector.get("requires_tools", []),
    }
