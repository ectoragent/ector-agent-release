...
Synthesized from:
- Public reverse-engineered Gemini OAuth flows — overall flow shape, public OAuth creds, request format
...
Storage (``~/.ector/auth/google_oauth.json``, chmod 0o600):
...
This matches the reverse-engineered storage contract exactly.
...
is consistent with the reverse-engineered flow and the official Google gemini-cli.
...
#
# Resolution order:
#   1. ECTOR_GEMINI_CLIENT_ID / ECTOR_GEMINI_CLIENT_SECRET env vars (power users)
...
#      security). Using these matches the reverse-engineered behavior.
...
ENV_CLIENT_ID = "ECTOR_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "ECTOR_GEMINI_CLIENT_SECRET"
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "ECTOR_HEADLESS")
...
def _credentials_path() -> Path:
    return get_ector_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "Ector looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
            "  1. Install it: npm install -g @google/gemini-cli  (or brew install gemini-cli)\n"
            "  2. Set ECTOR_GEMINI_CLIENT_ID and ECTOR_GEMINI_CLIENT_SECRET in ~/.ector/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `ector login --provider google-gemini-cli` first.",
...
<html><head><meta charset="utf-8"><title>Ector — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Ector — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — Ector will walk you through a manual paste fallback.</p></body></html>
"""
...
def start_oauth_flow(
    *,
    ...
    auth_url = AUTH_ENDPOINT + "?" + urllib.parse.urlencode(params) + "#ector"

    server_thread = threading.Thread(target=server.serve_forever, daemon=True)
    server_thread.start()

    print()
    print("Abrindo seu navegador para entrar com o Google…")
...
def _paste_mode_login(
    ...
    print()
    print("Abra esta URL em um navegador em qualquer dispositivo:")
    print(f"  {auth_url}")
    print()
    print("Após o login, o Google redirecionará para localhost (que não vai carregar).")
    print("Copie a URL completa do navegador e cole abaixo.")
    print()
...
def _prompt_paste_fallback() -> Optional[str]:
    print()
    print("Cole a URL completa de redirecionamento mostrada pelo Google, OU apenas o valor do parâmetro 'code='.")
    raw = input("URL de callback ou código: ").strip()
    if not raw:
        return None
