"""AWS Bedrock Converse API adapter for Ector Agent.
...
Bedrock Converse API adapter (see extensions/amazon-bedrock patterns):
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Ector with Bedrock support: pip install -e '.[bedrock]'"
        )
...
# Priority order for AWS SDK credential env vars:
...
def resolve_aws_auth_env_var(env: Optional[Dict[str, str]] = None) -> Optional[str]:
    ...
    Resolve AWS SDK credential env var name — used to detect
...
# ---------------------------------------------------------------------------
# Failover reason classification and context-overflow detection
