...
This adapter lets Ector use the ``google-gemini-cli`` provider as if it were
...
Attribution
-----------
Translation semantics follow public reverse-engineered Gemini OAuth flows and the public
Gemini API docs. Request envelope shape
(``{project, model, user_prompt_id, request}``) is documented nowhere; it is
reverse-engineered from public implementations (including clawdbot).
"""
...
def _translate_tool_call_to_gemini(tool_call: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        # Sentinel signature — matches the reverse-engineered approach.
    ...
    def _create_chat_completion(
        ...
        **_: Any,
        ...
        headers = {
            ...
            "User-Agent": "ector-agent (gemini-cli-compat)",
            "X-Goog-Api-Client": "gl-python/ector",
...
def _gemini_http_error(response: httpx.Response) -> CodeAssistError:
    ...
    if status == 429 and error_reason == "MODEL_CAPACITY_EXHAUSTED":
        ...
        message = (
            ...
            f"not a Ector issue). Try a different Gemini model or set a "
        ...
        message = (
            ...
            f"retired. Check ector_cli/models.py for the current list."
        )
    elif err_message:
