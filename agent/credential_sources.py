"""Unified removal contract for every credential source Ector reads from.

Ector seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.ector/.env
    claude_code   — ~/.claude/.credentials.json
    ector_pkce   — ~/.ector/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (ector, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `ector auth add`
...
unify here is **removal**:
    ...
    ``ector auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, ector device_code (partial), ector_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"ector"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.ector/.env  → clear it
    ...
    """
    from ector_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Ector.",
            f"  The pool entry is now suppressed — Ector will ignore "
            f"{env_var} until you run `ector auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Ector stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `ector auth add anthropic` to re-enable if needed.",
    ])


def _remove_ector_pkce(provider: str, removed) -> RemovalResult:
    """~/.ector/.anthropic_oauth.json is ours — delete it outright."""
    from ector_constants import get_ector_home

    result = RemovalResult()
    oauth_file = get_ector_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Ector Anthropic OAuth credentials")
...
def _remove_ector_device_code(provider: str, removed) -> RemovalResult:
    """Legacy inference OAuth in auth.json providers.ector — clear and suppress.

    Provider inference login no longer uses auth.json; identity lives in
    ``~/.ector/identity.json`` (``ector login``).  We still clear stale
    ``providers.ector`` entries and suppress re-seeding from the old path.
    """
    result = RemovalResult()
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Ector auth store is not enough — _seed_from_singletons() would
    ...
    via ``ector auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `ector auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `ector auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Ector stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `ector auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="ector_pkce",
        remove_fn=_remove_ector_pkce,
        description="~/.ector/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="ector", source_id="device_code",
        remove_fn=_remove_ector_device_code,
        description="auth.json providers.ector (legacy)",
    ))
    register(RemovalStep(
