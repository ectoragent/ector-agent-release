...
# Auxiliary tasks that call chat.completions (not embeddings).
_CHAT_AUX_TASKS = frozenset({
    "compression",
    "vision",
    "web_extract",
    "session_search",
...
# Product attribution for auxiliary calls routed through ector.cc inference.
ECTOR_EXTRA_BODY = {"tags": ["product=ector-agent"]}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Ector Agent)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for ector).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. ector prefers inference_base_url).
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Ector auth store.
    ...
    try:
        from ector_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from ector_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from ector_cli.auth import is_provider_explicitly_configured
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `ector setup` or `ector provider` and enter a valid http(s) base URL."
        ) from exc
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `ector provider` but the
    #    old OPENAI_BASE_URL lingers in ~/.ector/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: ector provider to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.ector/.env",
    ...
    if main_model and _is_embedding_model_name(main_model):
        logger.warning(
            "Auxiliary auto-detect: main model %r is an embedding model — "
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: ector provider)")
                return None, None
    ...
    try:
        from ector_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("ector_cli.auth not available for provider %s", provider)
        return None, None
...
def get_auxiliary_extra_body() -> dict:
    """Return extra_body kwargs for auxiliary calls on ector.cc-hosted backends."""
    return dict(ECTOR_EXTRA_BODY)
...
def auxiliary_max_tokens_param(value: int) -> dict:
    ...
    if not or_key and base_url_hostname(custom_base) == "api.openai.com":
...
def _coerce_chat_aux_model(task: Optional[str], model: Optional[str]) -> Optional[str]:
    """Drop embedding model slugs from chat auxiliary tasks."""
    if not model or not task or task == "embedding" or task not in _CHAT_AUX_TASKS:
        return model
    if _is_embedding_model_name(model):
        logger.warning(
            "Auxiliary %s: model %r is for embeddings, not chat — ignoring",
            task,
            model,
        )
        return None
...
def _resolve_task_provider_model(
    ...
    resolved_model = _coerce_chat_aux_model(task, model or cfg_model) or None
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        # Codex adapter handles max_tokens internally; OpenRouter/Ector use max_tokens.
    ...
    if base_url_host_matches(base_url or "", "ector.cc"):
        merged_extra.setdefault("tags", []).extend(["product=ector-agent"])
    if merged_extra:
...
_aux_call_context = threading.local()


def _set_aux_call_context(
    *,
...
def _clear_aux_call_context() -> None:
    if hasattr(_aux_call_context, "ctx"):
        del _aux_call_context.ctx
...
def _validate_llm_response(response: Any, task: str = None) -> Any:
    ...
    ctx = getattr(_aux_call_context, "ctx", None) or {}
    try:
        from agent.usage_recorder import record_auxiliary_llm_usage

        record_auxiliary_llm_usage(
            response,
            task=ctx.get("task") or task,
            provider=ctx.get("provider"),
            model=ctx.get("model"),
            base_url=ctx.get("base_url"),
            api_key=ctx.get("api_key"),
            api_mode=ctx.get("api_mode"),
        )
    except Exception:
        pass
    return response
        ...
        if client is None:
            ...
            if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `ector provider`."
                )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        return await _async_call_llm_body(
            task=task,
            provider=provider,
            model=model,
            base_url=base_url,
...
async def _async_call_llm_body(
    *,
    task: str = None,
    provider: str = None,
    model: str = None,
...
# ---------------------------------------------------------------------------
# Embeddings (tool routing, semantic recall helpers)
# ---------------------------------------------------------------------------

_DEFAULT_EMBEDDING_MODEL = "text-embedding-3-small"


def get_embedding_model_name(task: str = "embedding") -> str:
    """Return the configured embedding model slug for *task*."""
    task_config = _get_auxiliary_task_config(task)
    model = str(task_config.get("model", "") or "").strip()
    return model or _DEFAULT_EMBEDDING_MODEL


def embed_text(text: str, *, task: str = "embedding") -> Optional[List[float]]:
    """Return an embedding vector for *text* using the auxiliary embedding backend.

    Uses OpenAI-compatible ``/embeddings`` when available. Returns ``None`` when
    ...
    resolved_provider, resolved_model, resolved_base_url, resolved_api_key, _api_mode = (
        _resolve_task_provider_model(task)
    )
    model = resolved_model or get_embedding_model_name(task)
    timeout = _get_task_timeout(task, default=15.0)
    ...
    try:
        ...
        logger.debug("embed_text: client resolution failed: %s", exc)
        return None

    if client is None:
        return None

    effective_model = final_model or model
    try:
        ...
        logger.debug("embed_text: API call failed (%s): %s", resolved_provider, exc)
        return None
