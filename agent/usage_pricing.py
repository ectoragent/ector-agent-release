...
_OFFICIAL_DOCS_PRICING: Dict[tuple[str, str], PricingEntry] = {
    ...
    (
        ...
        source="official_docs_snapshot",
        source_url="https://docs.anthropic.com/en/docs/about-claude/pricing",
        pricing_version="anthropic-pricing-2026-05-08",
    ),
    (
        "anthropic",
        "claude-opus-4.6",
    ...
    (
        ...
        output_cost_per_million=Decimal("15.00"),
        cache_read_cost_per_million=Decimal("0.30"),
        cache_write_cost_per_million=Decimal("3.75"),  # 5m cache write
        source="official_docs_snapshot",
        source_url="https://docs.anthropic.com/en/docs/about-claude/pricing",
        pricing_version="anthropic-pricing-2026-05-08",
    ),
    ...
    (
        ...
        cache_write_cost_per_million=Decimal("1.25"),  # 5m cache write
        source="official_docs_snapshot",
        source_url="https://docs.anthropic.com/en/docs/about-claude/pricing",
        pricing_version="anthropic-pricing-2026-05-08",
    ),
    ...
    (
        ...
        pricing_version="openai-pricing-2026-05-08",
    ),
    (
        "openai",
        "gpt-4o-mini",
    ...
    (
        ...
        input_cost_per_million=Decimal("0.14"),          # cache miss
        output_cost_per_million=Decimal("0.28"),
        cache_read_cost_per_million=Decimal("0.0028"),  # cache hit (input)
        source="official_docs_snapshot",
    ...
    (
        ...
        input_cost_per_million=Decimal("0.435"),          # cache miss
        output_cost_per_million=Decimal("0.87"),
        cache_read_cost_per_million=Decimal("0.003625"),  # cache hit (input)
        source="official_docs_snapshot",
        source_url="https://api-docs.deepseek.com/quick_start/pricing/",
        pricing_version="deepseek-pricing-2026-04-26",
    ),
    (
        ...
        # Compatibility alias — server-side non-thinking mode of deepseek-v4-flash.
        input_cost_per_million=Decimal("0.14"),          # cache miss
        output_cost_per_million=Decimal("0.28"),
        cache_read_cost_per_million=Decimal("0.0028"),  # cache hit (input)
        source="official_docs_snapshot",
    ...
    (
        ...
        # Compatibility alias — server-side thinking mode of deepseek-v4-flash.
        input_cost_per_million=Decimal("0.14"),          # cache miss
        output_cost_per_million=Decimal("0.28"),
        cache_read_cost_per_million=Decimal("0.0028"),  # cache hit (input)
        source="official_docs_snapshot",
    ...
    (
        ...
        # Paid tier (<=200k prompts). Google also publishes a higher tier for >200k.
        input_cost_per_million=Decimal("1.25"),
        output_cost_per_million=Decimal("10.00"),
        cache_read_cost_per_million=Decimal("0.125"),
        source="official_docs_snapshot",
        source_url="https://ai.google.dev/gemini-api/docs/pricing",
        pricing_version="google-pricing-2026-05-08",
    ),
    (
        "google",
        "gemini-2.5-flash",
...
def _usage_attr(usage: Any, key: str, default: Any = None) -> Any:
    if usage is None:
        return default
...
def extract_actual_cost_usd(
    response_usage: Any,
    *,
    provider: Optional[str] = None,
    base_url: Optional[str] = None,
) -> Optional[Decimal]:
    """Extract provider-reported USD cost when present on the usage object.

    OpenRouter and some OpenAI-compatible gateways attach ``cost`` (or
    ``total_cost``) on the usage block of chat completion responses.
    """
    if not response_usage:
        return None

    for key in ("cost", "total_cost", "total_cost_usd", "cost_usd"):
        raw = _usage_attr(response_usage, key)
        if raw is None:
            continue
        try:
    ...
    if provider_name == "openrouter" or "openrouter.ai" in host:
        details = _usage_attr(response_usage, "cost_details")
        if isinstance(details, dict):
            for key in ("total_cost", "cost", "upstream_cost"):
                raw = details.get(key)
                if raw is None:
                    continue
                try:
