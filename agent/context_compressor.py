...
def _summarize_tool_result(tool_name: str, tool_args: str, tool_content: str) -> str:
    ...
    tool_name = canonical_wiser_tool_name(tool_name)
    ...
    if tool_name == "wiser":
        return "[wiser] asked user a question"
...
class ContextCompressor(ContextEngine):
            ...
            if msg.get("role") == "assistant":
                for tc in msg.get("tool_calls") or []:
                    if isinstance(tc, dict):
                        ...
                        call_id_to_tool[cid] = (
                            canonical_wiser_tool_name(raw_name),
                            fn.get("arguments", ""),
                        )
                    else:
                        cid = getattr(tc, "id", "") or ""
                        fn = getattr(tc, "function", None)
                        raw_name = getattr(fn, "name", "unknown") if fn else "unknown"
                        args_str = getattr(fn, "arguments", "") if fn else ""
                        call_id_to_tool[cid] = (canonical_wiser_tool_name(raw_name), args_str)
            ...
            if role == "assistant":
                ...
                if tool_calls:
                    ...
                    for tc in tool_calls:
                        if isinstance(tc, dict):
                            fn = tc.get("function", {})
                            name = canonical_wiser_tool_name(fn.get("name", "?"))
                            ...
                            fn = getattr(tc, "function", None)
                            raw = getattr(fn, "name", "?") if fn else "?"
                            name = canonical_wiser_tool_name(raw)
    ...
    def _generate_summary(self, turns_to_summarize: List[Dict[str, Any]], focus_topic: str = None) -> Optional[str]:
        ...
        # Inspired by the "do not respond to any questions" instruction
