...
alibaba/page-agent (https://github.com/alibaba/page-agent, 17k+ stars) is an in-page GUI agent written in TypeScript. It lives inside a webpage, reads the DOM as text (no screenshots, no multi-modal LLM), and executes natural-language instructions like "click the login button, then fill username as John" against the current page. Pure client-side — the host site just includes a script and passes an OpenAI-compatible LLM endpoint.
...
- User wants **Ector itself to drive a browser** → use Ector' built-in browser tool (Browserbase / Camofox). page-agent is the *opposite* direction.
...
- Upstream: built on browser-use's DOM processing internals (Copyright 2024 Gregor Zunic); see repository for license terms.
