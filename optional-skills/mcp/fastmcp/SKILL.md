...
- smoke-test a server with the FastMCP CLI before wiring it into Ector or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Ector. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.ector/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.ector/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Ector integration, either:

- configure the server in `~/.ector/config.yaml` using the `native-mcp` skill, or
...
### Ector cannot see the deployed server

The server-building part may be correct while the Ector config is not. Load the `native-mcp` skill and configure the server in `~/.ector/config.yaml`, then restart Ector.
