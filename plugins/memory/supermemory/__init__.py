...
def _load_supermemory_config(ector_home: str) -> dict:
    config = _default_config()
    config_path = Path(ector_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, ector_home: str) -> None:
    config_path = Path(ector_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `ector memory setup`.
        # All other options are documented for $ECTOR_HOME/supermemory.json
    ...
    def save_config(self, values, ector_home):
        ...
        _save_supermemory_config(sanitized, ector_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from ector_constants import get_ector_home
        self._ector_home = kwargs.get("ector_home") or str(get_ector_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._ector_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "ector", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "ector_memory", "target": target, "type": "explicit_memory"},
