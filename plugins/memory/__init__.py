...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with ector-agent)
2. User-installed providers: ``$ECTOR_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ECTOR_HOME/plugins/`` or None if unavailable."""
    try:
        from ector_constants import get_ector_home
        d = get_ector_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ECTOR_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_ector_user_memory.{name}"
...
def _get_active_memory_provider() -> Optional[str]:
    ...
    Returns the provider name (e.g. the plugin id) or None if no
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_ector_user_memory.{active_provider}.cli"
    try:
