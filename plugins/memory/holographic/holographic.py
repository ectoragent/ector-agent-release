...
HRRs are a ector symbolic architecture for encoding compositional structure
into fixed-width distributed representations. This module uses *phase vectors*:
each concept is a ector of angles in [0, 2π). The algebraic operations are:
...
References:
  ...
  Gayler (2004) — Ector Symbolic Architectures answer Jackendoff's challenges
"""
...
def encode_atom(word: str, dim: int = 1024) -> "np.ndarray":
    """Deterministic phase ector via SHA-256 counter blocks.
...
def bind(a: "np.ndarray", b: "np.ndarray") -> "np.ndarray":
    ...
    Binding associates two concepts into a single composite ector.
...
def unbind(memory: "np.ndarray", key: "np.ndarray") -> "np.ndarray":
    ...
    Unbinding retrieves the value associated with a key from a memory ector.
...
def phases_to_bytes(phases: "np.ndarray") -> bytes:
    """Serialize phase ector to bytes. float64 tobytes — 8 KB at dim=1024."""
    _require_numpy()
    return phases.tobytes()


def bytes_to_phases(data: bytes) -> "np.ndarray":
    """Deserialize bytes back to phase ector. Inverse of phases_to_bytes.
