"""disk_cleanup — ephemeral file cleanup for Ector Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ECTOR_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ECTOR_HOME and /tmp/ector-*
Never touches: ~/.ector/logs/ or any system directory.
"""
...
try:
    ...
    def get_ector_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ECTOR_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".ector").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ECTOR_HOME/logs/``."""
    return get_ector_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ECTOR_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ECTOR_HOME or ``/tmp/ector-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    ector_home = get_ector_home()
    try:
        path.resolve().relative_to(ector_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/ector-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("ector-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ECTOR_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ECTOR_HOME (but leave ECTOR_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "ector-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(ector_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == ector_home:
                continue
            try:
                rel_parts = dirpath.relative_to(ector_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    ector_home = get_ector_home()
    try:
        rel = path.resolve().relative_to(ector_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "ector-agent",
        }:
            return None
        ...
        # Path isn't under ECTOR_HOME (e.g. /tmp/ector-*) — fall through.
        pass
