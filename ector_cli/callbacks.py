...
These bridge terminal_tool's interactive prompts (sudo, approval, secrets)
into prompt_toolkit's event loop. Wiser uses ``cli._wiser_callback`` or the
Ink TUI gateway (``wiser.request`` / ``wiser.respond``).
"""
...
from ector_cli.presentation import cprint, _DIM, _RST
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.ector/.env and never exposed to the model.
    """
