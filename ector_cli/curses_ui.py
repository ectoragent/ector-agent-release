"""Shared curses-based UI components for Ector CLI.

Used by `ector tools` and `ector skills` for interactive checklists.
...
def _get_active_accent_color(curses_module) -> int:
    """Return a vivid cyan-blue accent color index when supported."""
    try:
        if curses_module.COLORS >= 256:
            # xterm-256: 45 is a bright cyan close to our accent (#00D1FF).
            return 45
    except Exception:
        pass
    # On limited palettes, cyan is the closest accent.
    return curses_module.COLOR_CYAN
    ...
    try:
        ...
        def _draw(stdscr):
            ...
            if curses.has_colors():
                ...
                curses.init_pair(1, _get_active_accent_color(curses), -1)
                curses.init_pair(2, _get_active_accent_color(curses), -1)
            ...
            while True:
                ...
                filtered_indices = [
                    i for i, label in enumerate(items)
                    if query.lower() in label.lower()
                ]
                if filtered_indices and cursor not in filtered_indices:
                ...
                try:
                    ...
                    stdscr.addnstr(
                        1, 0,
                        "  ↑↓ navegar  SPACE marcar  ENTER confirmar  ESC cancelar  Ctrl+C sair",
                        max_x - 1, curses.A_DIM,
                    )
                    stdscr.addnstr(
                        2, 0,
                        "  Dica: digite para filtrar as opções",
                        max_x - 1, curses.A_DIM,
                    )
                    if query:
                        stdscr.addnstr(3, 0, f"  filtro: {query}", max_x - 1, curses.A_DIM)
                except curses.error:
                    pass

                # Scrollable item list
                items_start = 5
                visible_rows = max_y - items_start - footer_rows
                if cursor < scroll_offset:
                    scroll_offset = cursor
                elif cursor >= scroll_offset + visible_rows:
                    scroll_offset = cursor - visible_rows + 1

                visible = filtered_indices[scroll_offset: scroll_offset + max(0, visible_rows)]
                for draw_i, i in enumerate(visible):
                ...
                if not filtered_indices:
                    try:
                        stdscr.addnstr(items_start, 0, "  (nenhuma opção encontrada)", max_x - 1, curses.A_DIM)
                    except curses.error:
                        pass
                ...
                if key in (curses.KEY_UP, ord("k")):
                    if filtered_indices:
                        pos = filtered_indices.index(cursor) if cursor in filtered_indices else 0
                        cursor = filtered_indices[(pos - 1) % len(filtered_indices)]
        ...
        flush_stdin()
        print(color("\nOperação cancelada pelo usuário.", Colors.DIM))
    ...
    try:
        ...
        def _draw(stdscr):
            ...
            while True:
                ...
                # Header (accent cyan — matches highlighted row)
                try:
                    hattr = curses.A_BOLD
                    if curses.has_colors():
                        hattr |= curses.color_pair(1)
                    ...
                    stdscr.addnstr(
                        row, 0,
                        "  \u2191\u2193 navegar  ENTER/SPACE selecionar  ESC cancelar  Ctrl+C sair",
                        max_x - 1, curses.A_DIM,
                    )
                    row += 1
                    stdscr.addnstr(row, 0, "  Dica: digite para filtrar as opções", max_x - 1, curses.A_DIM)
                    row += 1
                    if query:
                        stdscr.addnstr(row, 0, f"  filtro: {query}", max_x - 1, curses.A_DIM)
                        row += 1
                except curses.error:
                    pass
    ...
    try:
        ...
        def _draw(stdscr):
            ...
            while True:
                ...
                max_y, max_x = stdscr.getmaxyx()
                filtered_indices = [
                    i for i, label in enumerate(all_items)
                    if i == cancel_idx or query.lower() in label.lower()
                ]
                if filtered_indices and cursor not in filtered_indices:
                    cursor = filtered_indices[0]
                ...
                try:
                    ...
                    stdscr.addnstr(
                        1, 0,
                        "  ↑↓ navegar  ENTER confirmar  ESC/q cancelar  Ctrl+C sair",
                        max_x - 1, curses.A_DIM,
                    )
                    stdscr.addnstr(
                        2, 0,
                ...
                items_start = 5
                visible_rows = max_y - items_start
                if cursor < scroll_offset:
                    scroll_offset = cursor
                elif cursor >= scroll_offset + visible_rows:
                    scroll_offset = cursor - visible_rows + 1
