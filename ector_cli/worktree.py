...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        print("\033[31m✖ --worktree precisa ser executado dentro de um repositório git.\033[0m")
        print("  entre na pasta do seu projeto e depois execute ector -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"ector-{short_id}"
    branch_name = f"ector/{wt_name}"
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"\033[31m✖ Falha ao criar worktree: {result.stderr.strip()}\033[0m")
            return None
    except Exception as e:
        print(f"\033[31m✖ Falha ao criar worktree: {e}\033[0m")
        return None
    ...
    print(f"\033[32m✔ Worktree criado:\033[0m {wt_path}")
...
def _cleanup_worktree(info: Dict[str, str] = None) -> None:
    ...
    if has_unpushed:
        print(f"\n\033[33m▲ O worktree tem commits não enviados, mantendo: {wt_path}\033[0m")
        print(f"  Para limpar manualmente: git worktree remove --force {wt_path}")
    ...
    _active_worktree = None
    print(f"\033[32m✔ Worktree limpo: {wt_path}\033[0m")
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``ector/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("ector-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``ector/ector-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``ector -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("ector/ector-") or b.startswith("pr-"))
    ]
...
# - Accent: #00D1FF (headers, highlights)
# - Highlight: #00D1FF (secondary highlights)
# - Border: #00D1FF (borders, tertiary elements)
# - Light: #E6E6FA (text)
# - Dim: #00D1FF (muted text)

# ANSI building blocks for conversation display
_ACCENT_ANSI_DEFAULT = "\033[1;38;2;0;209;255m"  # True-color #00D1FF bold
_BOLD = "\033[1m"
_RST = "\033[0m"
