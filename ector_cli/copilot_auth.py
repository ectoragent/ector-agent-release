...
# OAuth device code flow constants (same client ID as Copilot CLI)
...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  → `copilot login` or `ector provider` to authenticate via OAuth\n"
...
def copilot_device_code_login(
    *,
    ...
    This replicates the flow used by the Copilot CLI.
    """
...
def copilot_request_headers(
    *,
    ...
    Replicates the header set used by the Copilot CLI.
    """
