...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with ector-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.ector/plugins/<name>/``
3. **Project plugins** – ``./.ector/plugins/<name>/`` (opt-in via
   ``ECTOR_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``ector_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "ector_agent.plugins"

_NS_PARENT = "ector_plugins"
...
class PluginManifest:
    ...
    # lookups and by ``ector plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``ector memory ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``ector <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.ector/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to ector_cli/. Two layouts are
        ...
        # 2. User plugins (~/.ector/plugins/)
        user_dir = get_ector_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.ector/plugins/)
        if _env_enabled("ECTOR_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".ector" / "plugins"
        ...
        for manifest in winners.values():
            ...
            if lookup_key in disabled or manifest.name in disabled:
                ...
                loaded.error = "desabilitado via configuração"
            ...
            if manifest.kind == "exclusive":
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "plugin exclusivo — ative via configuração <category>.provider"
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
            ...
            # Built-in backends auto-load — they ship with ector and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "não habilitado na configuração (execute `ector plugins enable {}` para ativar)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                loaded.error = "nenhuma função register()"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``ector_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``ector_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``ector tools`` TUI so plugin-provided toolsets appear
