...
from ector_cli import auth as auth_mod
...
from ector_cli.auth import (
    ...
    PROVIDER_REGISTRY,
    _getenv_provider_base_url_override,
    format_auth_error,
    resolve_provider,
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from ector_cli.models import copilot_model_api_mode
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    # When the caller is resolving for a specific target model (e.g. a /provider
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from ector_cli.models import azure_foundry_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("ECTOR_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'ector provider' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.ector/.env or run 'ector provider' to configure."
        )
    ...
    *,
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if pconfig.base_url_env_var:
            env_url = _getenv_provider_base_url_override(pconfig.base_url_env_var).strip().rstrip("/")
...
def resolve_runtime_provider(
    *,
    ...
    api_mode. Callers performing an explicit mid-session model switch should
    pass the new model here so api_mode is derived from the model they are
    switching TO, not the stale persisted default.
    """
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "ector-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        # Google AI Studio in config.yaml + Gemini CLI OAuth login (no API key).
        if provider == "gemini" and not has_usable_secret(api_key):
            try:
                oauth = resolve_gemini_oauth_runtime_credentials()
                return {
                    "provider": "google-gemini-cli",
                    "api_mode": "chat_completions",
                    "base_url": oauth.get("base_url", ""),
                    "api_key": oauth.get("api_key", ""),
                    "source": oauth.get("source", "google-oauth"),
                    "expires_at_ms": oauth.get("expires_at_ms"),
                ...
                if requested_provider not in ("auto",):
                    raise
                logger.info(
                    "Gemini API key missing and OAuth unavailable; "
                    "falling through to next provider."
                )
        # Honour model.base_url from config.yaml when the configured provider
