"""ector hooks — inspect and manage shell-script hooks.

Usage::

    ector hooks list
    ector hooks test <event> [--for-tool X] [--payload-file F]
    ector hooks revoke <command>
    ector hooks doctor

Consent records live under ``~/.ector/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.ector/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``ector hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: ector hooks {list|test|revoke|doctor}")
        print("Run 'ector hooks --help' for details.")
        return
...
def _empty_hooks_hint() -> str:
    _dhh = display_ector_home()
    return f"""
  Nenhum shell hook configurado em {_dhh}/config.yaml.

  Shell hooks são scripts que o Ector executa em eventos internos — antes ou
  depois de ferramentas, chamadas LLM, início/fim de sessão, etc.

  {_section_title("1. Adicione hooks em config.yaml")}

     hooks:
       pre_tool_call:
         - matcher: "terminal"
           command: "{_dhh}/agent-hooks/block-dangerous.sh"
           timeout: 10
       post_tool_call:
         - matcher: "write_file|patch"
           command: "{_dhh}/agent-hooks/auto-format.sh"

  {_section_title("2. Teste e valide")}

     ector hooks test pre_tool_call
     ector hooks doctor

  {_section_title("3. Saiba mais")}

     ector hooks --help
     https://ector.cc/docs/user-guide/cli

  [dim]Na primeira execução, cada script pede consentimento e fica registrado
  em {_dhh}/shell-hooks-allowlist.json.[/dim]
"""


def _print_rich(text: str) -> None:
...
# stdin a script sees under `ector hooks test` and `ector hooks doctor`
...
def _cmd_test(args) -> None:
    from ector_cli.config import load_config
    from ector_cli.plugins import VALID_HOOKS
    from agent import shell_hooks
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Ector wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ▲ script modified since approval "
                  ...
                  f"then `ector hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `ector hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `ector hooks doctor`.")
