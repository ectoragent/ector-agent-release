"""
Multi-provider authentication system for Ector Agent.

Supports OAuth flows (external providers) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.ector/auth.json with cross-process file locking.
...
from ector_cli.config import get_ector_home, get_config_path, read_raw_config
...
# Optional display names for non-inference auth providers (auth.json / pooled).
...
def _getenv_provider_base_url_override(env_var: str) -> str:
    """Lê sobrescrita de URL base do ambiente."""
    if not env_var:
        return ""
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "qwen-oauth": ProviderConfig(
        ...
        base_url_env_var="ECTOR_QWEN_BASE_URL",
    ),
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from ector_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.ector/.env file
        ...
        if pool and pool.has_credentials():
            ...
            if entry:
                ...
                if has_usable_secret(key):
                    return f"Erro de Autenticação ({provider_id}): {key}", f"credential_pool:{provider_id}"
    except Exception:
        pass
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Execute `ector provider` para se re-autenticar."

    if error.code == "subscription_required":
        return (
            "Nenhuma assinatura paga ativa encontrada em ector.cc. "
            "Por favor, adquira/ative uma assinatura e tente novamente."
        )

    if error.code == "insufficient_credits":
        return (
            "Os créditos da assinatura acabaram. "
            "Recarregue/renove os créditos em ector.cc e tente novamente."
        )

    if error.code == "temporarily_unavailable":
        return f"{error} Por favor, tente novamente em alguns segundos."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("ECTOR_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.ector/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_ector_home() / "auth.json"
    # Seat belt: if pytest is running and ECTOR_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ECTOR_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".ector" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ECTOR_HOME to a tmp_path in your test fixture, or run "
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Ector.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `ector logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'ector doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'ector provider' for available providers, or run 'ector doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'ector provider' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.ector/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = _getenv_provider_base_url_override("ECTOR_QWEN_BASE_URL").rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.ector/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `ector auth list` / `ector status`."""
    try:
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.ector/auth.json (not ~/.codex/)
#
# Ector maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Ector auth store (~/.ector/auth.json).
    ...
    if not state:
        raise AuthError(
            "Nenhuma credencial Codex armazenada. Execute `ector auth` para autenticar.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Estado de autenticação Codex sem tokens. Execute `ector auth` para reautenticar.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Autenticação Codex sem access_token. Execute `ector auth` para reautenticar.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Autenticação Codex sem refresh_token. Execute `ector auth` para reautenticar.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Ector auth store (~/.ector/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Ector auth state."""
    ...
    if response.status_code != 200:
        ...
        message = f"A atualização do token Codex falhou com status {response.status_code}."
            ...
            if isinstance(err, dict):
                ...
                if isinstance(err_obj, dict):
                    ...
                    if isinstance(nested_msg, str) and nested_msg.strip():
                        message = f"A atualização do token Codex falhou: {nested_msg.strip()}"
                # OAuth spec shape: {"error": "code_str", "error_description": "..."}
                    ...
                    if isinstance(err_desc, str) and err_desc.strip():
                        message = f"A atualização do token Codex falhou: {err_desc.strip()}"
        except Exception:
            pass
        ...
        if code == "refresh_token_reused":
            message = (
                "O token de atualização do Codex já foi consumido por outro cliente "
                "(ex: Codex CLI ou extensão VS Code). "
                "Execute `codex` no seu terminal para gerar novos tokens, "
                "depois execute `ector auth` para reautenticar."
            )
            relogin_required = True
    ...
    try:
        ...
        raise AuthError(
            "A atualização do token Codex retornou um JSON inválido.",
            provider="openai-codex",
    ...
    if not isinstance(refreshed_access, str) or not refreshed_access.strip():
        raise AuthError(
            "A resposta de atualização do token Codex não continha um access_token.",
            provider="openai-codex",
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Ector auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Ector's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("ECTOR_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Ector processes
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `ector auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `ector auth` and
    # `ector provider` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    if pconfig.base_url_env_var:
        env_url = _getenv_provider_base_url_override(pconfig.base_url_env_var)
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("ECTOR_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("ECTOR_COPILOT_ACP_ARGS", "").strip()
    args = shlex.split(raw_args) if raw_args else ["--acp", "--stdio"]
    base_url = _getenv_provider_base_url_override(pconfig.base_url_env_var) if pconfig.base_url_env_var else ""
    if not base_url:
        base_url = pconfig.inference_base_url
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    print(f"\n[yellow]Você não está logado no {target.title()}.[/yellow]")
    print(f"Execute [bold]ector login {target}[/bold] para começar.\n")
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set ECTOR_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `ector logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    *available_count*, when set, is shown under the title as
    ``Modelos disponíveis: N``.
    """
    from ector_cli.models import _format_price_per_mtok
    ...
    menu_heading = "Selecione o modelo padrão:"
    pricing_header = ""
    if has_pricing:
        pad = " " * 5
        pricing_header = (
            f"{pad}{'':>{name_col}} {'In':>{price_col}}  {'Out':>{price_col}}"
        )
        if has_cache:
            pricing_header += f"  {'Cache':>{cache_col}}"
        pricing_header += "  /Mtok"

    def _picker_description() -> str:
        lines: list[str] = []
        if available_count is not None:
            lines.append(f"  Modelos disponíveis: {available_count}")
        if pricing_header:
            lines.append(pricing_header)
        if _unavailable:
            _upgrade_url = (portal_url or "https://ector.cc").rstrip("/")
            lines.append("")
            lines.append(
                f"  Modelos indisponiveis (requer plano pago — upgrade em {_upgrade_url}):"
            )
            for mid in _unavailable:
                lines.append(f"    {_label(mid)}")
        return "\n".join(lines)

    from ector_cli.curses_ui import curses_radiolist, flush_stdin

    _ACTION_CUSTOM = "Inserir nome de modelo customizado"
    n_models = len(ordered)
    choice_labels = [_label(mid) for mid in ordered]
    choice_labels.append(_ACTION_CUSTOM)
    custom_idx = n_models
    ...
    idx = curses_radiolist(
        ...
        selected=0,
        cancel_returns=-1,
        description=_picker_description(),
    )
    flush_stdin()
    print()
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from ector_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'ector provider' or 'ector setup' instead."""
    print("O comando 'ector login' foi removido.")
    print("Use 'ector auth' para gerenciar credenciais,")
    print("'ector provider' para selecionar um provedor, ou 'ector setup' para a configuração completa.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.ector/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Ector-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Credenciais Codex existentes encontradas no armazenamento de autenticação do Ector.")
                try:
                    reuse = input("Usar credenciais existentes? (s/n): ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
                if reuse in ("", "s", "sim", "y", "yes"):
                    config_path = _update_config_for_provider("openai-codex", existing.get("base_url", DEFAULT_CODEX_BASE_URL))
                    print()
                    print("Login realizado com sucesso!")
                    print(f"  Configuração atualizada: {config_path} (model.provider=openai-codex)")
                    return
            else:
                print("As credenciais Codex existentes estão expiradas. Iniciando um novo login...")
        except AuthError:
            pass
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            print("Encontradas credenciais Codex CLI existentes em ~/.codex/auth.json")
            print("O Ector criará sua própria sessão para evitar conflitos com Codex CLI / VS Code.")
            try:
                do_import = input("Importar estas credenciais? (recomenda-se um login separado) (s/n): ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "n"
            if do_import in ("s", "sim", "y", "yes"):
                _save_codex_tokens(cli_tokens)
                base_url = os.getenv("ECTOR_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credenciais importadas. Nota: se o Codex CLI atualizar seu token,")
                print("o Ector continuará funcionando de forma independente com sua própria sessão.")
                print(f"  Configuração atualizada: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Ector gets its own OAuth session
    print()
    print("Conectando-se ao OpenAI Codex...")
    print("(O Ector cria sua própria sessão — não afetará o Codex CLI ou VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Ector auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider(creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    
    from ector_constants import display_ector_home
    user_display = "usuário" # placeholder if email not available
    print(f"\n[#00D1FF]Sucesso![/#00D1FF] Logado como [bold]{user_display}[/bold].")
    print(f"Seu token foi salvo com segurança em {display_ector_home()}/auth.json")
    print(f"  Estado de autenticação: {display_ector_home()}/auth.json")
    print(f"  Configuração atualizada: {config_path} (model.provider=openai-codex)")
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    try:
        ...
        raise AuthError(
            f"Falha ao solicitar código do dispositivo: {exc}",
            provider="openai-codex", code="device_code_request_failed",
        )

    if resp.status_code != 200:
        raise AuthError(
            f"A solicitação do código do dispositivo retornou status {resp.status_code}.",
    ...
    if not user_code or not device_auth_id:
        raise AuthError(
            "Resposta do código do dispositivo ausente campos necessários.",
            provider="openai-codex", code="device_code_incomplete",
        )

    # Step 2: Show user the code
    print("\n[bold]Para autenticar com o OpenAI Codex:[/bold]")
    print(f"1. Visite: [bold cyan]{device_data['verification_uri']}[/bold cyan]")
    print(f"2. Digite o código: [bold #00D1FF]{user_code}[/bold #00D1FF]")
        ...
        with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
            while _time.monotonic() - start < max_wait:
                ...
                if poll_resp.status_code == 200:
                    ...
                    raise AuthError(
                        f"O polling de autenticação do dispositivo retornou status {poll_resp.status_code}.",
    ...
    if code_resp is None:
        raise AuthError(
            "O login expirou após 15 minutos.",
    ...
    if not authorization_code or not code_verifier:
        raise AuthError(
            "Resposta da autenticação do dispositivo sem authorization_code ou code_verifier.",
    ...
    try:
        ...
        raise AuthError(
            f"A troca de token falhou: {exc}",
            provider="openai-codex", code="token_exchange_failed",
        )

    if token_resp.status_code != 200:
        raise AuthError(
            f"A troca de token retornou status {token_resp.status_code}.",
    ...
    if not access_token:
        raise AuthError(
            "A troca de token não retornou um access_token.",
...
def logout_command(args) -> None:
    ...
    if provider_id and not is_known_auth_provider(provider_id):
        print(f"Provedor desconhecido: {provider_id}")
        raise SystemExit(1)
    ...
    if not target:
        print("Nenhum provedor está logado no momento.")
        return
    ...
    if clear_provider_auth(target) or config_matches:
        ...
        print(f"[#00D1FF]Sucesso![/#00D1FF] Você saiu do {provider_name}.")
        if os.getenv("OPENROUTER_API_KEY"):
            print("O Ector usará o OpenRouter para inferência.")
        else:
            print("Execute `ector provider` ou configure uma chave API para usar o Ector.")
    else:
        print(f"Nenhum estado de autenticação encontrado para {provider_name}.")
