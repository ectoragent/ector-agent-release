"""Shared model-switching logic for CLI and gateway /provider commands.

Both the CLI (cli.py) and gateway (gateway/run.py) /provider handlers
...
- ``ector_cli.providers``        -- canonical provider identity + overlays
- ``ector_cli.model_normalize``  -- per-provider name formatting
...
from ector_cli.providers import (
...
_ECTOR_MODEL_WARNING = (
    "Modelos Ector Ector 3 e 4 NÃO são agentivos e não foram projetados "
    "para uso com o Ector Agent. Eles não possuem as capacidades de chamada "
    "de ferramentas necessárias para fluxos de trabalho agentivos. Considere usar "
    "um modelo agentivo (Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Ector Ector 3 / Ector 4 chat families.
# The previous substring check (`"ector" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``ector-brain:qwen3-14b-ctx16k`` that just
# happen to carry "ector" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   Ector/Ector-3-Llama-3.1-70B, ector-4-405b, openrouter/ector-3:70b
# Negative examples it must NOT match:
#   ector-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_ECTOR_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])ector[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_ector_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Ector Ector 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_ECTOR_NON_AGENTIC_RE.search(model_name))


def _check_ector_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Ector Ector 3/4 chat model."""
    if is_ector_non_agentic(model_name):
        return _ECTOR_MODEL_WARNING
    return ""
...
def parse_model_flags(raw_args: str) -> tuple[str, str, bool]:
    """Parse --provider and --global flags from /provider command args.
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "ector")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "ector")
...
def resolve_display_context_length(
    ...
    """Resolve the context length to show in /provider output.
    ...
    about Codex OAuth, Copilot, Ector, and falls back to models.dev for the
    rest.

    When ``custom_providers`` is provided, per-model ``context_length``
    overrides from ``custom_providers[].models.<id>.context_length`` are
    honored — this closes #15779 where ``/provider`` switch ignored user-set
    overrides.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                f"Provedor desconhecido '{explicit_provider}'. "
                f"Verifique 'ector provider' para provedores disponíveis ou defina-o "
                f"no config.yaml em 'providers:'."
            )
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nExecute 'ector doctor' — problemas de configuração detectados:"
            ...
            if pdef.base_url:
                ...
                if detected:
                    ...
                    return ModelSwitchResult(
                        ...
                        is_global=is_global,
                        error_message=(
                            f"Nenhum modelo detectado em {pdef.name} ({pdef.base_url}). "
                            f"Especifique o modelo explicitamente: /provider <nome-do-modelo> --provider {explicit_provider}"
                        ),
                    )
            else:
                return ModelSwitchResult(
                    ...
                    is_global=is_global,
                    error_message=(
                        f"O provedor '{pdef.name}' não tem URL base configurada. "
                        f"Especifique um modelo: /provider <nome-do-modelo> --provider {explicit_provider}"
                    ),
                )
            ...
            if key in MODEL_ALIASES:
                ...
                if fallback_result is not None:
                    ...
                    return ModelSwitchResult(
                        ...
                        is_global=is_global,
                        error_message=(
                            f"O alias '{key}' mapeia para {identity.vendor}/{identity.family} "
                            f"mas nenhum modelo correspondente foi encontrado em nenhum catálogo de provedor. "
                            f"Tente especificar o nome completo do modelo."
                        ),
                    )
            else:
    ...
    if provider_changed or explicit_provider:
        try:
            ...
            return ModelSwitchResult(
                ...
                is_global=is_global,
                error_message=(
                    f"Não foi possível resolver as credenciais para o provedor "
                    f"'{provider_label}': {e}"
                ),
            )
    else:
        try:
            ...
            # base_url adjustments for provider-specific quirks, etc.).
    ...
    try:
        ...
        validation = {
            ...
            "persist": False,
            "recognized": False,
            "message": f"Não foi possível validar `{new_model}`: {e}",
        }
    ...
    ector_warn = _check_ector_model_warning(new_model)
    if ector_warn:
        warnings.append(ector_warn)
...
# ---------------------------------------------------------------------------
# Authenticated providers listing (for /provider no-args display)
# ---------------------------------------------------------------------------
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from ector_cli/models.py (OPENROUTER_MODELS,
    ...
    # Build curated model lists keyed by ector provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "ector" shares OpenRouter's curated list if not separately defined
    if "ector" not in curated:
        curated["ector"] = curated["openrouter"]
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from ector_cli.models import fetch_ollama_cloud_models
        curated["ollama-cloud"] = fetch_ollama_cloud_models()

    # --- 1. Check Ector-mapped providers ---
    for ector_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(ector_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ECTOR_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                ...
                if store and ector_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # catalog so newly released models show up in the picker without
        # requiring a Ector release.
        model_ids = curated.get(ector_id, [])
        if ector_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(ector_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Ector-only providers (ector, openai-codex, copilot) ---
    from ector_cli.providers import ECTOR_OVERLAYS
    from ector_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Ector provider ID.
    # ECTOR_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Ector IDs ("copilot").
    _mdev_to_ector_slug = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ECTOR_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Ector slug — e.g. "github-copilot" → "copilot"
        ector_slug = _mdev_to_ector_slug.get(pid, pid)
        if ector_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or ector_slug in providers_store
                    or pid in pool_store or ector_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(ector_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", ector_slug, exc)
        ...
        # But the /provider picker is discovery-oriented — we WANT to show
        ...
        if ector_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(ector_slug)
        else:
            # Use curated list — look up by Ector slug, fall back to overlay key
            model_ids = curated.get(ector_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if ector_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(ector_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": ector_slug,
            "name": get_label(ector_slug),
            "is_current": ector_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "ector",
        ...
        seen_slugs.add(ector_slug.lower())

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ECTOR_OVERLAYS (keeps /provider in sync
    # with `ector provider`).
    try:
        from ector_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Ector's canonical write key (matches
            ...
            # Ector writes ``models:`` as a dict keyed by model id
            # (see ector_cli/main.py::_save_custom_provider); older
            ...
            # explicit models: dict — avoid a misleading zero count in /provider.
            if not models_list:
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Ector's own writer uses; a hyphen variant is accepted
            ...
            # active model. Ector's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, ector_cli/config.py) already consume that dict.
