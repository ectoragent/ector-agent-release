"""Slash command definitions and autocomplete for the ECTOR CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    # Sessão
    CommandDef("new", "Inicia uma nova sessão (novo ID de sessão + histórico)", "Sessão"),
    CommandDef("reset", "Formata o agente (memória, sessões, FTS/RAG e logs)", "Sessão",
               cli_only=True, args_hint="[--hard]"),
    CommandDef("clear", "Limpa a tela e inicia uma nova sessão", "Sessão",
               cli_only=True),
    CommandDef("redraw", "Força o redesenho da interface (recupera de drift no terminal)", "Sessão",
               cli_only=True),
    CommandDef("history", "Mostra o histórico da conversa", "Sessão",
               cli_only=True),
    CommandDef("save", "Salva a conversa atual", "Sessão",
               cli_only=True),
    CommandDef("retry", "Tenta novamente a última mensagem (reenvia para o agente)", "Sessão"),
    CommandDef("undo", "Remove a última troca entre usuário/assistente", "Sessão"),
    CommandDef("title", "Define um título para a sessão atual", "Sessão",
               args_hint="[nome]"),
    CommandDef("branch", "Ramifica a sessão atual (explora um caminho diferente)", "Sessão",
               aliases=("fork",), args_hint="[nome]"),
    CommandDef("compress", "Comprime manualmente o contexto da conversa", "Sessão",
               args_hint="[tópico de foco]"),
    CommandDef("rollback", "Lista ou restaura checkpoints do sistema de arquivos", "Sessão",
               args_hint="[número]"),
    CommandDef("snapshot", "Cria ou restaura snapshots de configuração/estado do ECTOR", "Sessão",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Encerra todos os processos em segundo plano", "Sessão"),
    CommandDef("approve", "Aprova um comando perigoso pendente", "Sessão",
               gateway_only=True, args_hint="[sessão|always]"),
    CommandDef("deny", "Nega um comando perigoso pendente", "Sessão",
               gateway_only=True),
    CommandDef("background", "Executa um prompt em segundo plano", "Sessão",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "Mostra agentes ativos e tarefas em execução", "Sessão",
               aliases=("tasks",)),
    CommandDef("queue", "Coloca um prompt na fila para o próximo turno (não interrompe)", "Sessão",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "Injeta uma mensagem após a próxima chamada de ferramenta sem interromper", "Sessão",
               args_hint="<prompt>"),
    CommandDef("status", "Mostra informações da sessão", "Sessão"),
    CommandDef("profile", "Mostra o nome do perfil ativo e o diretório principal", "Informações"),
    CommandDef("sethome", "Define este chat como o canal principal", "Sessão",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "Retoma uma sessão nomeada anteriormente", "Sessão",
               args_hint="[nome]"),

    # Configuração
    CommandDef("config", "Mostra a configuração atual", "Configuração",
               cli_only=True),
    CommandDef("provider", "Troca o provedor/modelo para esta sessão", "Configuração",
               args_hint="[modelo] [--provider nome] [--global]"),
    CommandDef("gquota", "Mostra o uso da cota do Google Gemini Code Assist", "Informações",
               cli_only=True),

    CommandDef("personality", "Define uma personalidade predefinida", "Configuração",
               args_hint="[nome]"),
    CommandDef("statusbar", "Alterna a barra de status de contexto/modelo", "Configuração",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "Cicla a exibição de progresso das ferramentas: off -> novo -> tudo -> detalhado",
               "Configuração", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("yolo", "Alterna o modo YOLO (pula todas as aprovações de comandos perigosos)",
               "Configuração"),
    CommandDef("plan", "Ativa o modo Plano (planeja sem executar) para esta sessão",
               "Configuração"),
    CommandDef("agent", "Volta ao modo Agente (execução normal) para esta sessão",
               "Configuração"),
    CommandDef("reasoning", "Gerencia o esforço de raciocínio e exibição", "Configuração",
               args_hint="[nível|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "Alterna o modo rápido — OpenAI Priority Processing / Anthropic Fast Mode (Normal/Rápido)", "Configuração",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("voice", "Alterna o modo de voz", "Configuração",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "Controla o que o Enter faz enquanto o ECTOR está trabalhando", "Configuração",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),
    CommandDef("rag", "Controla o RAG de sessões (ligado/desligado e limites)", "Configuração",
               args_hint="[on|off|status|max-results <n>|max-chars <n>|min-query-chars <n>]",
               subcommands=("on", "off", "status", "max-results", "max-chars", "min-query-chars")),

    # Ferramentas & Habilidades
    CommandDef("tools", "Gerencia ferramentas: /tools [list|disable|enable] [nome...]", "Ferramentas & Habilidades",
               args_hint="[list|disable|enable] [nome...]", cli_only=True),
    CommandDef("toolsets", "Lista os conjuntos de ferramentas disponíveis", "Ferramentas & Habilidades",
               cli_only=True),
    CommandDef("skills", "Pesquisa, instala, cria, inspeciona ou gerencia habilidades",
               "Ferramentas & Habilidades", cli_only=True,
               subcommands=(
                   "search", "browse", "inspect", "install", "create",
                   "sync", "remove", "update", "publish", "list",
               )),
    CommandDef("cron", "Gerencia tarefas agendadas", "Ferramentas & Habilidades",
               cli_only=True, args_hint="[subcomando]",
               subcommands=("list", "parity", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("reload", "Recarrega variáveis .env na sessão em execução", "Ferramentas & Habilidades",
               cli_only=True),
    CommandDef("reload-mcp", "Recarrega servidores MCP da configuração", "Ferramentas & Habilidades",
               aliases=("reload_mcp",)),
    CommandDef("browser", "Conecta ferramentas de navegador ao seu Chrome ativo via CDP", "Ferramentas & Habilidades",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "Lista plugins instalados e seus status",
               "Ferramentas & Habilidades", cli_only=True),

    # Informações
    CommandDef("commands", "Navega por todos os comandos e habilidades (paginado)", "Informações",
               gateway_only=True, args_hint="[página]"),
    CommandDef("help", "Mostra os comandos disponíveis", "Informações"),
    CommandDef("restart", "Reinicia o gateway graciosamente após drenar execuções ativas", "Sessão",
               gateway_only=True),
    CommandDef("usage", "Mostra o uso de tokens e limites de taxa para a sessão atual", "Informações"),
    CommandDef("stats", "Mostra estatísticas de uso e métricas", "Informações",
               aliases=("insights",), args_hint="[dias]"),
    CommandDef("platforms", "Mostra o status das plataformas de gateway/mensageria", "Informações",
               cli_only=True, aliases=("gateway",)),
    CommandDef("copy", "Copia a última resposta do assistente para a área de transferência", "Informações",
               cli_only=True, args_hint="[número]"),
    CommandDef("paste", "Anexa imagem da área de transferência", "Informações",
               cli_only=True),
    CommandDef("image", "Anexa um arquivo de imagem local para seu próximo prompt", "Informações",
               cli_only=True, args_hint="<caminho>"),
    CommandDef("debug", "Faz upload do relatório de debug (info do sistema + logs) e obtém links compartilháveis", "Informações"),

    # Identidade (ector.cc)
    CommandDef("login", "Autentica sua conta Ector (ector.cc)", "Sessão"),
    CommandDef("me", "Mostra nickname, usuário e status da sessão", "Informações",
               aliases=("whoami",)),
    CommandDef("logout", "Encerra a sessão Ector atual", "Sair",
               aliases=("signout",)),

    # Sair
    CommandDef("quit", "Sai do CLI", "Sair",
...
def _build_description(cmd: CommandDef) -> str:
    ...
    if cmd.args_hint:
        return f"{cmd.description} (uso: /{cmd.name} {cmd.args_hint})"
...
for _cmd in COMMAND_REGISTRY:
    if not _cmd.gateway_only:
        ...
        for _alias in _cmd.aliases:
            COMMANDS[f"/{_alias}"] = f"{_cmd.description} (atalho para /{_cmd.name})"
...
def should_bypass_active_session(command_name: str | None) -> bool:
    ...
    specific Level-2 handler in gateway/run.py (/stop, /new, /provider,
    ...
    the pending queue — which meant a mid-run /provider (or /reasoning,
    /voice, /stats, /title, /resume, /retry, /undo, /compress,
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`ector_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/ector`` subcommand mapping, and
    ...
    """
    try:
        from ector_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``ector skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    a standalone Slack slash command (e.g. ``/btw``, ``/stop``, ``/provider``),
    matching Discord's and Telegram's model where every command is a
    first-class slash and not a ``/ector <verb>`` subcommand.
    ...
    avoidance. ``/ector`` is always reserved as the first entry so the
    legacy ``/ector <subcommand>`` form keeps working for anything that
    ...
    # Reserve /ector as the catch-all top-level command.
    entries.append(("ector", "Fale com o ECTOR ou execute um subcomando", "[subcomando] [args]"))
    seen.add("ector")
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        for alias in cmd.aliases:
            ...
            _add(alias, f"Atalho para /{cmd.name} — {cmd.description}", cmd.args_hint or "")
...
def slack_app_manifest(request_url: str = "https://ector-agent.local/slack/commands") -> dict[str, Any]:
    ...
    for name, desc, usage in slack_native_slashes():
        entry = {
            "command": f"/{name}",
            "description": desc or f"Executar /{name}",
            "should_escape": False,
            "url": request_url,
        }
        if usage:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /ector handler.

    Maps both canonical names and aliases so /ector bg do stuff works
    the same as /ector background do stuff.

    Plugin-registered slash commands are included so ``/ector <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _context_completions(self, word: str, limit: int = 30):
        ...
        _STATIC_REFS = (
            ("@diff", "Diff da árvore de trabalho do Git"),
            ("@staged", "Diff do staging do Git"),
            ("@file:", "Anexar um arquivo"),
            ("@folder:", "Anexar uma pasta"),
            ("@git:", "Log do Git com diffs (ex: @git:5)"),
            ("@url:", "Buscar conteúdo web"),
        )
        ...
        try:
            ...
            if "none".startswith(sub_lower) and "none" != sub_lower:
                yield Completion(
                    ...
                    display="none",
                    display_meta="limpar sobreposição de personalidade",
                )
    ...
    def _model_completions(self, sub_text: str, sub_lower: str):
        """Yield completions for /provider from config aliases + built-in aliases."""
        seen = set()
    ...
    def get_completions(self, document, complete_event):
        ...
        try:
            ...
            for cmd_name, cmd_info in get_plugin_commands().items():
                if cmd_name.startswith(word):
                    desc = str(cmd_info.get("description", "Comando de plugin"))
