...
# DingTalk API ``source`` field — default is the ecosystem id DingTalk expects.
# Override via DINGTALK_REGISTRATION_SOURCE only after validating with DingTalk.
...
def _ensure_qrcode_installed() -> bool:
    ...
    # Try uv first (Ector convention), then pip
    for cmd in (
...
def dingtalk_qr_auth() -> Optional[Tuple[str, str]]:
    ...
    """
    from ector_cli.setup import print_info, print_success, print_warning, print_error

    print()
    print_info("  Initializing DingTalk device authorization...")
    print_info("  Note: DingTalk's scan page uses their ecosystem branding.")
    print_info("        Safe to use — credentials are registered for Ector.")
